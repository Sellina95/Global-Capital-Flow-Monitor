...
def _strength_label(key: str, pct_change: Optional[float]) -> str:
    ...
    # liquidity series: often slow-moving; treat as "Noise" threshold a bit tighter
...
# =========================
# Liquidity Filter (rates/dollar/vix)
# =========================
...
def fed_plumbing_filter(market_data: Dict[str, Any]) -> str:
    ...
    - "매일 업데이트"가 아니라도, last available 유동성 상태를 항상 보여주기
    - 방향/변화율은 prev가 있을 때만 의미 있음
    """
    ...
    if net["today"] is not None:
        level_notes.append(f"- **NET_LIQ level:** {_fmt_num(net['today'], 1)}")
    if tga["today"] is not None:
        level_notes.append(f"- **TGA level:** {_fmt_num(tga['today'], 1)}")
    if rrp["today"] is not None:
        level_notes.append(f"- **RRP level:** {_fmt_num(rrp['today'], 1)}")
...
def cross_asset_filter(market_data: Dict[str, Any]) -> str:
    ...
    lines.append("- **추가 이유:** 한 지표의 변화가 다른 자산군에 어떻게 전파되는지(연쇄효과) 파악")
    lines.append("")

    if us10y_dir == 1:
        lines.append("- **금리 상승(US10Y↑)** → 달러 강세(DXY↑) 및 밸류에이션 부담 확대 가능")
    elif us10y_dir == -1:
        lines.append("- **금리 하락(US10Y↓)** → 달러 약세(DXY↓) 및 성장/리스크 자산에 우호적일 수 있음")
    else:
        lines.append("- **금리 보합(US10Y→)** → 다른 자산의 독립 변수(정책/실적/지정학) 영향 확대")

    if vix_dir == 1:
        lines.append("- **변동성 상승(VIX↑)** → 리스크 회피 강화, 방어적 포지셔닝 확대")
    elif vix_dir == -1:
        lines.append("- **변동성 하락(VIX↓)** → 리스크 선호 회복, 포지션 재구축 가능")
    else:
        lines.append("- **변동성 보합(VIX→)** → ‘관망’ 또는 이벤트 대기 가능성")

    if wti_dir == 1:
        lines.append("- **유가 상승(WTI↑)** → 인플레 재점화/금리 상방 압력 변수")
    elif wti_dir == -1:
        lines.append("- **유가 하락(WTI↓)** → 물가 부담 완화/경기 둔화 해석이 갈릴 수 있음")
    else:
        lines.append("- **유가 보합(WTI→)** → 물가 변수 영향 제한")
...
def build_strategist_commentary(market_data: Dict[str, Any]) -> str:
    ...
    sections.append("")
    # ✅ NEW: Fed Plumbing (TGA/RRP/NET_LIQ)
