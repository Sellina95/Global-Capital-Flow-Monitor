...
- **변동성 변화 없음(VIX→)** → 달러와 유가는 큰 변화 없음
- **유가 하락(WTI↓)** → **리스크 회피, 금리 인하(US10Y↓)**
...
- **유가 하락(WTI↓)** → **리스크 감소**: 유가 하락은 경기 둔화 우려 완화
...
- **손해를 보는 주체:**
  ...
  - Oil Producers (lower oil prices)

### 🔍 8) Cause Filter
- **질문:** 무엇이 이 시장 움직임을 일으켰는가?
- **핵심 신호:** US10Y(→) / DXY(↑) / WTI(↓) / VIX(→)
- **판정:** **달러 강세(DXY 상승) 유가 하락(WTI 하락) **
- **이유:** 직접적인 원인 파악

### 🔄 9) Direction Filter
- **질문:** 시장이 어느 방향으로, 얼마나 움직였는가?
- **핵심 신호:** US10Y(Noise, →) / DXY(Noise, ↑) / WTI(Mild, ↓) / VIX(Noise, →)
- **판정:** **NO MOVEMENT**
- **근거:** 변화가 미미한 '노이즈' 또는 '의미 있는 변화'인지 분석 중

### ⏳ 10) Timing Filter
- **질문:** 시장 변화가 단기, 중기, 장기 관점에서 어떤 영향을 미치는지?
- **핵심 신호:** US10Y(0.00% short-term, 4.19% medium-term, 4.19% long-term) / DXY(0.04% short-term, 98.70% medium-term, 98.74% long-term) / VIX(0.00% short-term, 14.51% medium-term, 14.51% long-term)
