...
- **미국 10년물 금리**: 4.241  (-0.05% vs 4.243)
- **WTI 유가**: 94.450  (+0.02% vs 94.430)
- **변동성 지수 (VIX)**: 25.430  (-0.04% vs 25.440)
- **원/달러 환율**: 1492.910  (+0.05% vs 1492.180)
...
- 현재 시장은 단기 리스크 선호가 회복되는 국면이며, 구조 신호는 혼조.
- 유동성 여건이 개선되며 리스크 자산에 우호적이다.
- 전략적으로는 노출 확대가 가능하나, 규율 기반으로 80% 내에서 단계적으로 접근한다.

## 🧭 So What? (Decision Layer)
- **Risk Stance:** **INCREASE** *(target exposure: 80%)*
- **Context:** phase=RISK-ON (부분 정렬) / liquidity=UP-HIGH / credit_calm=True / geo=NORMAL

## 🗺️ Scenario Framework (Base / Bull / Bear)

### 🔹 Base Case
- 조건: 유동성 혼조 + 크레딧 안정 유지 / 변동성 급등 없이 박스권 장세 지속
- 전략: 노출 80% 유지, 퀄리티 중심 선별적 접근
...
- **VIX 레벨:** 25.43 → **High (Risk-off bias)**
...
- **Policy Bias: MIXED (혼조) (WEAK, score=+0.5) | REAL_RATEΔ +0.030 / FCIΔ +0.000 / DXYΔ N/A / US10YΔ -0.002**
...
- **방향(전일 대비):** HYG(→) / LQD(→)
- **HYG:** today 79.505 / prev 79.505 / pct 0.00%
- **LQD:** today 108.855 / prev 108.855 / pct 0.00%
- **판정:** **CREDIT NEUTRAL**
...
- VIX **(Noise, -0.04%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Noise, +0.05%)** → 원화 약세/수급 부담
...
- DEBUG: pct XLK=-0.6343749099399263, XLF=-0.00996725465972539, XLE=0.0, XLRE=-0.02351992925894455
...
- **Top Drivers:**
  ...
  - USDCNH: z_used=+0.35 (z1d=+0.23, z5d=+0.52, raw_w=0.18, norm_w=0.13) → contrib=+0.05
  - EMB: z_used=+0.47 (z1d=+0.09, z5d=-1.33, raw_w=0.12, norm_w=0.09) → contrib=+0.04
...
- **강도:** US10Y(Mild) / DXY(N/A) / WTI(Noise) / VIX(Noise)
...
- **Today snapshot:** US10Y(4.241), DXY(N/A), VIX(25.43)
...
- **Structure Bias:** Policy Bias: MIXED (혼조) (WEAK, score=+0.5) | REAL_RATEΔ +0.030 / FCIΔ +0.000 / DXYΔ N/A / US10YΔ -0.002
- **Sentiment (Fear&Greed):** 30.276148056585225 (NEUTRAL)
- **Credit Calm (HY OAS<4):** True
- **Liquidity (NET_LIQ):** dir=UP / level=HIGH
- **Phase:** RISK-ON (부분 정렬)

- **🎯 Final Risk Action:** **INCREASE**
- **Risk Budget (0~100):** **80**
- **Narrative:** 구조=MIXED / 심리=NEUTRAL / 유동성=증가/높음 / 크레딧=안정 → Phase=RISK-ON (부분 정렬)
...
- **Context:** phase=RISK-ON (부분 정렬) / liquidity=UP-HIGH / structure=MIXED / credit_calm=True
