...
- **미국 10년물 금리**: 4.136  (+0.00% vs 4.136)
- **달러 인덱스**: 98.914  (+0.19% vs 98.726)
- **WTI 유가**: 87.540  (+1.04% vs 86.640)
- **변동성 지수 (VIX)**: 24.930  (-2.24% vs 25.500)
- **원/달러 환율**: 1473.730  (+0.51% vs 1466.290)
...
- **VIX 레벨:** 24.93 → **High (Risk-off bias)**
...
- **Policy Bias: MIXED (혼조) (WEAK, score=+0.0) | REAL_RATEΔ -0.020 / FCIΔ +0.000 / DXYΔ +0.188 / US10YΔ +0.000**
...
- **방향(전일 대비):** HYG(↓) / LQD(↓)
- **HYG:** today 80.040 / prev 80.170 / pct -0.16%
- **LQD:** today 110.060 / prev 110.820 / pct -0.69%
- **판정:** **CREDIT NEUTRAL**
...
- WTI **(Clear, +1.04%)** → 인플레 재자극 가능성
- VIX **(Clear, -2.24%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Strong, +0.51%)** → 원화 약세/수급 부담
- HYG (High Yield ETF) **(Mild, -0.16%)** → 크레딧 스트레스↑
- LQD (IG Bond ETF) **(Clear, -0.69%)** → 우량채 약세(리스크온 성향)
...
- DEBUG: pct XLK=0.0, XLF=-0.5364602581776101, XLE=-1.2784112652779362, XLRE=-0.13960301053994298
Correlation Break Detected:
- WTI ↑ but XLE ↓ (Energy)
...
- **Top Drivers:**
  ...
  - TR10Y_SPREAD: z_used=+1.67 (mode=level, raw_w=0.05, norm_w=0.04) → contrib=+0.06
  - EMB: z_used=+0.63 (z1d=-0.02, z5d=-1.55, raw_w=0.12, norm_w=0.09) → contrib=+0.06
  - WTI: z_used=+0.65 (z1d=+0.07, z5d=+1.52, raw_w=0.10, norm_w=0.07) → contrib=+0.05
...
- **Country ETF Crash?** Yes (EEM, EIS, EMB, EWJ, FXI, GLD, SPY)
- **Extreme Country Risk:** EEM, EIS, EMB, EWJ, FXI, GLD, SPY
...
- **판정:** **달러 강세(DXY↑) + 유가 상승(WTI↑) + 변동성 완화(VIX↓)**

### 🔄 10) Direction Filter
- **질문:** 오늘 움직임은 ‘노이즈’인가 ‘의미 있는 변화’인가?
- **강도:** US10Y(Noise) / DXY(Clear) / WTI(Clear) / VIX(Clear)
...
- **Today snapshot:** US10Y(4.136), DXY(98.914), VIX(24.93)
...
- **Structure Bias:** Policy Bias: MIXED (혼조) (WEAK, score=+0.0) | REAL_RATEΔ -0.020 / FCIΔ +0.000 / DXYΔ +0.188 / US10YΔ +0.000
- **Sentiment (Fear&Greed):** 25.538814286656606 (FEAR)
