...
- **미국 10년물 금리**: 4.146  (+1.62% vs 4.080)
- **달러 인덱스**: 99.065  (+0.25% vs 98.822)
- **WTI 유가**: 78.870  (+3.63% vs 76.110)
- **변동성 지수 (VIX)**: 23.750  (+12.29% vs 21.150)
- **원/달러 환율**: 1480.680  (+1.29% vs 1461.770)
...
- **Context:** phase=RISK-OFF (긴축/불안·리스크 회피) / liquidity=UP-HIGH / credit_calm=True / geo=ELEVATED
...
- **VIX 레벨:** 23.75 → **High (Risk-off bias)**
...
- **현실(FCI):** level=EASY (완화) / dir(→) | as of: 2026-03-04 (FRED last available)
- **유인(Real Rates):** level=NEUTRAL (중립) / dir(↓) | as of: 2026-03-04 (FRED last available)
...
- **Policy Bias: TIGHTENING (긴축) (STRONG, score=+2.5) | REAL_RATEΔ +0.030 / FCIΔ +0.000 / DXYΔ +0.243 / US10YΔ +0.066**
...
- **Spread as of:** 2026-03-04 (FRED latest)
- **HY_OAS level:** 2.97% → **COOL (낮은 공포)**
- **방향(전일 대비):** HY_OAS(↓) / -3.57%
- **판정:** **CREDIT CALM**
...
- **방향(전일 대비):** HYG(↓) / LQD(↓)
- **HYG:** today 80.080 / prev 80.400 / pct -0.40%
- **LQD:** today 110.530 / prev 110.970 / pct -0.40%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, +1.62%)** → 완화 기대 약화/금리 부담
- DXY **(Clear, +0.25%)** → 달러 강세/신흥국 부담
- WTI **(Strong, +3.63%)** → 인플레 재자극 가능성
- VIX **(Strong, +12.29%)** → 심리 악화/리스크오프
- 원/달러(USDKRW) **(Strong, +1.29%)** → 원화 약세/수급 부담
- HYG (High Yield ETF) **(Mild, -0.40%)** → 크레딧 스트레스↑
- LQD (IG Bond ETF) **(Mild, -0.40%)** → 우량채 약세(리스크온 성향)
...
- DEBUG: pct XLK=0.24313239902345493, XLF=-0.524272733521693, XLE=0.5161077106513823, XLRE=-0.959776474074136
...
- **Geo Stress Score (z-composite):** **+1.07**  *(Level: ELEVATED)*
- **Top Drivers:**
  - USDCNH: z_used=+2.40 (w=0.18) → contrib=+0.43
  - VIX: z_used=+1.54 (w=0.18) → contrib=+0.28
  - EMB: z_used=+2.00 (w=0.12) → contrib=+0.24
  - EEM: z_used=+1.96 (w=0.10) → contrib=+0.20
...
- **Today snapshot:** US10Y(4.146), DXY(99.065), VIX(23.75)
...
- **Structure Bias:** Policy Bias: TIGHTENING (긴축) (STRONG, score=+2.5) | REAL_RATEΔ +0.030 / FCIΔ +0.000 / DXYΔ +0.243 / US10YΔ +0.066
- **Sentiment (Fear&Greed):** 28.045696054552103 (FEAR)
...
- **Narrative:** 구조=TIGHTENING / 심리=FEAR / 유동성=증가/높음 / 크레딧=안정 → Phase=RISK-OFF (긴축/불안·리스크 회피)
...
- **Context:** phase=RISK-OFF (긴축/불안·리스크 회피) / liquidity=UP-HIGH / structure=TIGHTENING / credit_calm=True
