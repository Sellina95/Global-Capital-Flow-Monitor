...
- **미국 10년물 금리**: 4.285  (+0.94% vs 4.245)
- **WTI 유가**: 99.310  (+4.82% vs 94.740)
- **변동성 지수 (VIX)**: 27.190  (+5.51% vs 25.770)
- **원/달러 환율**: 1497.530  (+0.30% vs 1493.080)
...
- **Context:** phase=RISK-OFF (부분 정렬) / liquidity=UP-HIGH / credit_calm=True / geo=NORMAL
...
- **VIX 레벨:** 27.19 → **High (Risk-off bias)**
...
- **현실(FCI):** level=EASY (완화) / dir(→) | as of: 2026-03-12 (FRED last available)
- **유인(Real Rates):** level=NEUTRAL (중립) / dir(↓) | as of: 2026-03-12 (FRED last available)
...
- **Policy Bias: TIGHTENING (긴축) (MODERATE, score=+1.5) | REAL_RATEΔ +0.040 / FCIΔ +0.000 / DXYΔ N/A / US10YΔ +0.040**
...
- **Liquidity as of:** 2026-03-11 (FRED latest)
- **NET_LIQ level:** 5808157.4
- **TGA level:** 838186.0
- **RRP level:** 0.552
...
- **Spread as of:** 2026-03-12 (FRED latest)
- **HY_OAS level:** 3.17% → **WARM (경계)**
- **방향(전일 대비):** HY_OAS(↑) / +2.59%
- **판정:** **CREDIT WATCH**
...
- **방향(전일 대비):** HYG(↓) / LQD(↓)
- **HYG:** today 79.200 / prev 79.525 / pct -0.41%
- **LQD:** today 108.170 / prev 108.825 / pct -0.60%
- **판정:** **CREDIT NEUTRAL**
...
- WTI **(Strong, +4.82%)** → 인플레 재자극 가능성
- VIX **(Strong, +5.51%)** → 심리 악화/리스크오프
- 원/달러(USDKRW) **(Clear, +0.30%)** → 원화 약세/수급 부담
- HYG (High Yield ETF) **(Clear, -0.41%)** → 크레딧 스트레스↑
- LQD (IG Bond ETF) **(Clear, -0.60%)** → 우량채 약세(리스크온 성향)
...
- DEBUG: pct XLK=-1.4480185449088343, XLF=-0.670462866457388, XLE=0.2782400120924916, XLRE=-0.5999319179091169
...
- **3D Avg Score:** +0.35
- **Geo Momentum:** +0.31 *(Status: RISING)*
- **Top Drivers:**
  - EMB: z_used=+2.11 (z1d=-2.02, z5d=-2.25, raw_w=0.12, norm_w=0.09) → contrib=+0.19
  - USDCNH: z_used=+0.87 (z1d=+1.15, z5d=+0.44, raw_w=0.18, norm_w=0.13) → contrib=+0.12
  - WTI: z_used=+1.37 (z1d=+1.09, z5d=+1.78, raw_w=0.10, norm_w=0.07) → contrib=+0.10
  - EEM: z_used=+1.15 (z1d=-1.08, z5d=-1.25, raw_w=0.10, norm_w=0.07) → contrib=+0.08
...
- **Country ETF Crash?** Yes (EIS, SPY)
- **Extreme Country Risk:** EIS, SPY
...
- **판정:** **금리 상승(US10Y↑) + 유가 상승(WTI↑) + 변동성 확대(VIX↑)**

### 🔄 10) Direction Filter
- **질문:** 오늘 움직임은 ‘노이즈’인가 ‘의미 있는 변화’인가?
- **강도:** US10Y(Strong) / DXY(N/A) / WTI(Strong) / VIX(Strong)
...
- **Today snapshot:** US10Y(4.285), DXY(N/A), VIX(27.19)
...
- **Structure Bias:** Policy Bias: TIGHTENING (긴축) (MODERATE, score=+1.5) | REAL_RATEΔ +0.040 / FCIΔ +0.000 / DXYΔ N/A / US10YΔ +0.040
- **Sentiment (Fear&Greed):** 26.68307661616072 (FEAR)
...
- **Narrative:** 구조=TIGHTENING / 심리=FEAR / 유동성=증가/높음 / 크레딧=안정 → Phase=RISK-OFF (부분 정렬)
...
- **Context:** phase=RISK-OFF (부분 정렬) / liquidity=UP-HIGH / structure=TIGHTENING / credit_calm=True
...
- **Rationale (top drivers):**
  ...
  - OW Health Care: +2: 리스크오프 → 퀄리티 선호
  - UW Real Estate: -2: 긴축 → 금리 민감/레버리지 비용 부담
  - UW Utilities: -1: 유동성 공급 → 방어 섹터 상대매력 감소
  - UW Technology: +2: 유동성 공급 → 베타 확장 환경
