...
### 🎯 Exposure Framework
- **Base Exposure (전략 기준): 56%**
- **Final Exposure (실행 기준): 56%**

- **Portfolio Stance:** HOLD / 56%
...
- **Why:** SEW STABLE → 실시간 이상징후 없음 → Divergence ALIGNED → 구조·가격·수급 정렬 → Narrative Action=HOLD 반영 → Warning Score 1 → 경미한 이상신호, 모니터링 강화 → Tactical=HOLD / Flow=👀 EARLY TRACE(4) / Drift=NO DRIFT(0) / Gamma=🟢 POSITIVE GAMMA (WEAK) → Tactical HOLD/MONITOR → 최종판단 변경 없음
...
- **미국 10년물 금리**: 4.310  (-0.30% vs 4.323)
- **달러 인덱스**: 98.510  (-0.29% vs 98.800)
- **WTI 유가**: 94.400  (-1.51% vs 95.850)
- **변동성 지수 (VIX)**: 18.710  (-3.11% vs 19.310)
- **원/달러 환율**: 1479.950  (+0.13% vs 1477.960)
...
- **VIX 레벨:** 18.71 → **Mid (Neutral/Mixed)**
...
- **현실(FCI):** level=EASY (완화) / dir(→) | as of: 2026-04-27 (FRED last available)
- **유인(Real Rates):** level=NEUTRAL (중립) / dir(→) | as of: 2026-04-27 (FRED last available)
...
- **Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ +0.000 / FCIΔ +0.000 / DXYΔ -0.290 / US10YΔ -0.013**
...
- **Spread as of:** 2026-04-23 (FRED latest)
...
- **방향(전일 대비):** HYG(↑) / LQD(↑)
- **HYG:** today 80.480 / prev 80.370 / pct 0.14%
- **LQD:** today 109.600 / prev 109.520 / pct 0.07%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, -0.30%)** → 완화 기대 강화
- DXY **(Clear, -0.29%)** → 달러 약세/리스크 선호
- WTI **(Strong, -1.51%)** → 물가 부담 완화
- VIX **(Strong, -3.11%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Mild, +0.13%)** → 원화 약세/수급 부담
- HYG (High Yield ETF) **(Mild, +0.14%)** → 크레딧 위험선호↑
- LQD (IG Bond ETF) **(Noise, +0.07%)** → 우량채 강세(리스크오프 성향)
...
- **WTI:** 🟢 UP | Short-term: SHORT DOWN | 1D=+0.99% / 5D=+6.38% | Strength: MEDIUM
- **DXY:** 🟡 PULLBACK | Short-term: SHORT DOWN | 1D=-0.08% / 5D=+0.39% | Strength: LOW
- **GOLD:** 🟡 REBOUND | Short-term: SHORT UP | 1D=+0.44% / 5D=-1.33% | Strength: LOW
...
- **Cosine Similarity Score:** 0.225
- **Similarity Signal:** Weak Historical Match
- **Top Similarity Matches:**
  - Taiwan_Tension: 0.225
  - China_Trade_2018: 0.189
  - Ukraine_2022: 0.140
- **Top Drivers:**
  - USDCNH: z_used=+0.37 (z1d=+0.23, z5d=+0.59, raw_w=0.18, norm_w=0.14) → contrib=+0.05
  - EEM: z_used=-0.66 (z1d=+1.18, z5d=-0.10, raw_w=0.10, norm_w=0.08) → contrib=-0.05
  - BDRY: z_used=-1.17 (z1d=-1.40, z5d=-0.83, raw_w=0.05, norm_w=0.04) → contrib=-0.05
  - KR10Y_SPREAD: z_used=-0.69 (mode=level, raw_w=0.08, norm_w=0.06) → contrib=-0.04
...
- **Country ETF Crash?** Yes (EIS, EWJ, VXX)
- **Extreme Country Risk:** EIS, EWJ, VXX
...
**핵심 신호:** 장단기차(53.00bp) | 실질금리(1.92%) | DXY(98.51)
*(as of: RealRate: 2026-04-27 / FRED last available)*
...
- **Today snapshot:** US10Y(4.310), DXY(98.510), VIX(18.71)

### 🏗️ 12) Structural Filter (v3)
- **질문:** 글로벌 화폐 가치와 에너지 패권 등 '판'의 변화가 있는가?
- **핵심 신호:** US10Y(↓) / DXY(↓) / GOLD(↑) / VIX(↓) / WTI(↓)
- **Meaningful Move Check:** DXY=-0.29352318478714434 / GOLD=0.36555454027041173 / US10Y=-0.30071736252022396 / VIX=-3.107200404267048 / WTI=-1.5127772262131831
- **판정:** **NEUTRAL**
...
- **Narrative:** 구조=EASING / 심리=NEUTRAL / 유동성=감소/중간 / 크레딧=안정 / 드리프트=NO DRIFT (REOPENING / DEMAND_BOOM) / 수급=1.64 ⚠️ 수급 다소 과열 → Phase=RISK-ON (완화 기대·리스크 선호)
...
- **Positioning Data:** Z-Score: `1.64` (>2.2 시 Run) | Gamma: `1.80` (<0.5 시 Run) | CTA: `1.0` (추세 변곡점 확인)
...
**Context:** phase=RISK-ON (완화 기대·리스크 선호) / T10Y2Y=0.53 (MODERATE STEEP) / VIX=18.71 (VOLATILITY NORMAL) / liquidity=DOWN-MID / credit=True

**Signal Priority:** VOL > LIQ > CURVE > CREDIT > PHASE > FLOW > MOM

**Flow Overlay:** flow_score=4 / flow_state=👀 EARLY TRACE / drift_label=REOPENING / DEMAND_BOOM / gamma=🟢 POSITIVE GAMMA (WEAK)
**Flow Notes:** Gamma POSITIVE → 리더 섹터 가점

**Overweight:** Industrials, Consumer Staples, Health Care, Technology, Utilities

**Underweight:** Energy, Real Estate, Consumer Discretionary

**Scoreboard:**
- Industrials: +2.5  (+1 CURVE, +1 PHASE, +0.5 FLOW, = +2.5)
- Consumer Staples: +2  (+2 LIQ, = +2)
- Health Care: +2  (+2 LIQ, = +2)
- Technology: +2  (-2 LIQ, +1 PHASE, +1 FLOW, +2 MOM, = +2)
- Utilities: +1  (+1 LIQ, = +1)
...
- **Strategic Exposure (15):** **30%**

| Sector | Score | Divergence | **Weight in Portfolio** | **Action** |
| :--- | :---: | :---: | :---: | :--- |
| Technology | +2.5 | ALIGNED | **7.1%** | HOLD |
| Industrials | +2.2 | ALIGNED | **7.3%** | HOLD |
| Consumer Staples | +2 | ALIGNED | **6.6%** | HOLD |
| Health Care | +2 | ALIGNED | **6.6%** | HOLD |
| Utilities | +1 | ALIGNED | **2.3%** | HOLD |
...
1. Industrials (priority_score=-0.0, score=2.2, weight=7.3%, div=ALIGNED, mom=0)
2. Consumer Staples (priority_score=-0.29, score=2, weight=6.6%, div=ALIGNED, mom=0)
3. Health Care (priority_score=-0.29, score=2, weight=6.6%, div=ALIGNED, mom=0)
4. Utilities (priority_score=-0.3, score=1, weight=2.3%, div=ALIGNED, mom=0)
5. Technology (priority_score=-1.19, score=2.5, weight=7.1%, div=ALIGNED, mom=2)
...
| Industrials | XLI | 7.3% | SMALL |
| Consumer Staples | XLP | 6.6% | SMALL |
| Health Care | XLV | 6.6% | SMALL |
| Utilities | XLU | 2.3% | SMALL |
...
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.1752919183580128
- **Z-Score (5d):** -0.10304806755261296

### EIS
- **Crash?** True
- **Risk Level:** EXTREME
- **Z-Score (1d):** -0.4107087137965958
- **Z-Score (5d):** -0.8638012618077012
...
- **Crash?** True
- **Risk Level:** EXTREME
- **Z-Score (1d):** 0.13841658811349125
- **Z-Score (5d):** -1.0147458265222558

### FXI
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.9160224440406048
- **Z-Score (5d):** -0.5331392175101085

### GLD
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.27567582407806757
- **Z-Score (5d):** -0.47290124259220445

### SPY
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.7517962789338211
- **Z-Score (5d):** 0.13852605723566772

### VXX
- **Crash?** True
- **Risk Level:** EXTREME
- **Z-Score (1d):** 0.15522045797618034
- **Z-Score (5d):** 0.2166926092341116
