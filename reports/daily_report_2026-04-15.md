...
- **국면 전환 감지:** 🚨 **RISK-OFF (부분 정렬)** → **RISK-ON (완화 기대·리스크 선호)**

---

## 📊 Daily Macro Signals

- **미국 10년물 금리**: 4.256  (-0.95% vs 4.297)
- **달러 인덱스**: 98.120  (-0.25% vs 98.370)
- **WTI 유가**: 91.280  (-7.87% vs 99.080)
- **변동성 지수 (VIX)**: 18.360  (-3.97% vs 19.120)
- **원/달러 환율**: 1476.840  (-0.36% vs 1482.180)
...
- **Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ +0.000 / FCIΔ +0.000 / DXYΔ -0.250 / US10YΔ -0.041**
...
- **방향(전일 대비):** HYG(↑) / LQD(↑)
- **HYG:** today 80.500 / prev 80.260 / pct 0.30%
- **LQD:** today 109.980 / prev 109.620 / pct 0.33%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, -0.95%)** → 완화 기대 강화
- DXY **(Clear, -0.25%)** → 달러 약세/리스크 선호
- WTI **(Strong, -7.87%)** → 물가 부담 완화
- VIX **(Strong, -3.97%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Clear, -0.36%)** → 원화 강세/수급 개선
- HYG (High Yield ETF) **(Mild, +0.30%)** → 크레딧 위험선호↑
- LQD (IG Bond ETF) **(Mild, +0.33%)** → 우량채 강세(리스크오프 성향)
...
- **Top Similarity Matches:**
  ...
  - Red_Sea: -0.193
- **Top Drivers:**
  - VIX: z_used=-1.15 (z1d=-0.49, z5d=-2.13, raw_w=0.18, norm_w=0.14) → contrib=-0.16
  - WTI: z_used=-2.06 (z1d=-1.82, z5d=-2.42, raw_w=0.10, norm_w=0.08) → contrib=-0.16
  - EMB: z_used=-1.64 (z1d=+0.92, z5d=+2.72, raw_w=0.12, norm_w=0.09) → contrib=-0.15
  - EEM: z_used=-1.51 (z1d=+1.01, z5d=+2.27, raw_w=0.10, norm_w=0.08) → contrib=-0.12
...
**핵심 신호:** 장단기차(50.00bp) | 실질금리(1.92%) | DXY(98.12)
...
- **Today snapshot:** US10Y(4.256), DXY(98.120), VIX(18.36)
...
- **Structure Bias:** Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ +0.000 / FCIΔ +0.000 / DXYΔ -0.250 / US10YΔ -0.041 (정상)
- **Sentiment (Fear&Greed):** 57.0679561383722 (NEUTRAL)
- **Credit Calm:** True
...
- **Narrative:** 구조=EASING / 심리=NEUTRAL / 유동성=증가/중간 / 크레딧=안정 / 수급=1.17 → Phase=RISK-ON (완화 기대·리스크 선호)
...
- **Positioning Data:** Z-Score: `1.17` (>2.2 시 Run) | Gamma: `1.00` (<0.5 시 Run) | CTA: `1.0` (추세 변곡점 확인)
...
- **VIX Level:** 18.36 (NORMAL) | **Change:** -3.97%
...
**Context:** phase=RISK-ON (완화 기대·리스크 선호) / T10Y2Y=0.50 (MODERATE STEEP) / VIX=18.36 (VOLATILITY NORMAL) / liquidity=UP-MID / credit=True
...
- **Total Target Exposure:** **85%** (from Filter 15)
...
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.9039188256498685
- **Z-Score (5d):** 1.0334401548788226
...
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.8921841453525108
- **Z-Score (5d):** 2.617918283232108
