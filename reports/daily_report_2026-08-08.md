# 🌍 Global Capital Flow – Daily Brief
**Date:** 2026-08-08
**Data as of:** 2026-08-07

🏦 GLOBAL MACRO DAILY — PM FINAL BRIEF

🧭 1. Executive Summary
Markets are operating in a GOLDILOCKS regime as liquidity conditions remain up and volatility pressures have increased. Participation remains 👀 EARLY TRACE, while positioning risk sits at 2.2. Maintain disciplined exposure and avoid adding marginal risk.
Rally Confidence: 🟢 HIGH
Participation and leadership support trend continuation. Risk exposure may be maintained, subject to volatility and positioning controls.

🌍 2. Macro Regime
Liquidity: 🟢 Improving
Flow: 🟡 Early Trace
Volatility: 🟢 Stable
Structure: EASING

⚠️ 3. Key Macro Conflict

2) Positioning Crowding
• POS_Z = 2.21
Interpretation:
Positioning remains elevated. Main risk is an air-pocket correction rather than structural collapse.

📊 4. Cross Asset Summary
Equities: 🟢 Stable
Bonds: 🟡 Neutral
USD: 🔻 Weaker
Oil: 🔻 Falling
Volatility: 🟢 Stable

Interpretation:
A weaker dollar and lower volatility are easing financial conditions.
This supports risk assets, provided breadth and credit remain stable.

🧠 5. Market Interpretation
Current market is best described as 'DISINFLATION'.
Liquidity is slowing, but systemic stress remains contained.
Credit markets remain stable, although participation quality requires monitoring.

⚠️ 6. Risk Matrix
Inflation Risk      🟡 Moderate
Liquidity Risk      🟢 Low
Positioning Risk    🔴 Elevated
Credit Risk         🟢 Low
Geopolitical Risk   🟢 Low

🏭 7. Sector Implication
🟢 Preferred:
- Quality leaders
- Confirmed relative-strength sectors

🔴 Avoid:
- Weak breadth / theory-only sector bets

🎯 8. Tactical Recommendation
Recommendation: HOLD (POS_OVERHEATED) / TAKE PROFIT
Confidence: MEDIUM
Rationale:
- No actionable alignment
- Positioning overheat
Maintain defensive discipline, avoid aggressive risk expansion, and prioritize cash plus resilient sectors.

📈 9. Internal Rotation Monitor
RSP vs SPY
Yesterday: -0.39%
Today: +0.08%
Change: +0.47%

QQQE vs QQQ
Yesterday: -0.34%
Today: +0.44%
Change: +0.77%

SMH vs SPY
Yesterday: -0.38%
Today: +1.35%
Change: +1.73%

IWM vs SPY
Yesterday: -0.79%
Today: +0.50%
Change: +1.29%


Interpretation:
Participation is broadening while semiconductor leadership remains supportive.
Market participation appears healthier than a pure mega-cap driven rally.

🏁 10. Final PM View
Action: HOLD (POS_OVERHEATED)
Exposure: 87%
Bias: GOLDILOCKS
Key focus: monitor liquidity, participation quality, and positioning risk.

---

# 🔬 ENGINE DIAGNOSTICS

## ⚡ Strategic War Room (통합 대응)
> **시스템 상태: 🚨 ALERT**
> **판단 요약: 포지셔닝 과열 감지 / Hard Deadman은 아니나 추격보다 리스크 축소 우선**
### 🎯 Exposure Framework
- **Base Exposure (전략 기준): 87%**
- **Final Exposure (실행 기준): 87%**

- **Portfolio Stance:** HOLD (POS_OVERHEATED) / 87%

- **[14번 구조·수급 괴리]:** 🚨 **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.21). 반전 가능성 상존.
### 🟢 Current SEW Status
- **SEW:** RISK_COMPRESSION | ⚠️ Risk Compression 발동 (권장 익스포저 55%)
- **Event Type:** NORMAL → 정상 상태 / 구조적 리스크 없음
- **Spike Monitor:** Spike 0 / Extreme 0
- **Current Reason:** ⚠️ CROWDING RISK: POS_Z Extreme (2.32) → Risk Compression (55%)

- **[15번 Hard Deadman]:** ✅ PASS
- **[14번 수급 시그널]:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🔬 Structural Layer (12.5~12.8)
- **Structural Layer:**
  - Growth Sustainability → **DURABLE_GROWTH** (Growth structure looks durable: demand, financing, and policy conditions are broadly supportive.)
  - Flow Authenticity → **REAL_ACCUMULATION** (Institutional participation appears broad and persistent.)
  - Leadership Breadth → **BROAD_LEADERSHIP** (Leadership is broadening across sectors, confirming healthier market participation.)
  - Positioning Stress → **SQUEEZE_RISK** (Positioning is stretched and vulnerable to squeeze-driven reversals.)

## 🎯 Final Decision (War Room Override)
- **Final Action:** **HOLD (POS_OVERHEATED)**
- **Final Exposure:** **87%**
- **Base Context:** phase=GOLDILOCKS / narrative=HOLD (POS_OVERHEATED) / base_exposure=87%
- **SEW:** RISK_COMPRESSION / NORMAL
- **Divergence:** N/A / **MONITOR REVERSAL (RUN 액션 준비)**
- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / NONE / score=7
- **Flow:** 👀 EARLY TRACE / score=4
- **Gamma:** 🟡 POSITIVE-TRANSITION
- **Tactical Action:** REDUCE / TAKE PROFIT / MEDIUM
- **Positioning:** pos_z=2.21
- **Warning Score:** 1 (6.6 섹터 상관관계 붕괴)
- **Tactical Why:** No actionable alignment / Positioning overheat
- **Why:** SEW STABLE → 실시간 이상징후 없음 → Divergence 비정렬 → 방어적 태도 유지 → 상위 레이어(SEW/Divergence)가 Narrative보다 우선 → Warning Score 1 → 경미한 이상신호, 모니터링 강화 → Tactical=REDUCE / Flow=👀 EARLY TRACE(4) / Drift=⚡ STRUCTURAL DRIFT(7) / Gamma=🟡 POSITIVE-TRANSITION → Tactical REDUCE → 방어 기조 유지 / 총노출 추가 감산 없이 배분 보수화

### 🚩 Market Regime Status
- **국면 전환 감지:** 🚨 **TRANSITION / MIXED (전환·혼조)** → **GOLDILOCKS**
- **Structural Regime:** **DISINFLATION**

---

## 📊 Daily Macro Signals

- **미국 10년물 금리**: 4.660  (-0.21% vs 4.670)
- **달러 인덱스**: 99.604  (-0.36% vs 99.960)
- **WTI 유가**: 77.080  (-1.23% vs 78.040)
- **변동성 지수 (VIX)**: 14.900 (-1.65% vs 15.150)
- **원/달러 환율**: 1407.450  (-0.96% vs 1421.160)

---

## 🧭 Strategist Commentary (Seyeon’s Filters)

### 🧩 1) Market Regime Filter
- **정의:** 지금 어떤 장(場)인지 판단하는 *시장 국면 필터*
- **추가 이유:** 같은 지표도 ‘국면’에 따라 의미가 완전히 달라지기 때문

- **VIX 레벨:** 14.90 → **Mid (Neutral/Mixed)**
- **핵심 조합(전일 대비 방향):** US10Y(↓) / DXY(↓) / VIX(↓)
- **판정:** **GOLDILOCKS | Flow:  (Flow Weak)**
- **근거:** Macro Narrative=DISINFLATION / Policy=MIXED / Credit=COOL

### 💧 2) Liquidity Filter (Enhanced)
- **질문:** 시장에 새 돈이 들어오는가, 말라가는가?
- **추가 이유:** US10Y/DXY/VIX는 ‘시장의 기대’를 보여주고, FCI는 ‘현실의 압박’을, Real Rates는 ‘위험을 감수할 유인’을 보여준다.

- **기대(가격) 신호:** US10Y(↓) / DXY(↓) / VIX(↓)
- **현실(FCI):** value=-0.529 / level=EASY (완화) / update=low-frequency | as of: 2026-08-08 (latest available)
- **유인(Real Rates):** value=2.430 / level=RESTRICTIVE (유인↓) / dir(→) | as of: 2026-08-08 (latest available)
- **판정:** **LIQUIDITY MIXED / FRAGILE (혼조·취약)**
- **근거:** 기대는 완화지만 FCI 압박 또는 실질금리 유인↓ → 리스크자산 지속성 약화 리스크
- **Note:** FCI는 저빈도 금융환경 프록시로 level 중심 해석, Real Rates는 영업일 기준 변화 방향을 함께 반영함

### 🏛️ 3) Policy Filter (with Expectations)
- **질문:** 중앙은행·정책 환경은 완화인가, 긴축인가?

- **가격(현재) 신호:** US10Y(↓) / DXY(↓) / VIX(↓)
- **Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ +0.000 / FCI value=-0.529 (low-frequency) / DXYΔ -0.356 / US10YΔ -0.010**
- **Expectations: dict received.**

- **판정:** **POLICY EASING (완화)**
- **근거:** 금리↓ + 달러↓ (+VIX 안정) → 완화 쪽
- **한줄요약 ~~** 구조=EASING (완화)(MODERATE)는 참고, 가격=POLICY EASING (완화) 중심 → 최종 POLICY EASING (완화)

### 🧰 4) Fed Plumbing Filter (TGA/RRP/Net Liquidity)
- **질문:** 시장의 ‘달러 체력’은 늘고 있나, 줄고 있나?
- **추가 이유:** 금리·달러가 안정적이어도 유동성이 빠지면 리스크 자산은 쉽게 흔들릴 수 있음
- **Liquidity as of:** 2026-08-05 (FRED latest)
- **NET_LIQ level:** 5841241.3
- **TGA level:** 907324.0
- **RRP level:** 1.650
- **방향(전일 대비):** TGA(↓) / RRP(↓) / NET_LIQ(↑)
- **판정:** **LIQUIDITY SUPPORTIVE (완만한 유동성 우호)**
- **근거:** Net Liquidity↑ → 시장 내 달러 여력 개선
- **Note:** TGA/RRP/WALCL은 매일 갱신되지 않을 수 있어, 리포트에는 ‘최근 available 값’을 반영함

### 🌡️ 4.2) High Yield Spread Filter (HY OAS)
- **질문:** 시장 공포의 ‘온도’는 올라가고 있나, 내려가고 있나?
- **추가 이유:** HYG/LQD가 ‘방향’이라면, HY Spread는 ‘강도(얼마나 무서워하는지)’를 보여줌
- **Spread as of:** 2026-08-06 (FRED latest)
- **HY_OAS level:** 2.71% → **COOL (낮은 공포)**
- **방향(전일 대비):** HY_OAS(↓) / -1.45%
- **판정:** **CREDIT CALM**
- **근거:** HY 스프레드 낮음 → 크레딧 스트레스 제한 / 스프레드가 좁혀지는 중 → 공포 온도 완화
- **Note:** HY OAS는 매일 갱신되지 않을 수 있어, ‘최근 available 값’을 반영함

### 🧾 4.5) Credit Stress Filter (HYG vs LQD)
- **질문:** 크레딧 시장이 먼저 ‘리스크오프’를 말하고 있는가?
- **추가 이유:** HYG가 LQD보다 약해지면, 시장이 ‘위험을 감수할 이유가 없다’고 판단하기 시작했을 가능성
- **방향(전일 대비):** HYG(↑) / LQD(↑)
- **HYG:** today 79.610 / prev 79.460 / pct 0.19%
- **LQD:** today 106.550 / prev 106.360 / pct 0.18%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, -0.21%)** → 완화 기대 강화
- DXY **(Strong, -0.36%)** → 달러 약세/리스크 선호
- WTI **(Clear, -1.23%)** → 물가 부담 완화
- VIX **(Clear, -1.65%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Strong, -0.96%)** → 원화 강세/수급 개선
- HYG (High Yield ETF) **(Mild, +0.19%)** → 크레딧 위험선호↑
- LQD (IG Bond ETF) **(Mild, +0.18%)** → 우량채 강세(리스크오프 성향)

### 🧩 6) Cross-Asset Filter (자산군 연쇄 반응 분석)
- **추가 이유:** 단일 지표의 노이즈를 제거하고, 매크로 충격이 자산군 전반으로 확산되는 **전이 경로(Transmission Path)**를 파악하기 위함

- **금리 하락(US10Y↓)** → 할인율 압박 완화 → 달러 약세(DXY↓) 유도: **위험자산(Growth/EM) 선호 심리 강화 및 유동성 환경 개선**
- **변동성 하락(VIX↓)** → 심리 개선(Risk-On): **자산군 전반의 위험 수용 여력(Risk Appetite) 회복 및 랠리 지속 가능성**
- **유가 하락(WTI↓)** → 물가 부담 완화: **실질 구매력 회복 및 긴축 압력 완화(Dovish Tilt) 가능성 시사**

> **[Strategic Note]:** 위 연쇄 반응이 역사적 상관관계에서 벗어날 경우, **6.5) Correlation Break Monitor**를 통해 국면 전환 여부를 정밀 판별함

### 🌊 Drift Monitor (v4)
- **정의:** 누적 흐름 + ATR 기반 강도 감지

- **SPY:** 🟢 UP | Short-term: SHORT UP | 1D=+0.59% / 5D=+3.49% | Strength: MEDIUM
- **WTI:** 🔴 DOWN | Short-term: MIXED | 1D=-0.27% / 5D=-8.96% | Strength: MEDIUM
- **DXY:** 🔴 DOWN | Short-term: MIXED | 1D=-0.37% / 5D=-0.20% | Strength: LOW
- **GOLD:** 🟢 UP | Short-term: SHORT UP | 1D=+3.76% / 5D=+8.70% | Strength: HIGH

- **Drift Score:** 7
- **State:** **⚡ STRUCTURAL DRIFT**
- **Label:** NEUTRAL
- **SEW Combo Signal:** NONE

- **Market Drift Summary:**
  - Equity (SPY): 🟢 UP / SHORT UP
  - Oil (WTI): 🔴 DOWN / MIXED
  - Dollar (DXY): 🔴 DOWN / MIXED
  - Gold (GOLD): 🟢 UP / SHORT UP

- **Drivers:**
  - SPY positive drift
  - Credit supports risk
  - Sector breadth expanding
  - Broad cyclical leadership
  - Dollar not restrictive
  - Gold event move

### ⚠ 6.5) Correlation Break Monitor
⚠ Market Closed / Stale Data → Correlation signals evaluated conservatively.

No significant correlation break detected.

### ⚠ 6.6) Sector Correlation Break Monitor
⚠ Market Closed / Stale Data → Sector signals evaluated conservatively.

Correlation Break Detected:
- US10Y ↓ but XLF ↓

So What?
- 결론: **섹터 ‘공식’이 깨진 구간** → 총노출 추가 감산보다 **섹터 배분 보수화 + 리더 중심**

### 🧩 7) Risk Exposure Filter (숨은 리스크 분석)
- **추가 이유:** 숫자는 괜찮아 보여도 그 뒤에 숨은 리스크를 식별하기 위함

- **VIX 하락(VIX↓)** → 심리 안정: 리스크 수용 여력 개선
- **금리 하락(US10Y↓)** → 완화 기대/할인율 부담 완화 가능
- **달러 약세(DXY↓)** → 위험자산 선호/신흥국 부담 완화 가능
- **유가 하락(WTI↓)** → 물가 부담 완화 가능

### 🛰️ 7.2) Geopolitical Early Warning Monitor (FX/Commodities Composite)
⚠ Market Closed / Stale Data → Price-based geo signals muted.

- **Geo Stress Score (z-composite):** **-0.58**  *(Level: NORMAL)*
- **Coverage:** 100% *(used weight: 1.30 / defined weight: 1.30)*
- **3D Avg Score:** -0.29
- **Geo Momentum:** -0.29 *(Status: FALLING)*

**Historical Pattern Match (Cosine Similarity):**
- **Closest Historical Match:** Iran_Crisis_2020
- **Cosine Similarity Score:** -0.082
- **Similarity Signal:** Weak Historical Match
- **Top Similarity Matches:**
  - Iran_Crisis_2020: -0.082
  - Red_Sea: -0.111
  - Israel_2023: -0.117
- **Top Drivers:**
  - GOLD: z_used=+1.89 (z1d=+1.24, z5d=+2.87, raw_w=0.12, norm_w=0.09) → contrib=+0.17
  - USDCNH: z_used=-1.04 (z1d=-0.84, z5d=-1.33, raw_w=0.18, norm_w=0.14) → contrib=-0.14
  - VIX: z_used=-0.93 (z1d=-0.20, z5d=-2.02, raw_w=0.18, norm_w=0.14) → contrib=-0.13
  - EMB: z_used=-0.94 (z1d=+0.88, z5d=+1.04, raw_w=0.12, norm_w=0.09) → contrib=-0.09
- **Missing/Skipped:** None
- **Sovereign Spread factors included:** KR10Y_SPREAD, JP10Y_SPREAD, DE10Y_SPREAD, IL10Y_SPREAD

**Trade Information:**
- 지정학 스트레스는 여전히 정상 범위에 있지만 최근 압력이 완화되고 있는 중입니다. 경계 유지.
- 역사적 위기 패턴 유사도는 낮습니다. 현재는 **Iran_Crisis_2020** 유형과 가장 가깝지만, 전면적 지정학 쇼크보다는 제한적·국지적 리스크 모니터링 구간으로 해석됩니다.
- **Country ETF Crash?** Yes (GLD)
- **Extreme Country Risk:** GLD

### ⚡ 7.3) Pseudo Gamma Filter
- **정의:** 옵션 데이터 없이 시장의 감마 환경을 추론
- **주의:** Dealer Gamma Bias 숫자와 Pseudo Gamma State는 서로 다른 레이어

- **Pseudo Gamma State:** 🟡 POSITIVE-TRANSITION
- **Dealer Gamma Bias:** 0.99 (NEUTRAL / transition zone)
- **Bias:** VIX는 안정적이나 Drift가 형성 중
- **Strategy:** 초기 방향성 관찰 / 과도한 추격 금지

- **Drift Score:** 7 (⚡ STRUCTURAL DRIFT)
- **VIX:** 14.899999618530272
- **SEW:** RISK_COMPRESSION / NORMAL

- **🚀 Combo Signal:** 🟢 EARLY FLOW WITHOUT SHOCK

### 🏦 Institutional Flow Engine (v2-minimal)
- **정의:** 기관성 자금이 뉴스 전에 남기는 흔적을 구조적으로 탐지

- **Raw Flow State:** **👀 EARLY TRACE**
- **Transition State:** **TRACE_BUILDING**
- **Flow Delta:** +1 (prev=3 → current=4)
- **Persistence Days:** 7
- **Transition Note:** 기관성 흐름이 전일 대비 강화
- **Confidence:** **MEDIUM**
- **Action Bias:** **MONITOR**

- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / NONE
- **Gamma:** 🟡 POSITIVE-TRANSITION / 🟢 EARLY FLOW WITHOUT SHOCK
- **SEW:** RISK_COMPRESSION / NORMAL
- **Positioning (POS_Z):** 2.21
- **Validation Score:** 3 (boost applied: +2)

- **Drivers:**
  - Drift strong
  - Gamma transition
  - Positioning overheated
  - Cross-asset risk participation
  - Credit confirms risk appetite
  - Leadership breadth expanding

### 🎯 8) Incentive Filter (Wall St. Logic)

**핵심 신호:** 장단기차(46.00bp) | 실질금리(2.43%) | DXY(99.60)
*(as of: RealRate: 2026-08-08 / FRED last available)*

❌ **자본이 탈출하는 곳 (Short Incentive):**
고금리(실질금리 2% 상회) 부담으로 인한 리스크 오프 신호

- **Note:** 실질금리와 달러는 자본의 '기회비용'을 결정하는 핵심 유인책입니다.

### 🔍 9) Cause Filter
- **질문:** 무엇이 이 움직임을 만들었는가?
- **핵심 신호:** 금리↓ + 달러↓ + 유가↓ + VIX↓
- **최종 판정:** **금리·달러 안정에 따른 전형적인 '위험자산 선호(Risk-On)'**

### 🔄 10) Direction Filter
- **질문:** 오늘 움직임은 ‘노이즈’인가 ‘의미 있는 변화’인가?
- **강도:** US10Y(Strong) / DXY(Strong) / WTI(Clear) / VIX(Clear)
- **판정:** **SIGNIFICANT MOVE (의미 있는 변화)**

### ⏳ 11) Timing Filter
- **질문:** 이 신호는 단기/중기/장기 중 어디에 더 중요하게 작용하는가?
- **가이드:**
  - 금리/달러의 ‘레벨’ 변화는 중기(수 주~수개월) 영향이 더 큼
  - VIX 급등/급락은 단기(수 일~수 주) 심리 변화에 민감
- **Today snapshot:** US10Y(4.660), DXY(99.604), VIX(14.90)

### 🏗️ 12) Structural Filter (v3)
- **질문:** 글로벌 화폐 가치와 에너지 패권 등 '판'의 변화가 있는가?
- **핵심 신호:** US10Y(↓) / DXY(↓) / GOLD(↑) / VIX(↓) / WTI(↓)
- **Meaningful Move Check:** DXY=-0.35614526898537474 / GOLD=1.886661504812393 / US10Y=-0.214137659924237 / VIX=-1.6501650580520142 / WTI=-1.2301372029861786
- **판정:** **NEUTRAL**
- **근거:** 글로벌 매크로 구조의 특이 신호가 감지되지 않음



### 12.5) Growth Sustainability Filter [SHADOW]
- **Score:** 6
- **Label:** DURABLE_GROWTH
- **Demand Proxy:** 1
- **Financing:** 0
- **Energy Burden:** 2
- **Policy Capacity:** 3
- **Strategic Interpretation:** Growth structure looks durable: demand, financing, and policy conditions are broadly supportive.
- **Input Check:** US10Y=4.659999847412109, RealYield=2.43, T10Y2Y=0.46, WTI=77.08000183105469, DXY=99.60399627685548, LiquidityDir=UP, CreditCalm=True, HY_OAS=2.71, DriftLabel=NEUTRAL, FredAsof=2026-08-07

📌 Shadow Note: This filter is observation-only and does not affect Final Exposure, Phase, or Sector Allocation.




### 12.8) Positioning Stress Filter [SHADOW]

- **Score:** 0
- **Label:** SQUEEZE_RISK
- **Strategic Interpretation:** Positioning is stretched and vulnerable to squeeze-driven reversals.

**Positioning Notes**
- Term Structure: VIX3M-VIX=3.82 → healthy contango / stable structure
- Short-Term Hedge: VIX9D/VIX=0.80 → calm front-end hedge
- Gamma Structure: Positive gamma mild
- Positioning: Crowded long positioning

📌 Shadow Note: This filter estimates whether current market behavior reflects structural participation or unstable positioning stress (squeeze / unwind / panic). No impact on Final Exposure or Phase, but used as context for Sector Allocation risk controls.



### 12.6) Flow Authenticity Filter [SHADOW]
- **Score:** 4
- **Label:** REAL_ACCUMULATION
- **Strategic Interpretation:** Institutional participation appears broad and persistent.
- **Breadth / Participation:** 1
- **Breadth Note:** RSP-SPY return spread=0.08%p → neutral breadth
- **Nasdaq Breadth Note:** QQQE-QQQ return spread=0.44%p → broad Nasdaq participation
- **Positioning / Gamma:** -1
- **Credit Confirmation:** 2
- **Macro Participation:** 2

📌 Shadow Note: This filter estimates whether upside is driven by real accumulation or short-covering. No impact on Final Exposure, Phase, or Allocation.



### 12.7) Leadership Breadth Filter [SHADOW]
- **Score:** 5
- **Label:** BROAD_LEADERSHIP
- **Strategic Interpretation:** Leadership is broadening across sectors, confirming healthier market participation.

**Leadership Notes**
- QQQ-SPY spread=0.56%p → growth leadership
- SMH-QQQ spread=0.79%p → semiconductor participation strong
- SOXX-QQQ spread=0.85%p → SOX proxy confirms chip breadth
- IWM-SPY spread=0.50%p → small-cap participation confirms broader risk-on
- XLF-SPY spread=-0.97%p → sector diffusion weak
- XLI-SPY spread=-0.38%p → sector diffusion weak
- XLY-SPY spread=0.88%p → sector diffusion positive

📌 Shadow Note: This filter checks whether leadership is broadening beyond mega-cap tech/AI. No impact on Final Exposure or Phase, but used as context for Sector Allocation risk controls.


### 🧠 13) Narrative Engine (v2 + Risk Budget + Drift)
- **정의:** 구조·심리·크레딧·유동성·국면을 통합해 오늘의 리스크 액션을 결정
- **추가 이유:** 지표는 많지만 전략가는 결국 ‘리스크를 늘릴지/줄일지/유지할지’를 판단해야 하기 때문

- **Structure Bias:** Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ +0.000 / FCI value=-0.529 (low-frequency) / DXYΔ -0.356 / US10YΔ -0.010 (정상)
- **Sentiment (Fear&Greed):** 72.99794933129868 (GREED)
- **Credit Calm:** True
- **Liquidity (NET_LIQ):** UP (MID)
- **Structural Regime:** DISINFLATION
- **Operational Phase:** GOLDILOCKS (Cap: 100)
- **Macro Tilt:** +8
- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / NONE
- **Drift Score:** 7
- **Flow Score:** 4
- **Flow Continuity:** FLOW_FADE → 👀 EARLY TRACE (FLOW_PERSISTENCE, tilt=+1)
- **Flow Regime Tilt:** +2 / Flow-Gamma Tilt: +2

- **🎯 Final Risk Action:** **HOLD (POS_OVERHEATED)**
- **Risk Budget (0~100):** **98**
- **Narrative:** 구조=EASING / 심리=GREED / 유동성=증가/중간 / 크레딧=안정 / 드리프트=⚡ STRUCTURAL DRIFT (NEUTRAL) / 수급=2.21 ⚠️ 수급 과열 감지 → Phase=GOLDILOCKS

### ⚠ 14) Divergence Monitor (Macro vs Positioning)
- **추가이유:** 시장 가격과 정책 사이의 괴리 및 수급의 '질'을 파악하여 폭발적 반전 가능성 진단
- **핵심질문:** 정책은 이런데 주가는 왜 반대로 가지?(Anomaly) 그 뒤에 숨은 수급 주체(CTA, Dealer)들은 지금 어떤 상태인가?

- **Structure(3번):** `EASING` | **Price(Regime):** `MIXED` | **Bucket:** `MIXED` | **VIX:** `14.90`
- **Positioning Data:** Z-Score: `2.21` (>1.8 시 Run) | Gamma: `0.99` (<0.5 시 Run) | CTA: `1.0` (추세 변곡점 확인)
- **Status:** **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.21). 반전 가능성 상존.
- **Action Signal:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🎯 15) Volatility-Controlled Exposure (v3.2)
- **정의:** 13번 Risk Budget 실행 브레이크 레이어
- **추가 이유:** 전략 판단(13) 이후 실제 진입 강도를 조절하기 위함

- **Base Risk Budget (13):** 98
- **VIX Level:** 14.90 (NORMAL) | **Change:** -1.65%
- **⚠️ STATUS:** RISK COMPRESSION
- **Reason:** POS_Z Extreme (2.21)
- **Action:** 신규 추격 금지 / 일부 이익실현 / 베타 노출 축소
- **Final Multiplier:** 0.89x (VIX x Positioning x Credit/Convexity)
- **Slope Intensity:** -0.0550
- **Positioning Layer:** ⚠️ Extreme Positioning Heat(2.21), Position Unwind
- **Brake Drivers:** ⚠️ Extreme Positioning Heat

- **📊 Recommended Exposure:** **87%**

### 🎨 16) Style Tilt (v1.1)
- **정의:** Macro 구조 기반 스타일 기울기 판단
- **추가 이유:** 같은 Risk-On이라도 어떤 유형의 자산이 유리한지 구분

- **Growth vs Value:** **GROWTH TILT**
- **Duration Tilt:** **LONG DURATION FAVORED**
- **Cyclical vs Defensive:** **CYCLICAL FAVORED**

### 🧩 17) Factor Layer (v1)
- **정의:** 시장을 움직이는 핵심 위험 요인 판별
- **추가 이유:** 자금이 무엇에 민감하게 반응하는지 파악

- **Duration Factor:** LONG DURATION FAVORED
- **Inflation Factor:** DISINFLATION
- **USD Factor:** USD EASING
- **Credit Factor:** CREDIT SUPPORTIVE

### 🏭 18) Sector Allocation Engine (v3.3)

**Context:** phase=GOLDILOCKS / T10Y2Y=0.46 (MODERATE STEEP) / VIX=14.90 (VOLATILITY CALM) / liquidity=UP-MID / credit=True

**Signal Priority:** VOL > LIQ > CURVE > CREDIT > PHASE > FLOW > MOM

**Macro Profile:** BALANCED
**Macro Inputs Debug:** phase=GOLDILOCKS / us10y_pct=-0.21% / dxy_pct=-0.36% / wti_pct=-1.23% / vix=14.90 / liq_easy=True / liq_tight=False / credit_calm=True / flow_score=4

**Flow Overlay:** flow_score=4 / flow_state=👀 EARLY TRACE / drift_label=NEUTRAL / gamma=🟡 POSITIVE-TRANSITION
**Flow Notes:** NEUTRAL + FLOW ACTIVE → XLK/XLI 소폭 가점 | Gamma POSITIVE → 리더 섹터 가점

**Overweight:** Technology, Industrials, Financials, Consumer Staples, Health Care

**Underweight:** Consumer Discretionary, Utilities, Real Estate

**Scoreboard:**
- Technology: +1.8  (+2 VOL, +2 LIQ, +1.5 FLOW, = +1.8)
- Industrials: +1.1  (+1.5 LIQ, +1 CURVE, +1 FLOW, = +1.1)
- Financials: +0.9  (+1 LIQ, +2 CURVE, +1 MOM, = +0.9)
- Consumer Staples: +0.2  (+0.5 PHASE, = +0.2)
- Health Care: +0.2  (+0.5 PHASE, = +0.2)
- Real Estate: -0.3  (-1 MOM, = -0.3)
- Utilities: -0.8  (-1 VOL, -1 LIQ, = -0.8)
- Consumer Discretionary: -1.2  (+1 VOL, +1.5 LIQ, -1 MOM, = -1.2)

**Rationale (Why the score exists: 섹터 점수의 핵심 드라이버)**
- OW Technology: FLOW_WEAK → 이론상 우호하나 실제 자금 유입 확인 부족
- OW Technology: +2: VOLATILITY CALM → 성장주 베팅 유효 (absolute mode: VIX 14.9)
- OW Technology: +2: 유동성 완화 → 성장주/베타 우호
- OW Industrials: FLOW_WEAK → 이론상 우호하나 실제 자금 유입 확인 부족
- OW Industrials: +1.5: 유동성 완화 → 경기민감 회복
- OW Industrials: +1: 완만한 스티프닝(0.46) → 성장 기대 반영
- OW Financials: FLOW_WEAK → 이론상 우호하나 실제 자금 유입 확인 부족
- OW Financials: +1: 유동성 완화 → 위험선호 회복
- OW Financials: +2: 완만한 스티프닝(0.46) → 예대마진 개선
- OW Consumer Staples: +0.5: Balanced Macro Profile → 방어 보완
- UW Consumer Discretionary: THEORY_TRAP → 거시/이론 우호 대비 실제 자금흐름 및 상대강도 약세
- UW Consumer Discretionary: +1: VOLATILITY CALM → 경기민감 소비 회복 (absolute mode: VIX 14.9)
- UW Consumer Discretionary: +1.5: 유동성 완화 → 소비 민감주 우호

**Regime Controller:**
- DISLOCATION (avg_divergence=-1.00, dispersion=1.57)
- Correlation Break: True / Type=FINANCIALS_WEAK_ON_RATE_RELIEF
- Break Reasons: US10Y ↓ but XLF ↓
- Interpretation: 섹터별 괴리가 큰 불안정 장세 → 포지션 축소와 리더 섹터 검증 필요

**Divergence / Classification Monitor (Theory vs Flow alignment: 이론과 실제 자금흐름 정렬 여부)**
- Technology: FLOW_WEAK (theory=+4.0, flow=+0.5, final=+1.8)
- Industrials: FLOW_WEAK (theory=+2.5, flow=+0.3, final=+1.1)
- Financials: FLOW_WEAK (theory=+3.0, flow=+0.7, final=+0.9)
- Communication Services: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Energy: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Materials: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Consumer Discretionary: THEORY_TRAP (theory=+2.5, flow=-0.7, final=-1.2)

### 💰 18.5) Tactical Asset Allocation (Execution Weight)
- **Strategic Exposure (15):** **87.0%** → **Regime Adjusted:** **87.0%**
- **Exposure Override:** DISLOCATION → Sector Weight Only (No Exposure Change)

| Sector | Score | Divergence | **Weight in Portfolio** | **Action** |
| :--- | :---: | :---: | :---: | :--- |
| Technology | +1.8 | NEGATIVE_DIVERGENCE | **15.0%** | SMALL ADJUST |
| Industrials | +1.1 | NEGATIVE_DIVERGENCE | **10.0%** | REBALANCE |
| Financials | +0.9 | NEGATIVE_DIVERGENCE | **10.0%** | REBALANCE |
| Consumer Staples | +0.2 | ALIGNED | **6.7%** | SMALL ADJUST |
| Health Care | +0.2 | ALIGNED | **6.7%** | SMALL ADJUST |
| **Cash & Hedge** | - | - | **51.6%** | DEFENSIVE |

- **Allocation Check:** Sector Weights + Cash = **100.0%**
- **Regime Cap Profile:** BALANCED
- **Participation / Quality Cap Applied:**
  - Technology: 35.2% → 15.0% (-20.2%)
  - Industrials: 20.5% → 10.0% (-10.5%)
  - Financials: 18.0% → 10.0% (-8.0%)
- **Strategic Cash (15):** 13.0%
- **Tactical Reserve (Cap / Unallocated):** 38.6%


**Deleveraging Priority Preview:**
- 기준: Divergence → Momentum → Score → Current Weight
1. Technology (priority_score=3.88, score=1.84, weight=15.0%, div=NEGATIVE_DIVERGENCE, mom=0)
2. Industrials (priority_score=3.51, score=1.07, weight=10.0%, div=NEGATIVE_DIVERGENCE, mom=0)
3. Financials (priority_score=2.58, score=0.94, weight=10.0%, div=NEGATIVE_DIVERGENCE, mom=1)
4. Consumer Staples (priority_score=-0.1, score=0.21, weight=6.7%, div=ALIGNED, mom=0)
5. Health Care (priority_score=-0.1, score=0.21, weight=6.7%, div=ALIGNED, mom=0)

**Leveraging Priority Preview:**
- 기준: Score → Momentum → Positive Divergence
1. Consumer Staples (priority_score=0.21, score=0.21, weight=6.7%, div=ALIGNED, mom=0)
2. Health Care (priority_score=0.21, score=0.21, weight=6.7%, div=ALIGNED, mom=0)
3. Financials (priority_score=-0.56, score=0.94, weight=10.0%, div=NEGATIVE_DIVERGENCE, mom=1)
4. Technology (priority_score=-1.16, score=1.84, weight=15.0%, div=NEGATIVE_DIVERGENCE, mom=0)
5. Industrials (priority_score=-1.93, score=1.07, weight=10.0%, div=NEGATIVE_DIVERGENCE, mom=0)
- **Divergence Adjustment:** Technology, Industrials, Financials penalized in weight sizing

### 🧬 19) Execution Layer (ETF Mapping)

| Sector | ETF | Weight | Action | Divergence | Classification |
| :--- | :---: | :---: | :--- | :--- | :--- |
| Technology | XLK | 15.0% | WATCHLIST_SMALL | NEGATIVE_DIVERGENCE | FLOW_WEAK |
| Industrials | XLI | 10.0% | WATCHLIST_SMALL | NEGATIVE_DIVERGENCE | FLOW_WEAK |
| Financials | XLF | 10.0% | WATCHLIST_SMALL | NEGATIVE_DIVERGENCE | FLOW_WEAK |
| Consumer Staples | XLP | 6.7% | SMALL | ALIGNED | ALIGNED |
| Health Care | XLV | 6.7% | SMALL | ALIGNED | ALIGNED |


### 🧬 19.5) Execution / Style Translation Layer
- **Implementation Focus:** Environment-Aware Stock Types

**Execution Notes:**
- Early flow trace → maintain leaders, wait for confirmation before broadening.
- Positioning heat elevated → prefer rebalancing over fresh chasing.

**Preferred Company Traits:**
- Leaders with improving breadth confirmation

**Risk Control / Avoid:**
- Broad beta expansion before confirmation
- Crowded late-entry trades

---


---

## 🌐 Country ETF Risk Monitor

### BND
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.8438374215486466
- **Z-Score (5d):** 1.2247493580978361

### EEM
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.4357188057829331
- **Z-Score (5d):** 0.6361806790680044

### EIS
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.396283354756151
- **Z-Score (5d):** 1.3073267889626607

### EMB
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.8186795332795396
- **Z-Score (5d):** 1.325644235183956

### EWJ
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.1742893531930707
- **Z-Score (5d):** 1.727230471481508

### FXI
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.4908168697718058
- **Z-Score (5d):** -0.22930234668983357

### GLD
- **Crash?** True
- **Risk Level:** EXTREME
- **Z-Score (1d):** 1.4939701162988217
- **Z-Score (5d):** 2.7560738924759653

### SPY
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.5896196510164038
- **Z-Score (5d):** 1.6620562623525

### VXX
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.30258524629313854
- **Z-Score (5d):** -0.3900968164792284
