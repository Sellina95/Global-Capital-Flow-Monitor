...
- **미국 10년물 금리**: 4.017  (+0.00% vs 4.017)
- **달러 인덱스**: 97.747  (-0.05% vs 97.799)
- **WTI 유가**: 66.910  (+2.20% vs 65.470)
- **변동성 지수 (VIX)**: 20.380  (+9.39% vs 18.630)
- **원/달러 환율**: 1441.480  (+0.57% vs 1433.300)
...
- **Context:** phase=RISK-OFF (부분 정렬) / liquidity=UP-MID / credit_calm=True
...
- **VIX 레벨:** 20.38 → **High (Risk-off bias)**
...
- **현실(FCI):** level=EASY (완화) / dir(→) | as of: 2026-02-25 (FRED last available)
- **유인(Real Rates):** level=NEUTRAL (중립) / dir(↑) | as of: 2026-02-25 (FRED last available)
...
- **Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ -0.010 / FCIΔ +0.000 / DXYΔ -0.052 / US10YΔ +0.000**
...
- **Spread as of:** 2026-02-25 (FRED latest)
...
- **방향(전일 대비):** HYG(→) / LQD(→)
- **HYG:** today 80.850 / prev 80.850 / pct 0.00%
- **LQD:** today 111.720 / prev 111.720 / pct 0.00%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Noise, +0.00%)** → 완화 기대 약화/금리 부담
- DXY **(Mild, -0.05%)** → 달러 약세/리스크 선호
- WTI **(Strong, +2.20%)** → 인플레 재자극 가능성
- VIX **(Strong, +9.39%)** → 심리 악화/리스크오프
- 원/달러(USDKRW) **(Strong, +0.57%)** → 원화 약세/수급 부담
...
- **Note:** sector ETFs missing: XLRE (add to fetch_macro_data.py)
...
- **Today snapshot:** US10Y(4.017), DXY(97.747), VIX(20.38)
...
- **Structure Bias:** Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ -0.010 / FCIΔ +0.000 / DXYΔ -0.052 / US10YΔ +0.000
...
- **Narrative:** 구조=EASING / 심리=NEUTRAL / 유동성=증가/중간 / 크레딧=안정 → Phase=RISK-OFF (부분 정렬)
