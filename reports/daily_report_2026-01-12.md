...
- **미국 10년물 금리**: 4.171  (-0.38% vs 4.187)
- **달러 인덱스**: 98.944  (+0.21% vs 98.738)
- **WTI 유가**: 59.200  (+3.77% vs 57.050)
- **변동성 지수 (VIX)**: 14.490  (-0.14% vs 14.510)
- **원/달러 환율**: 1467.980  (+1.47% vs 1446.670)

---

## 🚨 Regime Change Monitor (always-on)
- **Status:** ❎ NOT DETECTED
- **Current Regime:** RISK-ON (부분 정렬)
- **File:** not created
- **Email:** not sent
...
- **VIX 레벨:** 14.49 → **Mid (Neutral/Mixed)**
...
- **핵심 신호:** US10Y(↓, Strong) / DXY(↑, Clear) / VIX(↓, Noise)
...
- 미국 금리(US10Y) **(Strong, -0.38%)** → 완화 기대 강화
- DXY **(Clear, +0.21%)** → 달러 강세/신흥국 부담
- WTI **(Strong, +3.77%)** → 인플레 재자극 가능성
- VIX **(Noise, -0.14%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Strong, +1.47%)** → 원화 약세/수급 부담

### 🧩 5) Cross-Asset Filter (연쇄효과 분석)
- **추가 이유:** 한 지표의 변화가 다른 자산군에 어떻게 전파되는지, 즉 연쇄효과를 파악하기 위함

- **금리 하락(US10Y↓)** → **달러 약세(DXY↓)** 및 **유가 상승(WTI↑)** 경향
- **변동성 하락(VIX↓)** → **리스크 선호, 달러 약세(DXY↓)** 및 **유가 상승(WTI↑)**
- **유가 상승(WTI↑)** → **리스크 선호, 금리 인상(US10Y↑)**

### 🧩 6) Risk Exposure Filter (숨은 리스크 분석)
- **추가 이유:** 숫자는 괜찮아 보여도 그 뒤에 숨은 리스크를 식별하기 위함

- **VIX 하락(VIX↓)** → **리스크 감소**: 불안정성이 줄어들고 상대적 안정성 증가
- **금리 하락(US10Y↓)** → **리스크 증가**: 금리 하락은 경기 둔화 및 저금리 상황 지속
...
- **판정:** **금리 하락(US10Y 하락) 달러 강세(DXY 상승) 유가 상승(WTI 상승) 변동성 감소(VIX 하락) **
- **이유:** 직접적인 원인 파악

### 🔄 9) Direction Filter
- **질문:** 시장이 어느 방향으로, 얼마나 움직였는가?
- **핵심 신호:** US10Y(Strong, ↓) / DXY(Clear, ↑) / WTI(Strong, ↑) / VIX(Noise, ↓)
- **판정:** **SIGNIFICANT MOVEMENT (의미 있는 움직임)**
- **근거:** 금리나 달러의 변동이 크고 뚜렷함

### ⏳ 10) Timing Filter
- **질문:** 시장 변화가 단기, 중기, 장기 관점에서 어떤 영향을 미치는지?
- **핵심 신호:** US10Y(-0.38% short-term, 4.19% medium-term, 4.17% long-term) / DXY(0.21% short-term, 98.74% medium-term, 98.94% long-term) / VIX(-0.14% short-term, 14.51% medium-term, 14.49% long-term)
- **판정:** **NO SIGNIFICANT MOVEMENT**
- **근거:** 단기, 중기, 장기적으로 시장 변화가 균일하게 발생하고 있음