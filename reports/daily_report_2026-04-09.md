...
- **달러 인덱스**: 99.052  (+0.00% vs 99.052)
- **WTI 유가**: 97.130  (+0.02% vs 97.110)
- **변동성 지수 (VIX)**: 25.780  (+0.00% vs 25.780)
- **원/달러 환율**: 1479.800  (-1.29% vs 1499.130)
...
- 원/달러(USDKRW) **(Strong, -1.29%)** → 원화 강세/수급 개선
...
- **Top Similarity Matches:**
  - Israel_2023: -0.248
  - Red_Sea: -0.343
  - Iran_Crisis_2020: -0.379
- **Top Drivers:**
  - USDCNH: z_used=-1.47 (z1d=-1.18, z5d=-1.90, raw_w=0.18, norm_w=0.14) → contrib=-0.20
  ...
  - EEM: z_used=-0.65 (z1d=-0.05, z5d=+1.69, raw_w=0.10, norm_w=0.08) → contrib=-0.05
...
- **Today snapshot:** US10Y(4.291), DXY(99.052), VIX(25.78)
...
- **Sentiment (Fear&Greed):** 39.13359640503889 (NEUTRAL)
- **Credit Calm:** True
...
- **Narrative:** 구조=MIXED / 심리=NEUTRAL / 유동성=증가/중간 / 크레딧=안정 / 수급=0.71 → Phase=WAITING / RANGE (대기·박스권)
...
- **Structure(3번):** `MIXED` | **Price(Regime):** `MIXED` | **VIX:** `25.78`
- **Positioning Data:** Z-Score: `0.71`(2.0을 뚫으면 Run 액션준비) | Gamma: `1.00`(딜러들이 받쳐줄지, 던질지 확인 0.5 미만이면 Run 액션준비) | CTA: `1.0`(기계들이 밀어주고 있는지 추세확인 0이하 뚫으면 Run 액션준비)
...
**Context:** phase=WAITING / RANGE (대기·박스권) / T10Y2Y=0.50 (MODERATE STEEP) / VIX=25.78 (VOLATILITY ELEVATED) / liquidity=UP-MID / credit=True
