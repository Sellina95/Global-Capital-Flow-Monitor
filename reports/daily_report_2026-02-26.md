...
- **미국 10년물 금리**: 4.048  (-0.10% vs 4.052)
- **달러 인덱스**: 97.771  (-0.08% vs 97.851)
- **WTI 유가**: 65.400  (-0.11% vs 65.470)
- **변동성 지수 (VIX)**: 18.740  (+0.32% vs 18.680)
- **원/달러 환율**: 1426.610  (-0.03% vs 1427.000)
...
- **VIX 레벨:** 18.74 → **Mid (Neutral/Mixed)**
...
- **Policy Bias: EASING (완화) (STRONG, score=-2.5) | REAL_RATEΔ -0.030 / FCIΔ +0.000 / DXYΔ -0.080 / US10YΔ -0.004**
- **Expectations: dict received.**

- **판정:** **POLICY MIXED (정책 신호 혼조)**
- **근거:** 금리/달러/변동성 신호가 완전히 정렬되지 않음
- **한줄요약 ~~** 구조=EASING (완화)(STRONG)는 참고, 가격=POLICY MIXED (정책 신호 혼조) 중심 → 최종 POLICY MIXED (정책 신호 혼조)
...
- **방향(전일 대비):** HYG(↓) / LQD(↑)
- **HYG:** today 80.865 / prev 80.875 / pct -0.01%
- **LQD:** today 111.670 / prev 111.665 / pct 0.00%
...
- WTI **(Noise, -0.11%)** → 물가 부담 완화
- VIX **(Noise, +0.32%)** → 심리 악화/리스크오프
...
- **강도:** US10Y(Clear) / DXY(Mild) / WTI(Noise) / VIX(Noise)
...
- **Today snapshot:** US10Y(4.048), DXY(97.771), VIX(18.74)
...
- **Structure Bias:** Policy Bias: EASING (완화) (STRONG, score=-2.5) | REAL_RATEΔ -0.030 / FCIΔ +0.000 / DXYΔ -0.080 / US10YΔ -0.004
...
- **Narrative:** 구조=EASING / 심리=NEUTRAL / 유동성=감소/낮음 / 크레딧=안정 → Phase=TRANSITION / MIXED (전환·혼조)
