# 🌍 Global Capital Flow – Daily Brief
**Date:** 2026-05-27
**Data as of:** 2026-05-26

## ⚡ Strategic War Room (통합 대응)
> **시스템 상태: 🚨 ALERT**
> **판단 요약: 포지셔닝 과열 감지 / Hard Deadman은 아니나 추격보다 리스크 축소 우선**
### 🎯 Exposure Framework
- **Base Exposure (전략 기준): 64%**
- **Final Exposure (실행 기준): 64%**

- **Portfolio Stance:** REDUCE (POS_OVERHEATED) / 64%

- **[14번 구조·수급 괴리]:** 🚨 **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.34). 반전 가능성 상존.
### 🟢 Current SEW Status
- **SEW:** RISK_COMPRESSION | ⚠️ Risk Compression 발동 (권장 익스포저 55%)
- **Event Type:** NORMAL → 정상 상태 / 구조적 리스크 없음
- **Spike Monitor:** Spike 0 / Extreme 0
- **Current Reason:** ⚠️ CROWDING RISK: POS_Z Extreme (2.38) → Risk Compression (55%)

- **[15번 Hard Deadman]:** ✅ PASS
- **[14번 수급 시그널]:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🧠 Strategic Interpretation (PM Summary)
- 금일 시장은 **EARLY RISK-ON 환경**입니다.
- **Structure:** 변동성 확대가 동반되며 금융환경의 tightening pressure가 강화되는 구간입니다.
- **Liquidity:** 가격 환경은 긴축적으로 움직였지만, Net Liquidity는 증가 방향을 보이며 단기 달러 체력은 일부 유지되고 있습니다.
- **Participation:** 기관성 흐름은 **👀 EARLY TRACE**(score=4) 상태이며, Drift는 **⚡ STRUCTURAL DRIFT / NEUTRAL**(score=5)로 관찰됩니다. 다만 breadth와 리더십 확산 여부는 추가 확인이 필요합니다.
- **Risk:** 포지셔닝 과열(POS_Z=2.34)이 관찰되며, 크레딧은 안정적입니다. 따라서 신규 추격보다는 sizing control과 exposure discipline이 중요합니다.
- **Allocation:** 현 수준에서는 포지션 유지와 관망이 적절하며, 추가 확대는 breadth와 flow confirmation 이후가 더 안전합니다. 현재 실행 기준 노출은 약 **64%**입니다.

- **Structural Layer:**
  - Growth Sustainability → **MODERATE** (liquidity improving, credit stress contained, USD funding pressure moderate)
  - Short Covering Risk → **LOW** (broad demand participation)
  - Breadth Quality → **NARROW** (leadership concentrated)
  - Financing Condition → **SUPPORTIVE** (long-end yields stable, real financing pressure manageable, credit financing conditions stable)

### 🎯 Final Action Engine(Raw Signal)
- **Action:** REDUCE
- **Size:** TAKE PROFIT
- **Confidence:** MEDIUM
- **Reason:**
  - No actionable alignment
  - Positioning overheat

## 🎯 Final Decision (War Room Override)
- **Final Action:** **REDUCE (POS_OVERHEATED)**
- **Final Exposure:** **64%**
- **Base Context:** phase=EARLY RISK-ON / narrative=REDUCE (POS_OVERHEATED) / base_exposure=64%
- **SEW:** RISK_COMPRESSION / NORMAL
- **Divergence:** N/A / **MONITOR REVERSAL (RUN 액션 준비)**
- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / NONE / score=5
- **Flow:** 👀 EARLY TRACE / score=4
- **Gamma:** 🟡 POSITIVE-TRANSITION
- **Tactical Action:** REDUCE / TAKE PROFIT / MEDIUM
- **Positioning:** pos_z=2.34
- **Warning Score:** 2 (6.5 상관관계 붕괴)
- **Tactical Why:** No actionable alignment / Positioning overheat
- **Why:** SEW STABLE → 실시간 이상징후 없음 → Divergence 비정렬 → 방어적 태도 유지 → 상위 레이어(SEW/Divergence)가 Narrative보다 우선 → Warning Score 2 → 섹터/상관관계 경고, 총노출 추가 감산 없이 배분 보수화 → Tactical=REDUCE / Flow=👀 EARLY TRACE(4) / Drift=⚡ STRUCTURAL DRIFT(5) / Gamma=🟡 POSITIVE-TRANSITION → Tactical REDUCE → 방어 기조 유지 / 총노출 추가 감산 없이 배분 보수화

### 🚩 Market Regime Status
- **Operational Phase:** ✅ **EARLY RISK-ON**
- **Structural Regime:** **DISINFLATION**

---

## 📊 Daily Macro Signals

- **미국 10년물 금리**: 4.493  (-1.43% vs 4.558)
- **달러 인덱스**: 99.150  (-0.17% vs 99.320)
- **WTI 유가**: 93.890  (-2.81% vs 96.600)
- **변동성 지수 (VIX)**: 17.010 (+1.86% vs 16.700)
- **원/달러 환율**: 1514.790  (+0.69% vs 1504.400)

---

---

## 🧭 Strategist Commentary (Seyeon’s Filters)

### 🧩 1) Market Regime Filter
- **정의:** 지금 어떤 장(場)인지 판단하는 *시장 국면 필터*
- **추가 이유:** 같은 지표도 ‘국면’에 따라 의미가 완전히 달라지기 때문

- **VIX 레벨:** 17.01 → **Mid (Neutral/Mixed)**
- **핵심 조합(전일 대비 방향):** US10Y(↓) / DXY(↓) / VIX(↑)
- **판정:** **EARLY RISK-ON | Flow:  (Flow Weak)**
- **근거:** Macro Narrative=DISINFLATION / Policy=MIXED / Credit=COOL

### 💧 2) Liquidity Filter (Enhanced)
- **질문:** 시장에 새 돈이 들어오는가, 말라가는가?
- **추가 이유:** US10Y/DXY/VIX는 ‘시장의 기대’를 보여주고, FCI는 ‘현실의 압박’을, Real Rates는 ‘위험을 감수할 유인’을 보여준다.

- **기대(가격) 신호:** US10Y(↓) / DXY(↓) / VIX(↑)
- **현실(FCI):** value=-0.523 / level=EASY (완화) / update=low-frequency | as of: 2026-05-27 (latest available)
- **유인(Real Rates):** value=2.160 / level=RESTRICTIVE (유인↓) / dir(→) | as of: 2026-05-27 (latest available)
- **판정:** **LIQUIDITY MIXED / FRAGILE (혼조·취약)**
- **근거:** 기대(가격)와 현실(FCI)/유인(실질금리) 정렬이 불완전
- **Note:** FCI는 저빈도 금융환경 프록시로 level 중심 해석, Real Rates는 영업일 기준 변화 방향을 함께 반영함

### 🏛️ 3) Policy Filter (with Expectations)
- **질문:** 중앙은행·정책 환경은 완화인가, 긴축인가?

- **가격(현재) 신호:** US10Y(↓) / DXY(↓) / VIX(↑)
- **Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ +0.000 / FCI value=-0.523 (low-frequency) / DXYΔ -0.170 / US10YΔ -0.065**
- **Expectations: dict received.**

- **판정:** **POLICY MIXED (정책 신호 혼조)**
- **근거:** 금리/달러/변동성 신호가 완전히 정렬되지 않음
- **한줄요약 ~~** 구조=EASING (완화)(MODERATE)는 참고, 가격=POLICY MIXED (정책 신호 혼조) 중심 → 최종 POLICY MIXED (정책 신호 혼조)

### 🧰 4) Fed Plumbing Filter (TGA/RRP/Net Liquidity)
- **질문:** 시장의 ‘달러 체력’은 늘고 있나, 줄고 있나?
- **추가 이유:** 금리·달러가 안정적이어도 유동성이 빠지면 리스크 자산은 쉽게 흔들릴 수 있음
- **Liquidity as of:** 2026-05-20 (FRED latest)
- **NET_LIQ level:** 5932325.1
- **TGA level:** 781293.0
- **RRP level:** 24.867
- **방향(전일 대비):** TGA(↓) / RRP(↑) / NET_LIQ(↑)
- **판정:** **LIQUIDITY SUPPORTIVE (완만한 유동성 우호)**
- **근거:** Net Liquidity↑ → 시장 내 달러 여력 개선
- **Note:** TGA/RRP/WALCL은 매일 갱신되지 않을 수 있어, 리포트에는 ‘최근 available 값’을 반영함

### 🌡️ 4.2) High Yield Spread Filter (HY OAS)
- **질문:** 시장 공포의 ‘온도’는 올라가고 있나, 내려가고 있나?
- **추가 이유:** HYG/LQD가 ‘방향’이라면, HY Spread는 ‘강도(얼마나 무서워하는지)’를 보여줌
- **Spread as of:** 2026-05-25 (FRED latest)
- **HY_OAS level:** 2.74% → **COOL (낮은 공포)**
- **방향(전일 대비):** HY_OAS(→) / +0.00%
- **판정:** **CREDIT CALM**
- **근거:** HY 스프레드 낮음 → 크레딧 스트레스 제한 / 방향성 제한 → 레벨 중심 해석
- **Note:** HY OAS는 매일 갱신되지 않을 수 있어, ‘최근 available 값’을 반영함

### 🧾 4.5) Credit Stress Filter (HYG vs LQD)
- **질문:** 크레딧 시장이 먼저 ‘리스크오프’를 말하고 있는가?
- **추가 이유:** HYG가 LQD보다 약해지면, 시장이 ‘위험을 감수할 이유가 없다’고 판단하기 시작했을 가능성
- **방향(전일 대비):** HYG(↑) / LQD(↑)
- **HYG:** today 80.180 / prev 79.910 / pct 0.34%
- **LQD:** today 108.780 / prev 108.370 / pct 0.38%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, -1.43%)** → 완화 기대 강화
- DXY **(Clear, -0.17%)** → 달러 약세/리스크 선호
- WTI **(Strong, -2.81%)** → 물가 부담 완화
- VIX **(Clear, +1.86%)** → 심리 악화/리스크오프
- 원/달러(USDKRW) **(Strong, +0.69%)** → 원화 약세/수급 부담
- HYG (High Yield ETF) **(Mild, +0.34%)** → 크레딧 위험선호↑
- LQD (IG Bond ETF) **(Mild, +0.38%)** → 우량채 강세(리스크오프 성향)

### 🧩 6) Cross-Asset Filter (자산군 연쇄 반응 분석)
- **추가 이유:** 단일 지표의 노이즈를 제거하고, 매크로 충격이 자산군 전반으로 확산되는 **전이 경로(Transmission Path)**를 파악하기 위함

- **금리 하락(US10Y↓)** → 할인율 압박 완화 → 달러 약세(DXY↓) 유도: **위험자산(Growth/EM) 선호 심리 강화 및 유동성 환경 개선**
- **변동성 상승(VIX↑)** → 위험회피(Risk-Off) 강화: **안전 자산(Cash/USD) 선호도 급증 및 하이일드 스프레드 확대 압력**
- **유가 하락(WTI↓)** → 물가 부담 완화: **실질 구매력 회복 및 긴축 압력 완화(Dovish Tilt) 가능성 시사**

> **[Strategic Note]:** 위 연쇄 반응이 역사적 상관관계에서 벗어날 경우, **6.5) Correlation Break Monitor**를 통해 국면 전환 여부를 정밀 판별함

### 🌊 Drift Monitor (v4)
- **정의:** 누적 흐름 + ATR 기반 강도 감지

- **SPY:** 🟢 UP | Short-term: SHORT UP | 1D=+0.65% / 5D=+1.60% | Strength: LOW
- **WTI:** 🔴 DOWN | Short-term: SHORT UP | 1D=-3.45% / 5D=-15.89% | Strength: HIGH
- **DXY:** 🔴 DOWN | Short-term: SHORT UP | 1D=-0.01% / 5D=-0.16% | Strength: LOW
- **GOLD:** 🔴 DOWN | Short-term: SHORT DOWN | 1D=-0.77% / 5D=-0.90% | Strength: LOW

- **Drift Score:** 5
- **State:** **⚡ STRUCTURAL DRIFT**
- **Label:** NEUTRAL
- **SEW Combo Signal:** NONE

- **Market Drift Summary:**
  - Equity (SPY): 🟢 UP / SHORT UP
  - Oil (WTI): 🔴 DOWN / SHORT UP
  - Dollar (DXY): 🔴 DOWN / SHORT UP
  - Gold (GOLD): 🔴 DOWN / SHORT DOWN

- **Drivers:**
  - SPY positive drift
  - Sector breadth expanding
  - Broad cyclical leadership
  - Dollar not restrictive

### ⚠ 6.5) Correlation Break Monitor
Correlation Break Detected:
- VIX ↑ but SPY ↑
- VIX ↑ but Technology ↑

So What?
- 결론: **공식이 깨진 구간** → 총노출 추가 감산보다 **추격 자제 + 퀄리티/리더 중심 배분**

### ⚠ 6.6) Sector Correlation Break Monitor
No significant sector-level correlation break detected.

### 🧩 7) Risk Exposure Filter (숨은 리스크 분석)
- **추가 이유:** 숫자는 괜찮아 보여도 그 뒤에 숨은 리스크를 식별하기 위함

- **VIX 상승(VIX↑)** → 변동성 확대: 포지션 축소/헤지 수요 증가 가능
- **금리 하락(US10Y↓)** → 완화 기대/할인율 부담 완화 가능
- **달러 약세(DXY↓)** → 위험자산 선호/신흥국 부담 완화 가능
- **유가 하락(WTI↓)** → 물가 부담 완화 가능

### 🛰️ 7.2) Geopolitical Early Warning Monitor (FX/Commodities Composite)
- **Geo Stress Score (z-composite):** **-0.61**  *(Level: NORMAL)*
- **Coverage:** 100% *(used weight: 1.30 / defined weight: 1.30)*
- **3D Avg Score:** -0.48
- **Geo Momentum:** -0.13 *(Status: FLAT)*

**Historical Pattern Match (Cosine Similarity):**
- **Closest Historical Match:** Red_Sea
- **Cosine Similarity Score:** -0.269
- **Similarity Signal:** Weak Historical Match
- **Top Similarity Matches:**
  - Red_Sea: -0.269
  - Iran_Crisis_2020: -0.309
  - Taiwan_Tension: -0.333
- **Top Drivers:**
  - EEM: z_used=-1.59 (z1d=+1.84, z5d=+1.23, raw_w=0.10, norm_w=0.08) → contrib=-0.12
  - KR10Y_SPREAD: z_used=-1.66 (mode=level, raw_w=0.08, norm_w=0.06) → contrib=-0.10
  - EMB: z_used=-1.10 (z1d=+0.99, z5d=+1.26, raw_w=0.12, norm_w=0.09) → contrib=-0.10
  - WTI: z_used=-1.02 (z1d=-0.65, z5d=-1.57, raw_w=0.10, norm_w=0.08) → contrib=-0.08
- **Missing/Skipped:** None
- **Sovereign Spread factors included:** KR10Y_SPREAD, JP10Y_SPREAD, DE10Y_SPREAD, IL10Y_SPREAD

**Trade Information:**
- 지정학 스트레스 프록시가 평온. 기존 매크로 레짐/리스크 예산 신호를 우선.
- 역사적 위기 패턴 유사도는 낮습니다. 현재는 **Red_Sea** 유형과 가장 가깝지만, 전면적 지정학 쇼크보다는 제한적·국지적 리스크 모니터링 구간으로 해석됩니다.
- **Country ETF Crash?** No (BND, EEM, EIS, EMB, EWJ, FXI, GLD, SPY, VXX)

### ⚡ 7.3) Pseudo Gamma Filter
- **정의:** 옵션 데이터 없이 시장의 감마 환경을 추론
- **주의:** Dealer Gamma Bias 숫자와 Pseudo Gamma State는 서로 다른 레이어

- **Pseudo Gamma State:** 🟡 POSITIVE-TRANSITION
- **Dealer Gamma Bias:** 2.22 (STABILIZING / dealer gamma proxy supportive)
- **Bias:** VIX는 안정적이나 Drift가 형성 중
- **Strategy:** 초기 방향성 관찰 / 과도한 추격 금지

- **Drift Score:** 5 (⚡ STRUCTURAL DRIFT)
- **VIX:** 17.010000228881836
- **SEW:** RISK_COMPRESSION / NORMAL

- **🚀 Combo Signal:** 🟢 EARLY FLOW WITHOUT SHOCK

### 🏦 Institutional Flow Engine (v2-minimal)
- **정의:** 기관성 자금이 뉴스 전에 남기는 흔적을 구조적으로 탐지

- **Raw Flow State:** **👀 EARLY TRACE**
- **Transition State:** **👀 EARLY TRACE**
- **Flow Delta:** +0 (prev=4 → current=4)
- **Persistence Days:** 2
- **Transition Note:** 기관성 흐름 상태 유지
- **Confidence:** **MEDIUM**
- **Action Bias:** **MONITOR**

- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / NONE
- **Gamma:** 🟡 POSITIVE-TRANSITION / 🟢 EARLY FLOW WITHOUT SHOCK
- **SEW:** RISK_COMPRESSION / NORMAL
- **Positioning (POS_Z):** 2.34
- **Validation Score:** 3 (boost applied: +2)

- **Drivers:**
  - Drift strong
  - Gamma transition
  - Positioning overheated
  - Cross-asset risk participation
  - Leadership breadth expanding
  - Cyclical leadership over defensives

### 🎯 8) Incentive Filter (Wall St. Logic)

**핵심 신호:** 장단기차(49.00bp) | 실질금리(2.16%) | DXY(99.15)
*(as of: RealRate: 2026-05-27 / FRED last available)*

❌ **자본이 탈출하는 곳 (Short Incentive):**
고금리(실질금리 2% 상회) 부담으로 인한 리스크 오프 신호

- **Note:** 실질금리와 달러는 자본의 '기회비용'을 결정하는 핵심 유인책입니다.

### 🔍 9) Cause Filter
- **질문:** 무엇이 이 움직임을 만들었는가?
- **핵심 신호:** 금리↓ + 달러↓ + 유가↓ + VIX↑
- **최종 판정:** **인플레이션 압력 둔화 및 비용 감소형 안도 랠리**

### 🔄 10) Direction Filter
- **질문:** 오늘 움직임은 ‘노이즈’인가 ‘의미 있는 변화’인가?
- **강도:** US10Y(Strong) / DXY(Clear) / WTI(Strong) / VIX(Clear)
- **판정:** **SIGNIFICANT MOVE (의미 있는 변화)**

### ⏳ 11) Timing Filter
- **질문:** 이 신호는 단기/중기/장기 중 어디에 더 중요하게 작용하는가?
- **가이드:**
  - 금리/달러의 ‘레벨’ 변화는 중기(수 주~수개월) 영향이 더 큼
  - VIX 급등/급락은 단기(수 일~수 주) 심리 변화에 민감
- **Today snapshot:** US10Y(4.493), DXY(99.150), VIX(17.01)

### 🏗️ 12) Structural Filter (v3)
- **질문:** 글로벌 화폐 가치와 에너지 패권 등 '판'의 변화가 있는가?
- **핵심 신호:** US10Y(↓) / DXY(↓) / GOLD(↓) / VIX(↑) / WTI(↓)
- **Meaningful Move Check:** DXY=-0.1711620715542264 / GOLD=-0.45565356461512946 / US10Y=-1.4260652911201688 / VIX=1.8562841423955614 / WTI=-2.805382119336843
- **판정:** **WEAK DEMAND + RISK-OFF (수요 둔화)**
- **근거:** 유가 하락과 변동성 상승이 동시에 의미 있게 나타나 성장 둔화 우려를 반영



### 12.5) Growth Sustainability Filter [SHADOW]
- **Score:** 5
- **Label:** DURABLE_GROWTH
- **Demand Proxy:** 1
- **Financing:** 2
- **Energy Burden:** -1
- **Policy Capacity:** 3
- **Strategic Interpretation:** Growth structure looks durable: demand, financing, and policy conditions are broadly supportive.
- **Input Check:** US10Y=4.493000030517578, RealYield=2.16, T10Y2Y=0.49, WTI=93.88999938964844, DXY=99.1500015258789, LiquidityDir=UP, CreditCalm=True, HY_OAS=2.74, DriftLabel=NEUTRAL, FredAsof=2026-05-26

📌 Shadow Note: This filter is observation-only and does not affect Final Exposure, Phase, or Sector Allocation.



### 12.8) Positioning Stress Filter [SHADOW]

- **Score:** -1
- **Label:** SQUEEZE_RISK

**Positioning Notes**
- Term Structure: VIX3M-VIX=2.88 → healthy contango / stable structure
- Short-Term Hedge: VIX9D/VIX=0.87 → calm front-end hedge
- Gamma Structure: Positive gamma extreme → dealer-supported squeeze risk
- Positioning: Crowded long positioning

📌 Shadow Note: This filter estimates whether current market behavior reflects structural participation or unstable positioning stress (squeeze / unwind / panic). No impact on Final Exposure, Phase, or Allocation.



### 12.6) Flow Authenticity Filter [SHADOW]
- **Score:** 2
- **Label:** EARLY_ROTATION
- **Breadth / Participation:** 0
- **Breadth Note:** RSP-SPY return spread=-0.15%p → neutral breadth
- **Nasdaq Breadth Note:** QQQE-QQQ return spread=-0.27%p → neutral Nasdaq breadth
- **Positioning / Gamma:** -2
- **Credit Confirmation:** 2
- **Macro Participation:** 2

📌 Shadow Note: This filter estimates whether upside is driven by real accumulation or short-covering. No impact on Final Exposure, Phase, or Allocation.



### 12.7) Leadership Breadth Filter [SHADOW]
- **Score:** 5
- **Label:** BROAD_LEADERSHIP

**Leadership Notes**
- QQQ-SPY spread=1.11%p → growth leadership
- SMH-QQQ spread=2.70%p → semiconductor participation strong
- SOXX-QQQ spread=4.32%p → SOX proxy confirms chip breadth
- IWM-SPY spread=1.23%p → small-cap participation confirms broader risk-on
- XLF-SPY spread=-0.84%p → sector diffusion weak
- XLI-SPY spread=0.81%p → sector diffusion positive
- XLY-SPY spread=-0.44%p → sector diffusion weak

📌 Shadow Note: This filter checks whether leadership is broadening beyond mega-cap tech/AI. No impact on Final Exposure, Phase, or Allocation.


### 🧠 13) Narrative Engine (v2 + Risk Budget + Drift)
- **정의:** 구조·심리·크레딧·유동성·국면을 통합해 오늘의 리스크 액션을 결정
- **추가 이유:** 지표는 많지만 전략가는 결국 ‘리스크를 늘릴지/줄일지/유지할지’를 판단해야 하기 때문

- **Structure Bias:** Policy Bias: EASING (완화) (MODERATE, score=-1.5) | REAL_RATEΔ +0.000 / FCI value=-0.523 (low-frequency) / DXYΔ -0.170 / US10YΔ -0.065 (정상)
- **Sentiment (Fear&Greed):** 62.88998583175773 (NEUTRAL)
- **Credit Calm:** True
- **Liquidity (NET_LIQ):** UP (MID)
- **Structural Regime:** DISINFLATION
- **Operational Phase:** EARLY RISK-ON (Cap: 100)
- **Macro Tilt:** +0
- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / NONE
- **Drift Score:** 5
- **Flow Score:** 4
- **Flow Continuity:** 👀 EARLY TRACE → 👀 EARLY TRACE (FLOW_PERSISTENCE, tilt=+1)
- **Flow Regime Tilt:** +2 / Flow-Gamma Tilt: +2

- **🎯 Final Risk Action:** **REDUCE (POS_OVERHEATED)**
- **Risk Budget (0~100):** **73**
- **Narrative:** 구조=EASING / 심리=NEUTRAL / 유동성=증가/중간 / 크레딧=안정 / 드리프트=⚡ STRUCTURAL DRIFT (NEUTRAL) / 수급=2.34 ⚠️ 수급 과열 감지 → Phase=EARLY RISK-ON

### ⚠ 14) Divergence Monitor (Macro vs Positioning)
- **추가이유:** 시장 가격과 정책 사이의 괴리 및 수급의 '질'을 파악하여 폭발적 반전 가능성 진단
- **핵심질문:** 정책은 이런데 주가는 왜 반대로 가지?(Anomaly) 그 뒤에 숨은 수급 주체(CTA, Dealer)들은 지금 어떤 상태인가?

- **Structure(3번):** `EASING` | **Price(Regime):** `RISK-ON` | **Bucket:** `RISK-ON` | **VIX:** `17.01`
- **Positioning Data:** Z-Score: `2.34` (>2.2 시 Run) | Gamma: `2.22` (<0.5 시 Run) | CTA: `1.0` (추세 변곡점 확인)
- **Status:** **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.34). 반전 가능성 상존.
- **Action Signal:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🎯 15) Volatility-Controlled Exposure (v3.2)
- **정의:** 13번 Risk Budget 실행 브레이크 레이어
- **추가 이유:** 전략 판단(13) 이후 실제 진입 강도를 조절하기 위함

- **Base Risk Budget (13):** 73
- **VIX Level:** 17.01 (NORMAL) | **Change:** +1.86%
- **⚠️ STATUS:** RISK COMPRESSION
- **Reason:** POS_Z Extreme (2.34)
- **Action:** 신규 추격 금지 / 일부 이익실현 / 베타 노출 축소
- **Final Multiplier:** 0.88x (VIX x Positioning x Credit/Convexity)
- **Slope Intensity:** 0.0600
- **Positioning Layer:** ⚠️ Extreme Positioning Heat(2.34), Positive Gamma(2.22)
- **Brake Drivers:** ⚠️ Extreme Positioning Heat

- **📊 Recommended Exposure:** **64%**

### 🎨 16) Style Tilt (v1.1)
- **정의:** Macro 구조 기반 스타일 기울기 판단
- **추가 이유:** 같은 Risk-On이라도 어떤 유형의 자산이 유리한지 구분

- **Growth vs Value:** **GROWTH TILT**
- **Duration Tilt:** **LONG DURATION FAVORED**
- **Cyclical vs Defensive:** **NEUTRAL**

### 🧩 17) Factor Layer (v1)
- **정의:** 시장을 움직이는 핵심 위험 요인 판별
- **추가 이유:** 자금이 무엇에 민감하게 반응하는지 파악

- **Duration Factor:** LONG DURATION FAVORED
- **Inflation Factor:** DISINFLATION
- **USD Factor:** NEUTRAL
- **Credit Factor:** CREDIT SUPPORTIVE

### 🏭 18) Sector Allocation Engine (v3.3)

**Context:** phase=EARLY RISK-ON / T10Y2Y=0.49 (MODERATE STEEP) / VIX=17.01 (VOLATILITY NORMAL) / liquidity=UP-MID / credit=True

**Signal Priority:** VOL > LIQ > CURVE > CREDIT > PHASE > FLOW > MOM

**Macro Profile:** DISINFLATION_RISK_ON
**Macro Inputs Debug:** phase=EARLY RISK-ON / us10y_pct=-1.43% / dxy_pct=-0.17% / wti_pct=-2.81% / vix=17.01 / liq_easy=True / liq_tight=False / credit_calm=True / flow_score=4

**Flow Overlay:** flow_score=4 / flow_state=👀 EARLY TRACE / drift_label=NEUTRAL / gamma=🟡 POSITIVE-TRANSITION
**Flow Notes:** NEUTRAL + FLOW ACTIVE → XLK/XLI 소폭 가점 | Gamma POSITIVE → 리더 섹터 가점

**Overweight:** Technology, Communication Services

**Underweight:** Consumer Discretionary, Industrials, Financials, Utilities, Energy, Real Estate

**Scoreboard:**
- Technology: +2.7  (+2 LIQ, +1.5 PHASE, +1.5 FLOW, +2 MOM, = +2.7)
- Communication Services: +0.2  (+0.5 PHASE, = +0.2)
- Energy: -0.3  (-1 MOM, = -0.3)
- Real Estate: -0.3  (-1 MOM, = -0.3)
- Utilities: -0.6  (-1 LIQ, -0.5 PHASE, = -0.6)
- Financials: -1.3  (+1 LIQ, +2 CURVE, -2 MOM, = -1.3)
- Industrials: -1.4  (+1.5 LIQ, +1 CURVE, +1 FLOW, -2 MOM, = -1.4)
- Consumer Discretionary: -1.5  (+1.5 LIQ, +1 PHASE, -2 MOM, = -1.5)

**Rationale (Why the score exists: 섹터 점수의 핵심 드라이버)**
- OW Technology: +2: 유동성 완화 → 성장주/베타 우호
- OW Technology: +1.5: Disinflation Risk-On → 성장주/장기 듀레이션 우호
- OW Communication Services: +0.5: Disinflation Risk-On → 플랫폼/성장 섹터 우호
- UW Consumer Discretionary: THEORY_TRAP → 거시/이론 우호 대비 실제 자금흐름 및 상대강도 약세
- UW Consumer Discretionary: +1.5: 유동성 완화 → 소비 민감주 우호
- UW Consumer Discretionary: +1: Disinflation Risk-On → 소비 베타 우호
- UW Industrials: THEORY_TRAP → 거시/이론 우호 대비 실제 자금흐름 및 상대강도 약세
- UW Industrials: +1.5: 유동성 완화 → 경기민감 회복
- UW Industrials: +1: 완만한 스티프닝(0.49) → 성장 기대 반영
- UW Financials: THEORY_TRAP → 거시/이론 우호 대비 실제 자금흐름 및 상대강도 약세
- UW Financials: +1: 유동성 완화 → 위험선호 회복
- UW Financials: +2: 완만한 스티프닝(0.49) → 예대마진 개선

**Regime Controller:**
- DISLOCATION (avg_divergence=-1.27, dispersion=1.81)
Correlation Break: True / Leader=Technology
- Interpretation: 섹터별 괴리가 큰 불안정 장세 → 포지션 축소와 리더 섹터 검증 필요
- Correlation Break: True / Leader=Technology

**Divergence / Classification Monitor (Theory vs Flow alignment: 이론과 실제 자금흐름 정렬 여부)**
- Technology: HIGH_CONVICTION_ALIGNED (theory=+3.5, flow=+1.9, final=+2.7)
- Consumer Staples: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Health Care: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Materials: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Financials: THEORY_TRAP (theory=+3.0, flow=-1.4, final=-1.3)
- Industrials: THEORY_TRAP (theory=+2.5, flow=-1.1, final=-1.4)
- Consumer Discretionary: THEORY_TRAP (theory=+2.5, flow=-1.4, final=-1.5)

### 💰 18.5) Tactical Asset Allocation (Execution Weight)
- **Strategic Exposure (15):** **64.0%** → **Regime Adjusted:** **64.0%**
- **Exposure Override:** DISLOCATION → Sector Weight Only (No Exposure Change)

| Sector | Score | Divergence | **Weight in Portfolio** | **Action** |
| :--- | :---: | :---: | :---: | :--- |
| Technology | +3.2 | ALIGNED | **35.0%** | REBALANCE |
| Communication Services | +0.2 | ALIGNED | **3.4%** | NEW |
| **Cash & Hedge** | - | - | **61.6%** | DEFENSIVE |

- **Allocation Check:** Sector Weights + Cash = **100.0%**
- **Regime Cap Profile:** DISINFLATION_RISK_ON
- **Regime Cap Applied:**
  - Technology: 60.6% → 35.0% (-25.6%)
- **Strategic Cash (15):** 36.0%
- **Tactical Reserve (Cap / Unallocated):** 25.6%


**Deleveraging Priority Preview:**
- 기준: Divergence → Momentum → Score → Current Weight
1. Communication Services (priority_score=-0.1, score=0.21, weight=3.4%, div=ALIGNED, mom=0)
2. Technology (priority_score=-5.63, score=3.21, weight=35.0%, div=ALIGNED, mom=2)

**Leveraging Priority Preview:**
- 기준: Score → Momentum → Positive Divergence
1. Technology (priority_score=8.21, score=3.21, weight=35.0%, div=ALIGNED, mom=2)
2. Communication Services (priority_score=0.21, score=0.21, weight=3.4%, div=ALIGNED, mom=0)

### 🧬 19) Execution Layer (ETF Mapping)

| Sector | ETF | Weight | Action | Divergence | Classification |
| :--- | :---: | :---: | :--- | :--- | :--- |
| Technology | XLK | 35.0% | CORE_LEADER | ALIGNED | HIGH_CONVICTION_ALIGNED |
| Communication Services | XLC | 3.4% | SMALL | ALIGNED | ALIGNED |


### 🧬 19.5) Execution / Style Translation Layer
- **Implementation Focus:** Environment-Aware Stock Types

**Execution Notes:**
- Early flow trace → maintain leaders, wait for confirmation before broadening.
- Positioning heat elevated → prefer rebalancing over fresh chasing.

**Preferred Company Traits:**
- Leaders with improving breadth confirmation

**Risk Control / Avoid:**
- Broad beta expansion before confirmation
- Crowded late-entry trades

---


---

## 🌐 Country ETF Risk Monitor

### BND
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.0814962734012523
- **Z-Score (5d):** 1.7121591106389047

### EEM
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.835779855957296
- **Z-Score (5d):** 1.230816008270504

### EIS
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.2899069170545647
- **Z-Score (5d):** 1.3613728288360887

### EMB
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.9522480253020736
- **Z-Score (5d):** 1.130143300082495

### EWJ
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.8489761857441345
- **Z-Score (5d):** 0.7272469064527317

### FXI
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.5497001709318108
- **Z-Score (5d):** -0.27210670225443284

### GLD
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.164465040619628
- **Z-Score (5d):** 0.008930020388943308

### SPY
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.5507690438107757
- **Z-Score (5d):** 0.4389894577743271

### VXX
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** -0.6722051757088067
- **Z-Score (5d):** -0.7310848918135026
