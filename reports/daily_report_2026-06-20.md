# 🌍 Global Capital Flow – Daily Brief
**Date:** 2026-06-20
**Data as of:** 2026-06-18 🔒 MARKET CLOSED

🏦 GLOBAL MACRO DAILY — PM FINAL BRIEF

🧭 1. Executive Summary
Markets are operating in a TRANSITION / MIXED regime as liquidity conditions remain down and volatility pressures have increased. Participation remains 👀 EARLY TRACE, while positioning risk sits at 2.1. Maintain disciplined exposure and avoid adding marginal risk.
Rally Confidence: 🟡 MEDIUM
Participation is emerging, but confirmation remains limited. Wait for stronger breadth and persistence before increasing risk exposure.

🌍 2. Macro Regime
Liquidity: 🔻 Tightening
Flow: 🟡 Early Trace
Volatility: 🟢 Stable
Structure: MIXED

⚠️ 3. Key Macro Conflict

1) Liquidity vs Credit
• Liquidity: Tightening
• Credit: Stable
Interpretation:
Liquidity conditions are deteriorating, but credit markets have not yet confirmed stress.
Early-stage fragility signal rather than crisis.

2) Positioning Crowding
• POS_Z = 2.11
Interpretation:
Positioning remains elevated. Main risk is an air-pocket correction rather than structural collapse.

📊 4. Cross Asset Summary
Equities: 🟢 Stable
Bonds: 🟡 Neutral
USD: 🔺 Stronger
Oil: 🔻 Falling
Volatility: 🟢 Stable

Interpretation:
Falling yields and oil prices suggest growth-scare or disinflation pressure.
Defensive assets may outperform unless risk participation broadens.

🧠 5. Market Interpretation
Current market is best described as 'UNKNOWN_TRANSITION'.
Liquidity is slowing, but systemic stress remains contained.
Credit markets remain stable, although participation quality requires monitoring.

⚠️ 6. Risk Matrix
Inflation Risk      🟡 Moderate
Liquidity Risk      🔴 Elevated
Positioning Risk    🔴 Elevated
Credit Risk         🟢 Low
Geopolitical Risk   🟢 Low

🏭 7. Sector Implication
🟢 Preferred:
- Quality leaders
- Confirmed relative-strength sectors

🔴 Avoid:
- Weak breadth / theory-only sector bets

🎯 8. Tactical Recommendation
Recommendation: REDUCE / NONE
Confidence: LOW
Rationale:
- No actionable alignment
Maintain defensive discipline, avoid aggressive risk expansion, and prioritize cash plus resilient sectors.

📈 9. Internal Rotation Monitor
RSP vs SPY
Yesterday: -0.25%
Today: -0.32%
Change: -0.07%

QQQE vs QQQ
Yesterday: +0.11%
Today: -0.65%
Change: -0.76%

SMH vs SPY
Yesterday: +2.54%
Today: +4.98%
Change: +2.43%

IWM vs SPY
Yesterday: +0.50%
Today: +1.19%
Change: +0.69%


Interpretation:
Leadership narrowed back toward mega-cap and semiconductor exposure.
Participation remains weak outside leadership groups.

🏁 10. Final PM View
Action: REDUCE
Exposure: 46%
Bias: TRANSITION / MIXED
Key focus: monitor liquidity, participation quality, and positioning risk.

---

# 🔬 ENGINE DIAGNOSTICS

## ⚡ Strategic War Room (통합 대응)
> **시스템 상태: 🚨 ALERT**
> **판단 요약: 포지셔닝 과열 감지 / Hard Deadman은 아니나 추격보다 리스크 축소 우선**
### 🎯 Exposure Framework
- **Base Exposure (전략 기준): 46%**
- **Final Exposure (실행 기준): 46%**

- **Portfolio Stance:** REDUCE / 46%

- **[14번 구조·수급 괴리]:** 🚨 **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.11). 반전 가능성 상존.
### 🟢 Current SEW Status
- **SEW:** RISK_COMPRESSION | ⚠️ Risk Compression 발동 (권장 익스포저 55%)
- **Event Type:** NORMAL → 정상 상태 / 구조적 리스크 없음
- **Spike Monitor:** Spike 1 / Extreme 0
- **Current Reason:** ⚠️ CROWDING RISK: POS_Z Extreme (2.10) → Risk Compression (55%)

- **[15번 Hard Deadman]:** ✅ PASS
- **[14번 수급 시그널]:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🔬 Structural Layer (12.5~12.8)
- **Structural Layer:**
  - Growth Sustainability → **FRAGILE_EXPANSION** (Expansion remains possible, but the growth structure is not yet broad or durable. Monitor demand confirmation.)
  - Flow Authenticity → **SHORT_COVERING** (Price strength may be driven more by positioning than durable accumulation.)
  - Leadership Breadth → **BROAD_LEADERSHIP** (Leadership is broadening across sectors, confirming healthier market participation.)
  - Positioning Stress → **SQUEEZE_RISK** (Positioning is stretched and vulnerable to squeeze-driven reversals.)

## 🎯 Final Decision (War Room Override)
- **Final Action:** **REDUCE**
- **Final Exposure:** **46%**
- **Base Context:** phase=TRANSITION / MIXED / narrative=REDUCE / base_exposure=46%
- **SEW:** RISK_COMPRESSION / NORMAL
- **Divergence:** N/A / **MONITOR REVERSAL (RUN 액션 준비)**
- **Drift:** 🔥 EXTREME DRIFT / NEUTRAL / NONE / score=8
- **Flow:** 👀 EARLY TRACE / score=4
- **Gamma:** 🟡 POSITIVE-TRANSITION
- **Tactical Action:** HOLD / NONE / LOW
- **Positioning:** pos_z=2.11
- **Warning Score:** 4 (6.5 상관관계 붕괴, 6.6 섹터 상관관계 붕괴)
- **Tactical Why:** No actionable alignment
- **Why:** SEW STABLE → 실시간 이상징후 없음 → Divergence 비정렬 → 방어적 태도 유지 → 상위 레이어(SEW/Divergence)가 Narrative보다 우선 → Warning Score 3+ → 공격적 확장 금지 / 총노출 추가 감산 없이 방어적 배분 → Tactical=HOLD / Flow=👀 EARLY TRACE(4) / Drift=🔥 EXTREME DRIFT(8) / Gamma=🟡 POSITIVE-TRANSITION → Tactical HOLD/MONITOR → 최종판단 변경 없음

### 🚩 Market Regime Status
- **Operational Phase:** ✅ **TRANSITION / MIXED**
- **Structural Regime:** **UNKNOWN_TRANSITION**

---

## 📊 Daily Macro Signals

- **미국 10년물 금리**: 4.451  (-0.87% vs 4.490)
- **달러 인덱스**: 100.850  (+0.76% vs 100.090)
- **WTI 유가**: 76.600  (-0.25% vs 76.790)
- **변동성 지수 (VIX)**: 16.400 (-11.06% vs 18.440)
- **원/달러 환율**: 1525.420  (+0.96% vs 1510.960)

---

## 🧭 Strategist Commentary (Seyeon’s Filters)

### 🧩 1) Market Regime Filter
- **정의:** 지금 어떤 장(場)인지 판단하는 *시장 국면 필터*
- **추가 이유:** 같은 지표도 ‘국면’에 따라 의미가 완전히 달라지기 때문

- **VIX 레벨:** 16.40 → **Mid (Neutral/Mixed)**
- **핵심 조합(전일 대비 방향):** US10Y(↓) / DXY(↑) / VIX(↓)
- **판정:** **TRANSITION / MIXED | Flow:  (Flow Weak)**
- **근거:** Macro Narrative=UNKNOWN_TRANSITION / Policy=MIXED / Credit=COOL

### 💧 2) Liquidity Filter (Enhanced)
- **질문:** 시장에 새 돈이 들어오는가, 말라가는가?
- **추가 이유:** US10Y/DXY/VIX는 ‘시장의 기대’를 보여주고, FCI는 ‘현실의 압박’을, Real Rates는 ‘위험을 감수할 유인’을 보여준다.

- **기대(가격) 신호:** US10Y(↓) / DXY(↑) / VIX(↓)
- **현실(FCI):** value=-0.505 / level=EASY (완화) / update=low-frequency | as of: 2026-06-20 (latest available)
- **유인(Real Rates):** value=2.230 / level=RESTRICTIVE (유인↓) / dir(→) | as of: 2026-06-20 (latest available)
- **판정:** **LIQUIDITY MIXED / FRAGILE (혼조·취약)**
- **근거:** 기대(가격)와 현실(FCI)/유인(실질금리) 정렬이 불완전
- **Note:** FCI는 저빈도 금융환경 프록시로 level 중심 해석, Real Rates는 영업일 기준 변화 방향을 함께 반영함

### 🏛️ 3) Policy Filter (with Expectations)
- **질문:** 중앙은행·정책 환경은 완화인가, 긴축인가?

- **가격(현재) 신호:** US10Y(↓) / DXY(↑) / VIX(↓)
- **Policy Bias: MIXED (혼조) (WEAK, score=+0.5) | REAL_RATEΔ +0.000 / FCI value=-0.505 (low-frequency) / DXYΔ +0.760 / US10YΔ -0.039**
- **Expectations: dict received.**

- **판정:** **POLICY MIXED (정책 신호 혼조)**
- **근거:** 금리/달러/변동성 신호가 완전히 정렬되지 않음
- **한줄요약 ~~** 구조=MIXED (혼조)(WEAK)는 참고, 가격=POLICY MIXED (정책 신호 혼조) 중심 → 최종 POLICY MIXED (정책 신호 혼조)

### 🧰 4) Fed Plumbing Filter (TGA/RRP/Net Liquidity)
- **질문:** 시장의 ‘달러 체력’은 늘고 있나, 줄고 있나?
- **추가 이유:** 금리·달러가 안정적이어도 유동성이 빠지면 리스크 자산은 쉽게 흔들릴 수 있음
- **Liquidity as of:** 2026-06-17 (FRED latest)
- **NET_LIQ level:** 5855704.2
- **TGA level:** 880713.0
- **RRP level:** 6.828
- **방향(전일 대비):** TGA(↑) / RRP(↑) / NET_LIQ(↓)
- **판정:** **LIQUIDITY DRAINING (유동성 흡수)**
- **근거:** Net Liquidity↓ → 시장 내 달러 여력 축소 가능
- **Note:** TGA/RRP/WALCL은 매일 갱신되지 않을 수 있어, 리포트에는 ‘최근 available 값’을 반영함

### 🌡️ 4.2) High Yield Spread Filter (HY OAS)
- **질문:** 시장 공포의 ‘온도’는 올라가고 있나, 내려가고 있나?
- **추가 이유:** HYG/LQD가 ‘방향’이라면, HY Spread는 ‘강도(얼마나 무서워하는지)’를 보여줌
- **Spread as of:** 2026-06-17 (FRED latest)
- **HY_OAS level:** 2.63% → **COOL (낮은 공포)**
- **방향(전일 대비):** HY_OAS(↓) / -2.95%
- **판정:** **CREDIT CALM**
- **근거:** HY 스프레드 낮음 → 크레딧 스트레스 제한 / 스프레드가 좁혀지는 중 → 공포 온도 완화
- **Note:** HY OAS는 매일 갱신되지 않을 수 있어, ‘최근 available 값’을 반영함

### 🧾 4.5) Credit Stress Filter (HYG vs LQD)
- **질문:** 크레딧 시장이 먼저 ‘리스크오프’를 말하고 있는가?
- **추가 이유:** HYG가 LQD보다 약해지면, 시장이 ‘위험을 감수할 이유가 없다’고 판단하기 시작했을 가능성
- **방향(전일 대비):** HYG(↑) / LQD(↑)
- **HYG:** today 80.010 / prev 79.730 / pct 0.35%
- **LQD:** today 109.070 / prev 108.770 / pct 0.28%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, -0.87%)** → 완화 기대 강화
- DXY **(Strong, +0.76%)** → 달러 강세/신흥국 부담
- WTI **(Mild, -0.25%)** → 물가 부담 완화
- VIX **(Strong, -11.06%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Strong, +0.96%)** → 원화 약세/수급 부담
- HYG (High Yield ETF) **(Mild, +0.35%)** → 크레딧 위험선호↑
- LQD (IG Bond ETF) **(Mild, +0.28%)** → 우량채 강세(리스크오프 성향)

### 🧩 6) Cross-Asset Filter (자산군 연쇄 반응 분석)
- **추가 이유:** 단일 지표의 노이즈를 제거하고, 매크로 충격이 자산군 전반으로 확산되는 **전이 경로(Transmission Path)**를 파악하기 위함

- **금리 하락(US10Y↓)** → 할인율 압박 완화 → 달러 약세(DXY↓) 유도: **위험자산(Growth/EM) 선호 심리 강화 및 유동성 환경 개선**
- **변동성 하락(VIX↓)** → 심리 개선(Risk-On): **자산군 전반의 위험 수용 여력(Risk Appetite) 회복 및 랠리 지속 가능성**
- **유가 하락(WTI↓)** → 물가 부담 완화: **실질 구매력 회복 및 긴축 압력 완화(Dovish Tilt) 가능성 시사**

> **[Strategic Note]:** 위 연쇄 반응이 역사적 상관관계에서 벗어날 경우, **6.5) Correlation Break Monitor**를 통해 국면 전환 여부를 정밀 판별함

### 🌊 Drift Monitor (v4)
- **정의:** 누적 흐름 + ATR 기반 강도 감지

- **SPY:** 🟢 UP | Short-term: SHORT UP | 1D=+0.76% / 5D=+1.20% | Strength: LOW
- **WTI:** 🔴 DOWN | Short-term: SHORT DOWN | 1D=-1.75% / 5D=-11.33% | Strength: HIGH
- **DXY:** 🟢 UP | Short-term: SHORT UP | 1D=+0.10% / 5D=+1.20% | Strength: LOW
- **GOLD:** 🔴 DOWN | Short-term: SHORT DOWN | 1D=-1.34% / 5D=-1.13% | Strength: LOW

- **Drift Score:** 8
- **State:** **🔥 EXTREME DRIFT**
- **Label:** NEUTRAL
- **SEW Combo Signal:** NONE

- **Market Drift Summary:**
  - Equity (SPY): 🟢 UP / SHORT UP
  - Oil (WTI): 🔴 DOWN / SHORT DOWN
  - Dollar (DXY): 🟢 UP / SHORT UP
  - Gold (GOLD): 🔴 DOWN / SHORT DOWN

- **Drivers:**
  - SPY positive drift
  - Credit supports risk
  - Sector breadth expanding
  - Strong SPY continuation
  - Broad cyclical leadership
  - Dollar not restrictive

### ⚠ 6.5) Correlation Break Monitor
⚠ Market Closed / Stale Data → Correlation signals evaluated conservatively.

Correlation Break Detected:
- DXY ↑ but SPY ↑
- DXY ↑ but Technology ↑

So What?
- 결론: **공식이 깨진 구간** → 총노출 추가 감산보다 **추격 자제 + 퀄리티/리더 중심 배분**

### ⚠ 6.6) Sector Correlation Break Monitor
⚠ Market Closed / Stale Data → Sector signals evaluated conservatively.

Correlation Break Detected:
- US10Y ↓ but XLF ↓
- DXY ↑ but XLK ↑

So What?
- 결론: **섹터 ‘공식’이 깨진 구간** → 총노출 추가 감산보다 **섹터 배분 보수화 + 리더 중심**

### 🧩 7) Risk Exposure Filter (숨은 리스크 분석)
- **추가 이유:** 숫자는 괜찮아 보여도 그 뒤에 숨은 리스크를 식별하기 위함

- **VIX 하락(VIX↓)** → 심리 안정: 리스크 수용 여력 개선
- **금리 하락(US10Y↓)** → 완화 기대/할인율 부담 완화 가능
- **달러 강세(DXY↑)** → 신흥국·원자재·원화 등 위험자산에 부담
- **유가 하락(WTI↓)** → 물가 부담 완화 가능

### 🛰️ 7.2) Geopolitical Early Warning Monitor (FX/Commodities Composite)
⚠ Market Closed / Stale Data → Price-based geo signals muted.

- **Geo Stress Score (z-composite):** **-0.42**  *(Level: NORMAL)*
- **Coverage:** 100% *(used weight: 1.30 / defined weight: 1.30)*
- **3D Avg Score:** -0.09
- **Geo Momentum:** -0.33 *(Status: FALLING)*

**Historical Pattern Match (Cosine Similarity):**
- **Closest Historical Match:** Taiwan_Tension
- **Cosine Similarity Score:** -0.040
- **Similarity Signal:** Weak Historical Match
- **Top Similarity Matches:**
  - Taiwan_Tension: -0.040
  - China_Trade_2018: -0.090
  - Ukraine_2022: -0.149
- **Top Drivers:**
  - VIX: z_used=-1.15 (z1d=-1.28, z5d=-0.96, raw_w=0.18, norm_w=0.14) → contrib=-0.16
  - EEM: z_used=-1.17 (z1d=+1.42, z5d=+0.81, raw_w=0.10, norm_w=0.08) → contrib=-0.09
  - EMB: z_used=-0.64 (z1d=+0.89, z5d=+0.25, raw_w=0.12, norm_w=0.09) → contrib=-0.06
  - BDRY: z_used=-1.52 (z1d=-1.26, z5d=-1.91, raw_w=0.05, norm_w=0.04) → contrib=-0.06
- **Missing/Skipped:** None
- **Sovereign Spread factors included:** KR10Y_SPREAD, JP10Y_SPREAD, DE10Y_SPREAD, IL10Y_SPREAD

**Trade Information:**
- 지정학 스트레스는 여전히 정상 범위에 있지만 최근 압력이 완화되고 있는 중입니다. 경계 유지.
- 역사적 위기 패턴 유사도는 낮습니다. 현재는 **Taiwan_Tension** 유형과 가장 가깝지만, 전면적 지정학 쇼크보다는 제한적·국지적 리스크 모니터링 구간으로 해석됩니다.
- **Country ETF Crash?** Yes (EIS, FXI)
- **Extreme Country Risk:** EIS, FXI

### ⚡ 7.3) Pseudo Gamma Filter
- **정의:** 옵션 데이터 없이 시장의 감마 환경을 추론
- **주의:** Dealer Gamma Bias 숫자와 Pseudo Gamma State는 서로 다른 레이어

- **Pseudo Gamma State:** 🟡 POSITIVE-TRANSITION
- **Dealer Gamma Bias:** 1.78 (STABILIZING / dealer gamma proxy supportive)
- **Bias:** VIX는 안정적이나 Drift가 형성 중
- **Strategy:** 초기 방향성 관찰 / 과도한 추격 금지

- **Drift Score:** 8 (🔥 EXTREME DRIFT)
- **VIX:** 16.399999618530273
- **SEW:** RISK_COMPRESSION / NORMAL

- **🚀 Combo Signal:** 🟢 EARLY FLOW WITHOUT SHOCK

### 🏦 Institutional Flow Engine (v2-minimal)
- **정의:** 기관성 자금이 뉴스 전에 남기는 흔적을 구조적으로 탐지

- **Raw Flow State:** **👀 EARLY TRACE**
- **Transition State:** **👀 EARLY TRACE**
- **Flow Delta:** +0 (prev=4 → current=4)
- **Persistence Days:** 1
- **Transition Note:** 기관성 흐름 상태 유지
- **Confidence:** **MEDIUM**
- **Action Bias:** **MONITOR**

- **Drift:** 🔥 EXTREME DRIFT / NEUTRAL / NONE
- **Gamma:** 🟡 POSITIVE-TRANSITION / 🟢 EARLY FLOW WITHOUT SHOCK
- **SEW:** RISK_COMPRESSION / NORMAL
- **Positioning (POS_Z):** 2.11
- **Validation Score:** 4 (boost applied: +2)

- **Drivers:**
  - Drift strong
  - Gamma transition
  - Positioning overheated
  - Cross-asset risk participation
  - Credit confirms risk appetite
  - Leadership breadth expanding
  - Cyclical leadership over defensives

### 🎯 8) Incentive Filter (Wall St. Logic)

**핵심 신호:** 장단기차(27.00bp) | 실질금리(2.23%) | DXY(100.85)
*(as of: RealRate: 2026-06-20 / FRED last available)*

❌ **자본이 탈출하는 곳 (Short Incentive):**
고금리(실질금리 2% 상회) 부담으로 인한 리스크 오프 신호

- **Note:** 실질금리와 달러는 자본의 '기회비용'을 결정하는 핵심 유인책입니다.

### 🔍 9) Cause Filter
- **질문:** 무엇이 이 움직임을 만들었는가?
- **핵심 신호:** 금리↓ + 달러↑ + 유가↓ + VIX↓
- **최종 판정:** **인플레이션 압력 둔화 및 비용 감소형 안도 랠리**

### 🔄 10) Direction Filter
- **질문:** 오늘 움직임은 ‘노이즈’인가 ‘의미 있는 변화’인가?
- **강도:** US10Y(Strong) / DXY(Strong) / WTI(Mild) / VIX(Strong)
- **판정:** **SIGNIFICANT MOVE (의미 있는 변화)**

### ⏳ 11) Timing Filter
- **질문:** 이 신호는 단기/중기/장기 중 어디에 더 중요하게 작용하는가?
- **가이드:**
  - 금리/달러의 ‘레벨’ 변화는 중기(수 주~수개월) 영향이 더 큼
  - VIX 급등/급락은 단기(수 일~수 주) 심리 변화에 민감
- **Today snapshot:** US10Y(4.451), DXY(100.850), VIX(16.40)

### 🏗️ 12) Structural Filter (v3)
- **질문:** 글로벌 화폐 가치와 에너지 패권 등 '판'의 변화가 있는가?
- **핵심 신호:** US10Y(↓) / DXY(↑) / GOLD(↓) / VIX(↓) / WTI(↓)
- **Meaningful Move Check:** DXY=0.7593187771381286 / GOLD=-3.0925189315547046 / US10Y=-0.868596881959924 / VIX=-11.062911369007717 / WTI=-0.24743122690578132
- **판정:** **NEUTRAL**
- **근거:** 글로벌 매크로 구조의 특이 신호가 감지되지 않음



### 12.5) Growth Sustainability Filter [SHADOW]
- **Score:** 4
- **Label:** FRAGILE_EXPANSION
- **Demand Proxy:** 1
- **Financing:** 2
- **Energy Burden:** 2
- **Policy Capacity:** -1
- **Strategic Interpretation:** Expansion remains possible, but the growth structure is not yet broad or durable. Monitor demand confirmation.
- **Input Check:** US10Y=4.451, RealYield=2.23, T10Y2Y=0.27, WTI=76.5999984741211, DXY=100.8499984741211, LiquidityDir=DOWN, CreditCalm=True, HY_OAS=2.63, DriftLabel=NEUTRAL, FredAsof=2026-06-18

📌 Shadow Note: This filter is observation-only and does not affect Final Exposure, Phase, or Sector Allocation.




### 12.8) Positioning Stress Filter [SHADOW]

- **Score:** -1
- **Label:** SQUEEZE_RISK
- **Strategic Interpretation:** Positioning is stretched and vulnerable to squeeze-driven reversals.

**Positioning Notes**
- Term Structure: VIX3M-VIX=3.17 → healthy contango / stable structure
- Short-Term Hedge: VIX9D/VIX=0.85 → calm front-end hedge
- Gamma Structure: Positive gamma extreme → dealer-supported squeeze risk
- Positioning: Crowded long positioning

📌 Shadow Note: This filter estimates whether current market behavior reflects structural participation or unstable positioning stress (squeeze / unwind / panic). No impact on Final Exposure or Phase, but used as context for Sector Allocation risk controls.



### 12.6) Flow Authenticity Filter [SHADOW]
- **Score:** -1
- **Label:** SHORT_COVERING
- **Strategic Interpretation:** Price strength may be driven more by positioning than durable accumulation.
- **Breadth / Participation:** -3
- **Breadth Note:** RSP-SPY return spread=-0.32%p → narrow cap-weight leadership
- **Nasdaq Breadth Note:** QQQE-QQQ return spread=-0.65%p → mega-cap concentrated Nasdaq rally
- **Positioning / Gamma:** -2
- **Credit Confirmation:** 2
- **Macro Participation:** 2

📌 Shadow Note: This filter estimates whether upside is driven by real accumulation or short-covering. No impact on Final Exposure, Phase, or Allocation.



### 12.7) Leadership Breadth Filter [SHADOW]
- **Score:** 6
- **Label:** BROAD_LEADERSHIP
- **Strategic Interpretation:** Leadership is broadening across sectors, confirming healthier market participation.

**Leadership Notes**
- QQQ-SPY spread=1.73%p → growth leadership
- SMH-QQQ spread=3.25%p → semiconductor participation strong
- SOXX-QQQ spread=4.12%p → SOX proxy confirms chip breadth
- IWM-SPY spread=1.19%p → small-cap participation confirms broader risk-on
- XLF-SPY spread=-1.67%p → sector diffusion weak
- XLI-SPY spread=-0.05%p → sector diffusion neutral
- XLY-SPY spread=0.67%p → sector diffusion positive

📌 Shadow Note: This filter checks whether leadership is broadening beyond mega-cap tech/AI. No impact on Final Exposure or Phase, but used as context for Sector Allocation risk controls.


### 🧠 13) Narrative Engine (v2 + Risk Budget + Drift)
- **정의:** 구조·심리·크레딧·유동성·국면을 통합해 오늘의 리스크 액션을 결정
- **추가 이유:** 지표는 많지만 전략가는 결국 ‘리스크를 늘릴지/줄일지/유지할지’를 판단해야 하기 때문

- **Structure Bias:** Policy Bias: MIXED (혼조) (WEAK, score=+0.5) | REAL_RATEΔ +0.000 / FCI value=-0.505 (low-frequency) / DXYΔ +0.760 / US10YΔ -0.039 (정상)
- **Sentiment (Fear&Greed):** 60.81826021324373 (NEUTRAL)
- **Credit Calm:** True
- **Liquidity (NET_LIQ):** DOWN (MID)
- **Structural Regime:** UNKNOWN_TRANSITION
- **Operational Phase:** TRANSITION / MIXED (Cap: 65)
- **Macro Tilt:** +0
- **Drift:** 🔥 EXTREME DRIFT / NEUTRAL / NONE
- **Drift Score:** 8
- **Flow Score:** 4
- **Flow Continuity:** 👀 EARLY TRACE → 👀 EARLY TRACE (FLOW_PERSISTENCE, tilt=+1)
- **Flow Regime Tilt:** +2 / Flow-Gamma Tilt: +2

- **🎯 Final Risk Action:** **REDUCE**
- **Risk Budget (0~100):** **50**
- **Narrative:** 구조=MIXED / 심리=NEUTRAL / 유동성=감소/중간 / 크레딧=안정 / 드리프트=🔥 EXTREME DRIFT (NEUTRAL) / 수급=2.11 ⚠️ 수급 과열 감지 → Phase=TRANSITION / MIXED

### ⚠ 14) Divergence Monitor (Macro vs Positioning)
- **추가이유:** 시장 가격과 정책 사이의 괴리 및 수급의 '질'을 파악하여 폭발적 반전 가능성 진단
- **핵심질문:** 정책은 이런데 주가는 왜 반대로 가지?(Anomaly) 그 뒤에 숨은 수급 주체(CTA, Dealer)들은 지금 어떤 상태인가?

- **Structure(3번):** `MIXED` | **Price(Regime):** `TRANSITION / MIXED` | **Bucket:** `MIXED` | **VIX:** `16.40`
- **Positioning Data:** Z-Score: `2.11` (>1.8 시 Run) | Gamma: `1.78` (<0.5 시 Run) | CTA: `1.0` (추세 변곡점 확인)
- **Status:** **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.11). 반전 가능성 상존.
- **Action Signal:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🎯 15) Volatility-Controlled Exposure (v3.2)
- **정의:** 13번 Risk Budget 실행 브레이크 레이어
- **추가 이유:** 전략 판단(13) 이후 실제 진입 강도를 조절하기 위함

- **Base Risk Budget (13):** 50
- **VIX Level:** 16.40 (NORMAL) | **Change:** -11.06%
- **⚠️ STATUS:** RISK COMPRESSION
- **Reason:** POS_Z Extreme (2.11)
- **Action:** 신규 추격 금지 / 일부 이익실현 / 베타 노출 축소
- **Final Multiplier:** 0.92x (VIX x Positioning x Credit/Convexity)
- **Slope Intensity:** 0.0050
- **Positioning Layer:** ⚠️ Extreme Positioning Heat(2.11), Positive Gamma(1.78)
- **Brake Drivers:** ⚠️ Extreme Positioning Heat

- **📊 Recommended Exposure:** **46%**

### 🎨 16) Style Tilt (v1.1)
- **정의:** Macro 구조 기반 스타일 기울기 판단
- **추가 이유:** 같은 Risk-On이라도 어떤 유형의 자산이 유리한지 구분

- **Growth vs Value:** **NEUTRAL**
- **Duration Tilt:** **LONG DURATION FAVORED**
- **Cyclical vs Defensive:** **NEUTRAL**

### 🧩 17) Factor Layer (v1)
- **정의:** 시장을 움직이는 핵심 위험 요인 판별
- **추가 이유:** 자금이 무엇에 민감하게 반응하는지 파악

- **Duration Factor:** LONG DURATION FAVORED
- **Inflation Factor:** NEUTRAL
- **USD Factor:** USD TIGHTENING
- **Credit Factor:** CREDIT SUPPORTIVE

### 🏭 18) Sector Allocation Engine (v3.3)

**Context:** phase=TRANSITION / MIXED / T10Y2Y=0.27 (MODERATE STEEP) / VIX=16.40 (VOLATILITY NORMAL) / liquidity=DOWN-MID / credit=True

**Signal Priority:** VOL > LIQ > CURVE > CREDIT > PHASE > FLOW > MOM

**Macro Profile:** BALANCED
**Macro Inputs Debug:** phase=TRANSITION / MIXED / us10y_pct=-0.87% / dxy_pct=+0.76% / wti_pct=-0.25% / vix=16.40 / liq_easy=False / liq_tight=True / credit_calm=True / flow_score=4

**Flow Overlay:** flow_score=4 / flow_state=👀 EARLY TRACE / drift_label=NEUTRAL / gamma=🟡 POSITIVE-TRANSITION
**Flow Notes:** NEUTRAL + FLOW ACTIVE → XLK/XLI 소폭 가점 | Gamma POSITIVE → 리더 섹터 가점

**Overweight:** Technology, Consumer Staples, Health Care, Utilities, Industrials, Communication Services, Financials

**Underweight:** Real Estate, Consumer Discretionary, Energy

**Scoreboard:**
- Technology: +1.4  (-2 LIQ, +1.5 FLOW, +2 MOM, = +1.4)
- Consumer Staples: +1.0  (+2 LIQ, +0.5 PHASE, = +1.0)
- Health Care: +1.0  (+2 LIQ, +0.5 PHASE, = +1.0)
- Utilities: +0.4  (+1 LIQ, = +0.4)
- Industrials: +0.3  (+1 CURVE, +1 FLOW, +1 MOM, = +0.3)
- Communication Services: +0.3  (= +0.3)
- Financials: +0.2  (+2 CURVE, = +0.2)
- Energy: -0.6  (-2 MOM, = -0.6)
- Consumer Discretionary: -1.5  (-1 LIQ, -1 MOM, = -1.5)
- Real Estate: -1.7  (-1.5 LIQ, -1 MOM, = -1.7)

**Rationale (Why the score exists: 섹터 점수의 핵심 드라이버)**
- OW Technology: POSITIVE_DIVERGENCE → 거시 대비 실제 자금 선행 유입
- OW Technology: +1: Gamma Overlay → POSITIVE, 추세 지속 우호
- OW Technology: +2: Relative Strength 강세 (vs SPY) → 자금 유입 확인
- OW Consumer Staples: FLOW_WEAK → 이론상 우호하나 실제 자금 유입 확인 부족
- OW Consumer Staples: +2: 유동성 긴축 → 방어적 필수소비 선호
- OW Consumer Staples: +0.5: Balanced Macro Profile → 방어 보완
- OW Health Care: FLOW_WEAK → 이론상 우호하나 실제 자금 유입 확인 부족
- OW Health Care: +2: 유동성 긴축 → 안정적 현금흐름 선호
- OW Health Care: +0.5: Balanced Macro Profile → 퀄리티 보완
- OW Utilities: +1: 유동성 긴축 → 방어주 버퍼
- UW Real Estate: -1.5: 유동성 긴축 → 조달비용 상승 부담
- UW Real Estate: -1: Relative Strength 약세 (vs SPY) → 소외 섹터

**Regime Controller:**
- DISLOCATION (avg_divergence=-0.40, dispersion=1.74)
- Correlation Break: True / Type=MACRO_TIGHTENING_TECH_RALLY
- Break Reasons: DXY ↑ but SPY ↑ | DXY ↑ but Technology ↑ | US10Y ↓ but XLF ↓ | DXY ↑ but XLK ↑
- Interpretation: 섹터별 괴리가 큰 불안정 장세 → 포지션 축소와 리더 섹터 검증 필요

**Divergence / Classification Monitor (Theory vs Flow alignment: 이론과 실제 자금흐름 정렬 여부)**
- Technology: POSITIVE_DIVERGENCE (theory=-2.0, flow=+1.9, final=+1.4)
- Consumer Staples: FLOW_WEAK (theory=+2.5, flow=+0.0, final=+1.0)
- Health Care: FLOW_WEAK (theory=+2.5, flow=+0.0, final=+1.0)
- Communication Services: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.3)
- Financials: FLOW_WEAK (theory=+2.0, flow=+0.0, final=+0.2)
- Materials: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Consumer Discretionary: AVOID (theory=-1.0, flow=-0.7, final=-1.5)
- Real Estate: AVOID (theory=-1.5, flow=-0.7, final=-1.7)

### 💰 18.5) Tactical Asset Allocation (Execution Weight)
- **Strategic Exposure (15):** **46.0%** → **Regime Adjusted:** **46.0%**
- **Exposure Override:** DISLOCATION → Sector Weight Only (No Exposure Change)

| Sector | Score | Divergence | **Weight in Portfolio** | **Action** |
| :--- | :---: | :---: | :---: | :--- |
| Technology | +1.4 | POSITIVE_DIVERGENCE | **15.0%** | HOLD |
| Consumer Staples | +1.0 | NEGATIVE_DIVERGENCE | **5.4%** | HOLD |
| Health Care | +1.0 | NEGATIVE_DIVERGENCE | **5.4%** | HOLD |
| Utilities | +0.4 | ALIGNED | **3.8%** | HOLD |
| Industrials | +0.3 | ALIGNED | **3.7%** | HOLD |
| Communication Services | +0.3 | ALIGNED | **2.8%** | HOLD |
| Financials | +0.2 | NEGATIVE_DIVERGENCE | **1.3%** | HOLD |
| **Cash & Hedge** | - | - | **62.6%** | DEFENSIVE |

- **Allocation Check:** Sector Weights + Cash = **100.0%**
- **Regime Cap Profile:** BALANCED
- **Participation / Quality Cap Applied:**
  - Technology: 19.8% → 15.0% (-4.8%)
- **Strategic Cash (15):** 54.0%
- **Tactical Reserve (Cap / Unallocated):** 8.6%


**Deleveraging Priority Preview:**
- 기준: Divergence → Momentum → Score → Current Weight
1. Financials (priority_score=3.88, score=0.23, weight=1.3%, div=NEGATIVE_DIVERGENCE, mom=0)
2. Consumer Staples (priority_score=3.52, score=0.97, weight=5.4%, div=NEGATIVE_DIVERGENCE, mom=0)
3. Health Care (priority_score=3.52, score=0.97, weight=5.4%, div=NEGATIVE_DIVERGENCE, mom=0)
4. Communication Services (priority_score=-0.15, score=0.3, weight=2.8%, div=ALIGNED, mom=0)
5. Utilities (priority_score=-0.2, score=0.41, weight=3.8%, div=ALIGNED, mom=0)

**Leveraging Priority Preview:**
- 기준: Score → Momentum → Positive Divergence
1. Technology (priority_score=5.39, score=1.39, weight=15.0%, div=POSITIVE_DIVERGENCE, mom=2)
2. Industrials (priority_score=1.81, score=0.31, weight=3.7%, div=ALIGNED, mom=1)
3. Utilities (priority_score=0.41, score=0.41, weight=3.8%, div=ALIGNED, mom=0)
4. Communication Services (priority_score=0.30, score=0.3, weight=2.8%, div=ALIGNED, mom=0)
5. Consumer Staples (priority_score=-2.03, score=0.97, weight=5.4%, div=NEGATIVE_DIVERGENCE, mom=0)
- **Divergence Adjustment:** Consumer Staples, Health Care, Financials penalized in weight sizing

### 🧬 19) Execution Layer (ETF Mapping)

| Sector | ETF | Weight | Action | Divergence | Classification |
| :--- | :---: | :---: | :--- | :--- | :--- |
| Technology | XLK | 15.0% | TACTICAL_LEADER | POSITIVE_DIVERGENCE | POSITIVE_DIVERGENCE |
| Consumer Staples | XLP | 5.4% | WATCHLIST_SMALL | NEGATIVE_DIVERGENCE | FLOW_WEAK |
| Health Care | XLV | 5.4% | WATCHLIST_SMALL | NEGATIVE_DIVERGENCE | FLOW_WEAK |
| Utilities | XLU | 3.8% | SMALL | ALIGNED | ALIGNED |
| Industrials | XLI | 3.7% | SMALL | ALIGNED | ALIGNED |
| Communication Services | XLC | 2.8% | NO_ACTION | ALIGNED | NEUTRAL |
| Financials | XLF | 1.3% | WATCHLIST_SMALL | NEGATIVE_DIVERGENCE | FLOW_WEAK |


### 🧬 19.5) Execution / Style Translation Layer
- **Implementation Focus:** Environment-Aware Stock Types

**Execution Notes:**
- Early flow trace → maintain leaders, wait for confirmation before broadening.
- Positioning heat elevated → prefer rebalancing over fresh chasing.

**Preferred Company Traits:**
- High Free Cash Flow generators
- Net cash or low leverage balance sheets
- Stable margins / pricing power
- Low to mid beta exposure
- RAROC-friendly profile
- Leaders with improving breadth confirmation

**Risk Control / Avoid:**
- Negative FCF / cash-burn models
- High leverage / refinancing-dependent names
- Long-duration, high-multiple growth
- Broad beta expansion before confirmation
- Crowded late-entry trades

---


---

## 🌐 Country ETF Risk Monitor

### BND
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.9751066735896321
- **Z-Score (5d):** -0.1024803782971909

### EEM
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.404785680104362
- **Z-Score (5d):** 0.9300265353154578

### EIS
- **Crash?** True
- **Risk Level:** EXTREME
- **Z-Score (1d):** 1.037761460833633
- **Z-Score (5d):** -0.929845879792897

### EMB
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.8509699321590369
- **Z-Score (5d):** 0.13645623624888562

### EWJ
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 1.1326189626042835
- **Z-Score (5d):** 1.508631244793492

### FXI
- **Crash?** True
- **Risk Level:** EXTREME
- **Z-Score (1d):** -0.7341739245618922
- **Z-Score (5d):** -1.6126679502857002

### GLD
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** -0.189728064678925
- **Z-Score (5d):** 0.21763939935543

### SPY
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.841345814047055
- **Z-Score (5d):** 0.17054579831421443

### VXX
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** -0.7567176266530208
- **Z-Score (5d):** -1.1140831078060278
