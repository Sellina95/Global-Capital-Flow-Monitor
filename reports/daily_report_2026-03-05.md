...
- **미국 10년물 금리**: 4.080  (-0.05% vs 4.082)
- **달러 인덱스**: 98.822  (-0.11% vs 98.928)
- **WTI 유가**: 76.110  (+2.28% vs 74.410)
- **변동성 지수 (VIX)**: 21.150  (-4.30% vs 22.100)
- **원/달러 환율**: 1461.770  (-0.31% vs 1466.380)
...
- 전략적으로는 노출 확대가 가능하나, 규율 기반으로 75% 내에서 단계적으로 접근한다.
...
- **VIX 레벨:** 21.15 → **High (Risk-off bias)**
...
- **현실(FCI):** level=EASY (완화) / dir(→) | as of: 2026-03-03 (FRED last available)
- **유인(Real Rates):** level=NEUTRAL (중립) / dir(↓) | as of: 2026-03-03 (FRED last available)
...
- **Policy Bias: MIXED (혼조) (WEAK, score=-0.5) | REAL_RATEΔ +0.010 / FCIΔ +0.000 / DXYΔ -0.106 / US10YΔ -0.002**
...
- **Spread as of:** 2026-03-03 (FRED latest)
- **HY_OAS level:** 3.08% → **WARM (경계)**
- **방향(전일 대비):** HY_OAS(↑) / +1.65%
- **판정:** **CREDIT WATCH**
...
- **방향(전일 대비):** HYG(↑) / LQD(↓)
- **HYG:** today 80.400 / prev 80.325 / pct 0.09%
- **LQD:** today 110.970 / prev 111.000 / pct -0.03%
...
- WTI **(Strong, +2.28%)** → 인플레 재자극 가능성
- VIX **(Strong, -4.30%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Clear, -0.31%)** → 원화 강세/수급 개선
- HYG (High Yield ETF) **(Noise, +0.09%)** → 크레딧 위험선호↑
...
- DEBUG: pct XLK=0.4489458868950109, XLF=0.06800708696850052, XLE=0.464863794233049, XLRE=0.6208246324352056
...
- **Top Drivers:**
  - USDMXN: z_used=+2.68 (w=0.05) → contrib=+0.13
  - USDCNH: z_used=-0.70 (w=0.18) → contrib=-0.13
  - VIX: z_used=-0.68 (w=0.18) → contrib=-0.12
  - KR10Y_SPREAD: z_used=+1.49 (w=0.08) → contrib=+0.12
...
- **Today snapshot:** US10Y(4.080), DXY(98.822), VIX(21.15)
...
- **Structure Bias:** Policy Bias: MIXED (혼조) (WEAK, score=-0.5) | REAL_RATEΔ +0.010 / FCIΔ +0.000 / DXYΔ -0.106 / US10YΔ -0.002
- **Sentiment (Fear&Greed):** 30.45315612620901 (NEUTRAL)
...
- **Narrative:** 구조=MIXED / 심리=NEUTRAL / 유동성=증가/중간 / 크레딧=안정 → Phase=RISK-ON (완화 기대·리스크 선호)
...
- **Context:** phase=RISK-ON (완화 기대·리스크 선호) / liquidity=UP-MID / structure=MIXED / credit_calm=True
