# 🌍 Global Capital Flow – Daily Brief
**Date:** 2026-06-13
**Data as of:** 2026-06-12

🏦 GLOBAL MACRO DAILY — PM FINAL BRIEF

🧭 1. Executive Summary
Markets are operating in a TRANSITION / MIXED regime as liquidity conditions remain up and volatility pressures have increased. Participation remains ⚡ BUILDING, while positioning risk sits at 1.8. Maintain disciplined exposure and avoid adding marginal risk.
Rally Confidence: 🟢 HIGH
Participation and leadership support trend continuation. Risk exposure may be maintained, subject to volatility and positioning controls.

🌍 2. Macro Regime
Liquidity: 🟢 Improving
Flow: ⚡ BUILDING
Volatility: 🟢 Stable
Structure: MIXED

⚠️ 3. Key Macro Conflict

2) Positioning Crowding
• POS_Z = 1.84
Interpretation:
Positioning remains elevated. Main risk is an air-pocket correction rather than structural collapse.

📊 4. Cross Asset Summary
Equities: 🟢 Stable
Bonds: 🟡 Neutral
USD: 🔻 Weaker
Oil: 🔻 Falling
Volatility: 🟢 Stable

Interpretation:
Dollar weakness and lower volatility are providing temporary breathing room for equities.
However, rising yields limit the quality of the risk-on signal.

🧠 5. Market Interpretation
Current market is best described as 'UNKNOWN_TRANSITION'.
Liquidity is slowing, but systemic stress remains contained.
Credit markets remain stable, although participation quality requires monitoring.

⚠️ 6. Risk Matrix
Inflation Risk      🟡 Moderate
Liquidity Risk      🟢 Low
Positioning Risk    🔴 Elevated
Credit Risk         🟢 Low
Geopolitical Risk   🟢 Low

🏭 7. Sector Implication
🟢 Preferred:
- Quality leaders
- Confirmed relative-strength sectors

🔴 Avoid:
- Weak breadth / theory-only sector bets

🎯 8. Tactical Recommendation
Recommendation: REDUCE / NONE
Confidence: LOW
Rationale:
- No actionable alignment
Maintain defensive discipline, avoid aggressive risk expansion, and prioritize cash plus resilient sectors.

📈 9. Internal Rotation Monitor
RSP vs SPY
Yesterday: -0.14%
Today: +0.37%
Change: +0.51%

QQQE vs QQQ
Yesterday: -0.17%
Today: +0.59%
Change: +0.76%

SMH vs SPY
Yesterday: +5.05%
Today: +1.18%
Change: -3.87%

IWM vs SPY
Yesterday: +1.26%
Today: +0.33%
Change: -0.93%


Interpretation:
Participation is broadening while semiconductor leadership remains supportive.
Market participation appears healthier than a pure mega-cap driven rally.

🏁 10. Final PM View
Action: REDUCE
Exposure: 48%
Bias: TRANSITION / MIXED
Key focus: monitor liquidity, participation quality, and positioning risk.

---

# 🔬 ENGINE DIAGNOSTICS

## ⚡ Strategic War Room (통합 대응)
> **시스템 상태: 🚨 ALERT**
> **판단 요약: 구조·수급 괴리 및 추세 피로 감지 / 반전 가능성 모니터링 필요**
### 🎯 Exposure Framework
- **Base Exposure (전략 기준): 61%**
- **Final Exposure (실행 기준): 48%**

- **Portfolio Stance:** REDUCE / 48%

- **[14번 구조·수급 괴리]:** 🚨 **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:1.84). 반전 가능성 상존.
### 🟢 Current SEW Status
- **SEW:** STABLE | ✅ 이상징후 없음 (5개 자산 정상 범위 / z-score 발작 없음)
- **Event Type:** NORMAL → 정상 상태 / 구조적 리스크 없음
- **Spike Monitor:** Spike 0 / Extreme 0

- **[15번 Hard Deadman]:** ✅ PASS
- **[14번 수급 시그널]:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🔬 Structural Layer (12.5~12.8)
- **Structural Layer:**
  - Growth Sustainability → **DURABLE_GROWTH** (Growth structure looks durable: demand, financing, and policy conditions are broadly supportive.)
  - Flow Authenticity → **REAL_ACCUMULATION** (Institutional participation appears broad and persistent.)
  - Leadership Breadth → **BROAD_LEADERSHIP** (Leadership is broadening across sectors, confirming healthier market participation.)
  - Positioning Stress → **SQUEEZE_RISK** (Positioning is stretched and vulnerable to squeeze-driven reversals.)

## 🎯 Final Decision (War Room Override)
- **Final Action:** **REDUCE**
- **Final Exposure:** **48%**
- **Base Context:** phase=TRANSITION / MIXED / narrative=HOLD / base_exposure=61%
- **SEW:** STABLE / NORMAL
- **Divergence:** N/A / **MONITOR REVERSAL (RUN 액션 준비)**
- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / 🟢 EARLY FLOW WITHOUT SHOCK / score=7
- **Flow:** ⚡ BUILDING / score=6
- **Gamma:** 🟡 POSITIVE-TRANSITION
- **Tactical Action:** HOLD / NONE / LOW
- **Positioning:** pos_z=1.84
- **Warning Score:** 4 (6.5 상관관계 붕괴, 6.6 섹터 상관관계 붕괴)
- **Tactical Why:** No actionable alignment
- **Why:** SEW STABLE → 실시간 이상징후 없음 → Divergence 비정렬 → HOLD에서 REDUCE로 전환 → 상위 레이어(SEW/Divergence)가 Narrative보다 우선 → Warning Score 3+ → 공격적 확장 금지 / 총노출 추가 감산 없이 방어적 배분 → Tactical=HOLD / Flow=⚡ BUILDING(6) / Drift=⚡ STRUCTURAL DRIFT(7) / Gamma=🟡 POSITIVE-TRANSITION → Tactical HOLD/MONITOR → 최종판단 변경 없음

### 🚩 Market Regime Status
- **국면 전환 감지:** 🚨 **RISK-ON (완화 기대·리스크 선호)** → **TRANSITION / MIXED**
- **Structural Regime:** **UNKNOWN_TRANSITION**

---

## 📊 Daily Macro Signals

- **미국 10년물 금리**: 4.487  (+0.54% vs 4.463)
- **달러 인덱스**: 99.750  (-0.11% vs 99.860)
- **WTI 유가**: 84.880  (-3.23% vs 87.710)
- **변동성 지수 (VIX)**: 17.680 (-9.05% vs 19.440)
- **원/달러 환율**: 1517.890  (-0.47% vs 1525.050)

---

## 🧭 Strategist Commentary (Seyeon’s Filters)

### 🧩 1) Market Regime Filter
- **정의:** 지금 어떤 장(場)인지 판단하는 *시장 국면 필터*
- **추가 이유:** 같은 지표도 ‘국면’에 따라 의미가 완전히 달라지기 때문

- **VIX 레벨:** 17.68 → **Mid (Neutral/Mixed)**
- **핵심 조합(전일 대비 방향):** US10Y(↑) / DXY(↓) / VIX(↓)
- **판정:** **TRANSITION / MIXED | Flow:  (Flow Weak)**
- **근거:** Macro Narrative=UNKNOWN_TRANSITION / Policy=MIXED / Credit=COOL

### 💧 2) Liquidity Filter (Enhanced)
- **질문:** 시장에 새 돈이 들어오는가, 말라가는가?
- **추가 이유:** US10Y/DXY/VIX는 ‘시장의 기대’를 보여주고, FCI는 ‘현실의 압박’을, Real Rates는 ‘위험을 감수할 유인’을 보여준다.

- **기대(가격) 신호:** US10Y(↑) / DXY(↓) / VIX(↓)
- **현실(FCI):** value=-0.506 / level=EASY (완화) / update=low-frequency | as of: 2026-06-13 (latest available)
- **유인(Real Rates):** value=2.160 / level=RESTRICTIVE (유인↓) / dir(→) | as of: 2026-06-13 (latest available)
- **판정:** **LIQUIDITY MIXED / FRAGILE (혼조·취약)**
- **근거:** 기대(가격)와 현실(FCI)/유인(실질금리) 정렬이 불완전
- **Note:** FCI는 저빈도 금융환경 프록시로 level 중심 해석, Real Rates는 영업일 기준 변화 방향을 함께 반영함

### 🏛️ 3) Policy Filter (with Expectations)
- **질문:** 중앙은행·정책 환경은 완화인가, 긴축인가?

- **가격(현재) 신호:** US10Y(↑) / DXY(↓) / VIX(↓)
- **Policy Bias: MIXED (혼조) (WEAK, score=-0.5) | REAL_RATEΔ +0.000 / FCI value=-0.506 (low-frequency) / DXYΔ -0.110 / US10YΔ +0.024**
- **Expectations: dict received.**

- **판정:** **POLICY MIXED (정책 신호 혼조)**
- **근거:** 금리/달러/변동성 신호가 완전히 정렬되지 않음
- **한줄요약 ~~** 구조=MIXED (혼조)(WEAK)는 참고, 가격=POLICY MIXED (정책 신호 혼조) 중심 → 최종 POLICY MIXED (정책 신호 혼조)

### 🧰 4) Fed Plumbing Filter (TGA/RRP/Net Liquidity)
- **질문:** 시장의 ‘달러 체력’은 늘고 있나, 줄고 있나?
- **추가 이유:** 금리·달러가 안정적이어도 유동성이 빠지면 리스크 자산은 쉽게 흔들릴 수 있음
- **Liquidity as of:** 2026-06-10 (FRED latest)
- **NET_LIQ level:** 5897274.6
- **TGA level:** 828122.0
- **RRP level:** 0.387
- **방향(전일 대비):** TGA(↓) / RRP(↓) / NET_LIQ(↑)
- **판정:** **LIQUIDITY SUPPORTIVE (완만한 유동성 우호)**
- **근거:** Net Liquidity↑ → 시장 내 달러 여력 개선
- **Note:** TGA/RRP/WALCL은 매일 갱신되지 않을 수 있어, 리포트에는 ‘최근 available 값’을 반영함

### 🌡️ 4.2) High Yield Spread Filter (HY OAS)
- **질문:** 시장 공포의 ‘온도’는 올라가고 있나, 내려가고 있나?
- **추가 이유:** HYG/LQD가 ‘방향’이라면, HY Spread는 ‘강도(얼마나 무서워하는지)’를 보여줌
- **Spread as of:** 2026-06-11 (FRED latest)
- **HY_OAS level:** 2.78% → **COOL (낮은 공포)**
- **방향(전일 대비):** HY_OAS(↓) / -0.71%
- **판정:** **CREDIT CALM**
- **근거:** HY 스프레드 낮음 → 크레딧 스트레스 제한 / 스프레드가 좁혀지는 중 → 공포 온도 완화
- **Note:** HY OAS는 매일 갱신되지 않을 수 있어, ‘최근 available 값’을 반영함

### 🧾 4.5) Credit Stress Filter (HYG vs LQD)
- **질문:** 크레딧 시장이 먼저 ‘리스크오프’를 말하고 있는가?
- **추가 이유:** HYG가 LQD보다 약해지면, 시장이 ‘위험을 감수할 이유가 없다’고 판단하기 시작했을 가능성
- **방향(전일 대비):** HYG(→) / LQD(↓)
- **HYG:** today 79.940 / prev 79.940 / pct 0.00%
- **LQD:** today 109.010 / prev 109.080 / pct -0.06%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, +0.54%)** → 완화 기대 약화/금리 부담
- DXY **(Mild, -0.11%)** → 달러 약세/리스크 선호
- WTI **(Strong, -3.23%)** → 물가 부담 완화
- VIX **(Strong, -9.05%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Clear, -0.47%)** → 원화 강세/수급 개선
- HYG (High Yield ETF) **(Noise, +0.00%)** → 보합(크레딧 변화 제한)
- LQD (IG Bond ETF) **(Noise, -0.06%)** → 우량채 약세(리스크온 성향)

### 🧩 6) Cross-Asset Filter (자산군 연쇄 반응 분석)
- **추가 이유:** 단일 지표의 노이즈를 제거하고, 매크로 충격이 자산군 전반으로 확산되는 **전이 경로(Transmission Path)**를 파악하기 위함

- **금리 상승(US10Y↑)** → 실질 금리 압박 → 달러 강세(DXY↑) 유도: **신흥국 자본 유출 및 고밸류 성장주 할인율 부담 증가**
- **변동성 하락(VIX↓)** → 심리 개선(Risk-On): **자산군 전반의 위험 수용 여력(Risk Appetite) 회복 및 랠리 지속 가능성**
- **유가 하락(WTI↓)** → 물가 부담 완화: **실질 구매력 회복 및 긴축 압력 완화(Dovish Tilt) 가능성 시사**

> **[Strategic Note]:** 위 연쇄 반응이 역사적 상관관계에서 벗어날 경우, **6.5) Correlation Break Monitor**를 통해 국면 전환 여부를 정밀 판별함

### 🌊 Drift Monitor (v4)
- **정의:** 누적 흐름 + ATR 기반 강도 감지

- **SPY:** 🟢 UP | Short-term: SHORT UP | 1D=+0.53% / 5D=+0.56% | Strength: LOW
- **WTI:** 🔴 DOWN | Short-term: MIXED | 1D=-3.90% / 5D=-6.90% | Strength: HIGH
- **DXY:** 🔴 DOWN | Short-term: SHORT UP | 1D=-0.05% / 5D=-0.26% | Strength: LOW
- **GOLD:** 🟡 REBOUND | Short-term: SHORT UP | 1D=+3.66% / 5D=-2.24% | Strength: MEDIUM

- **Drift Score:** 7
- **State:** **⚡ STRUCTURAL DRIFT**
- **Label:** NEUTRAL
- **SEW Combo Signal:** 🟢 EARLY FLOW WITHOUT SHOCK

- **Market Drift Summary:**
  - Equity (SPY): 🟢 UP / SHORT UP
  - Oil (WTI): 🔴 DOWN / MIXED
  - Dollar (DXY): 🔴 DOWN / SHORT UP
  - Gold (GOLD): 🟡 REBOUND / SHORT UP

- **Drivers:**
  - SPY positive drift
  - Credit supports risk
  - Sector breadth expanding
  - Broad cyclical leadership
  - Dollar not restrictive
  - Gold event move

### ⚠ 6.5) Correlation Break Monitor
⚠ Market Closed / Stale Data → Correlation signals evaluated conservatively.

Correlation Break Detected:
- US10Y ↑ but Technology ↑
- US10Y ↑ but SPY ↑

So What?
- 결론: **공식이 깨진 구간** → 총노출 추가 감산보다 **추격 자제 + 퀄리티/리더 중심 배분**

### ⚠ 6.6) Sector Correlation Break Monitor
⚠ Market Closed / Stale Data → Sector signals evaluated conservatively.

Correlation Break Detected:
- US10Y ↑ but XLRE ↑
- US10Y ↑ but XLK ↑

So What?
- 결론: **섹터 ‘공식’이 깨진 구간** → 총노출 추가 감산보다 **섹터 배분 보수화 + 리더 중심**

### 🧩 7) Risk Exposure Filter (숨은 리스크 분석)
- **추가 이유:** 숫자는 괜찮아 보여도 그 뒤에 숨은 리스크를 식별하기 위함

- **VIX 하락(VIX↓)** → 심리 안정: 리스크 수용 여력 개선
- **금리 상승(US10Y↑)** → 할인율 부담/유동성 압박 가능
- **달러 약세(DXY↓)** → 위험자산 선호/신흥국 부담 완화 가능
- **유가 하락(WTI↓)** → 물가 부담 완화 가능

### 🛰️ 7.2) Geopolitical Early Warning Monitor (FX/Commodities Composite)
⚠ Market Closed / Stale Data → Price-based geo signals muted.

- **Geo Stress Score (z-composite):** **-0.47**  *(Level: NORMAL)*
- **Coverage:** 100% *(used weight: 1.30 / defined weight: 1.30)*
- **3D Avg Score:** -0.17
- **Geo Momentum:** -0.30 *(Status: FALLING)*

**Historical Pattern Match (Cosine Similarity):**
- **Closest Historical Match:** Israel_2023
- **Cosine Similarity Score:** -0.431
- **Similarity Signal:** Weak Historical Match
- **Top Similarity Matches:**
  - Israel_2023: -0.431
  - Iran_Crisis_2020: -0.453
  - Taiwan_Tension: -0.478
- **Top Drivers:**
  - VIX: z_used=-1.14 (z1d=-1.09, z5d=-1.22, raw_w=0.18, norm_w=0.14) → contrib=-0.16
  - USDCNH: z_used=-0.64 (z1d=-0.98, z5d=-0.13, raw_w=0.18, norm_w=0.14) → contrib=-0.09
  - GOLD: z_used=+0.94 (z1d=+1.81, z5d=-0.36, raw_w=0.12, norm_w=0.09) → contrib=+0.09
  - WTI: z_used=-0.67 (z1d=-0.66, z5d=-0.67, raw_w=0.10, norm_w=0.08) → contrib=-0.05
- **Missing/Skipped:** None
- **Sovereign Spread factors included:** KR10Y_SPREAD, JP10Y_SPREAD, DE10Y_SPREAD, IL10Y_SPREAD

**Trade Information:**
- 지정학 스트레스는 여전히 정상 범위에 있지만 최근 압력이 완화되고 있는 중입니다. 경계 유지.
- 역사적 위기 패턴 유사도는 낮습니다. 현재는 **Israel_2023** 유형과 가장 가깝지만, 전면적 지정학 쇼크보다는 제한적·국지적 리스크 모니터링 구간으로 해석됩니다.
- **Country ETF Crash?** No (BND, EEM, EIS, EMB, EWJ, FXI, GLD, SPY, VXX)

### ⚡ 7.3) Pseudo Gamma Filter
- **정의:** 옵션 데이터 없이 시장의 감마 환경을 추론
- **주의:** Dealer Gamma Bias 숫자와 Pseudo Gamma State는 서로 다른 레이어

- **Pseudo Gamma State:** 🟡 POSITIVE-TRANSITION
- **Dealer Gamma Bias:** 1.03 (NEUTRAL / transition zone)
- **Bias:** VIX는 안정적이나 Drift가 형성 중
- **Strategy:** 초기 방향성 관찰 / 과도한 추격 금지

- **Drift Score:** 7 (⚡ STRUCTURAL DRIFT)
- **VIX:** 17.68000030517578
- **SEW:** STABLE / NORMAL

- **🚀 Combo Signal:** 🟢 EARLY FLOW WITHOUT SHOCK

### 🏦 Institutional Flow Engine (v2-minimal)
- **정의:** 기관성 자금이 뉴스 전에 남기는 흔적을 구조적으로 탐지

- **Raw Flow State:** **⚡ BUILDING**
- **Transition State:** **CONFIRMED_FLOW**
- **Flow Delta:** +0 (prev=6 → current=6)
- **Persistence Days:** 9
- **Transition Note:** 기관성 흐름이 높은 강도로 확인
- **Confidence:** **MEDIUM-HIGH**
- **Action Bias:** **WATCHLIST**

- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / 🟢 EARLY FLOW WITHOUT SHOCK
- **Gamma:** 🟡 POSITIVE-TRANSITION / 🟢 EARLY FLOW WITHOUT SHOCK
- **SEW:** STABLE / NORMAL
- **Positioning (POS_Z):** 1.84
- **Validation Score:** 3 (boost applied: +2)

- **Drivers:**
  - Drift strong
  - Gamma transition
  - No shock yet
  - Positioning somewhat stretched
  - Cross-asset risk participation
  - Credit confirms risk appetite
  - Leadership breadth expanding

### 🎯 8) Incentive Filter (Wall St. Logic)

**핵심 신호:** 장단기차(0.00bp) | 실질금리(2.16%) | DXY(99.75)
*(as of: RealRate: 2026-06-13 / FRED last available)*

❌ **자본이 탈출하는 곳 (Short Incentive):**
고금리(실질금리 2% 상회) 부담으로 인한 리스크 오프 신호

- **Note:** 실질금리와 달러는 자본의 '기회비용'을 결정하는 핵심 유인책입니다.

### 🔍 9) Cause Filter
- **질문:** 무엇이 이 움직임을 만들었는가?
- **핵심 신호:** 금리↑ + 달러↓ + 유가↓ + VIX↓
- **최종 판정:** **매크로 지표 혼조 속 시장 심리 개선 주도**

### 🔄 10) Direction Filter
- **질문:** 오늘 움직임은 ‘노이즈’인가 ‘의미 있는 변화’인가?
- **강도:** US10Y(Strong) / DXY(Mild) / WTI(Strong) / VIX(Strong)
- **판정:** **SIGNIFICANT MOVE (의미 있는 변화)**

### ⏳ 11) Timing Filter
- **질문:** 이 신호는 단기/중기/장기 중 어디에 더 중요하게 작용하는가?
- **가이드:**
  - 금리/달러의 ‘레벨’ 변화는 중기(수 주~수개월) 영향이 더 큼
  - VIX 급등/급락은 단기(수 일~수 주) 심리 변화에 민감
- **Today snapshot:** US10Y(4.487), DXY(99.750), VIX(17.68)

### 🏗️ 12) Structural Filter (v3)
- **질문:** 글로벌 화폐 가치와 에너지 패권 등 '판'의 변화가 있는가?
- **핵심 신호:** US10Y(↑) / DXY(↓) / GOLD(↑) / VIX(↓) / WTI(↓)
- **Meaningful Move Check:** DXY=-0.11015482643624154 / GOLD=3.048674906076919 / US10Y=0.5377586558555082 / VIX=-9.053498871043908 / WTI=-3.226544134756107
- **판정:** **NEUTRAL**
- **근거:** 글로벌 매크로 구조의 특이 신호가 감지되지 않음



### 12.5) Growth Sustainability Filter [SHADOW]
- **Score:** 6
- **Label:** DURABLE_GROWTH
- **Demand Proxy:** 1
- **Financing:** 2
- **Energy Burden:** 2
- **Policy Capacity:** 1
- **Strategic Interpretation:** Growth structure looks durable: demand, financing, and policy conditions are broadly supportive.
- **Input Check:** US10Y=4.486999988555908, RealYield=2.16, T10Y2Y=nan, WTI=84.87999725341797, DXY=99.75, LiquidityDir=UP, CreditCalm=True, HY_OAS=2.78, DriftLabel=NEUTRAL, FredAsof=2026-06-12

📌 Shadow Note: This filter is observation-only and does not affect Final Exposure, Phase, or Sector Allocation.




### 12.8) Positioning Stress Filter [SHADOW]

- **Score:** -1
- **Label:** SQUEEZE_RISK
- **Strategic Interpretation:** Positioning is stretched and vulnerable to squeeze-driven reversals.

**Positioning Notes**
- Term Structure: VIX3M-VIX=2.83 → healthy contango / stable structure
- Short-Term Hedge: VIX9D/VIX=0.98 → neutral short-term hedge
- Gamma Structure: Positive gamma extreme → dealer-supported squeeze risk
- Positioning: Elevated long positioning

📌 Shadow Note: This filter estimates whether current market behavior reflects structural participation or unstable positioning stress (squeeze / unwind / panic). No impact on Final Exposure or Phase, but used as context for Sector Allocation risk controls.



### 12.6) Flow Authenticity Filter [SHADOW]
- **Score:** 7
- **Label:** REAL_ACCUMULATION
- **Strategic Interpretation:** Institutional participation appears broad and persistent.
- **Breadth / Participation:** 3
- **Breadth Note:** RSP-SPY return spread=0.37%p → broad participation
- **Nasdaq Breadth Note:** QQQE-QQQ return spread=0.59%p → broad Nasdaq participation
- **Positioning / Gamma:** 0
- **Credit Confirmation:** 2
- **Macro Participation:** 2

📌 Shadow Note: This filter estimates whether upside is driven by real accumulation or short-covering. No impact on Final Exposure, Phase, or Allocation.



### 12.7) Leadership Breadth Filter [SHADOW]
- **Score:** 6
- **Label:** BROAD_LEADERSHIP
- **Strategic Interpretation:** Leadership is broadening across sectors, confirming healthier market participation.

**Leadership Notes**
- QQQ-SPY spread=0.05%p → neutral growth leadership
- SMH-QQQ spread=1.14%p → semiconductor participation strong
- SOXX-QQQ spread=1.00%p → SOX proxy confirms chip breadth
- IWM-SPY spread=0.33%p → small-cap participation confirms broader risk-on
- XLF-SPY spread=0.83%p → sector diffusion positive
- XLI-SPY spread=0.05%p → sector diffusion neutral
- XLY-SPY spread=-0.28%p → sector diffusion neutral

📌 Shadow Note: This filter checks whether leadership is broadening beyond mega-cap tech/AI. No impact on Final Exposure or Phase, but used as context for Sector Allocation risk controls.


### 🧠 13) Narrative Engine (v2 + Risk Budget + Drift)
- **정의:** 구조·심리·크레딧·유동성·국면을 통합해 오늘의 리스크 액션을 결정
- **추가 이유:** 지표는 많지만 전략가는 결국 ‘리스크를 늘릴지/줄일지/유지할지’를 판단해야 하기 때문

- **Structure Bias:** Policy Bias: MIXED (혼조) (WEAK, score=-0.5) | REAL_RATEΔ +0.000 / FCI value=-0.506 (low-frequency) / DXYΔ -0.110 / US10YΔ +0.024 (정상)
- **Sentiment (Fear&Greed):** 56.61605525725201 (NEUTRAL)
- **Credit Calm:** True
- **Liquidity (NET_LIQ):** UP (MID)
- **Structural Regime:** UNKNOWN_TRANSITION
- **Operational Phase:** TRANSITION / MIXED (Cap: 65)
- **Macro Tilt:** +0
- **Drift:** ⚡ STRUCTURAL DRIFT / NEUTRAL / 🟢 EARLY FLOW WITHOUT SHOCK
- **Drift Score:** 7
- **Flow Score:** 6
- **Flow Continuity:** TRACE_BUILDING → ⚡ BUILDING (FLOW_PERSISTENCE, tilt=+1)
- **Flow Regime Tilt:** +3 / Flow-Gamma Tilt: +2

- **🎯 Final Risk Action:** **HOLD**
- **Risk Budget (0~100):** **65**
- **Narrative:** 구조=MIXED / 심리=NEUTRAL / 유동성=증가/중간 / 크레딧=안정 / 드리프트=⚡ STRUCTURAL DRIFT (NEUTRAL) / 수급=1.84 ⚠️ 수급 다소 과열 → Phase=TRANSITION / MIXED

### ⚠ 14) Divergence Monitor (Macro vs Positioning)
- **추가이유:** 시장 가격과 정책 사이의 괴리 및 수급의 '질'을 파악하여 폭발적 반전 가능성 진단
- **핵심질문:** 정책은 이런데 주가는 왜 반대로 가지?(Anomaly) 그 뒤에 숨은 수급 주체(CTA, Dealer)들은 지금 어떤 상태인가?

- **Structure(3번):** `MIXED` | **Price(Regime):** `TRANSITION / MIXED` | **Bucket:** `MIXED` | **VIX:** `17.68`
- **Positioning Data:** Z-Score: `1.84` (>1.8 시 Run) | Gamma: `1.03` (<0.5 시 Run) | CTA: `1.0` (추세 변곡점 확인)
- **Status:** **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:1.84). 반전 가능성 상존.
- **Action Signal:** 🚨 **MONITOR REVERSAL (RUN 액션 준비)**

### 🎯 15) Volatility-Controlled Exposure (v3.2)
- **정의:** 13번 Risk Budget 실행 브레이크 레이어
- **추가 이유:** 전략 판단(13) 이후 실제 진입 강도를 조절하기 위함

- **Base Risk Budget (13):** 65
- **VIX Level:** 17.68 (NORMAL) | **Change:** -9.05%
- **Positioning Layer:** ⚠️ Elevated Positioning Heat(1.84)
- **Brake Drivers:** ⚠️ Elevated Positioning Heat

- **📊 Recommended Exposure:** **61%**

### 🎨 16) Style Tilt (v1.1)
- **정의:** Macro 구조 기반 스타일 기울기 판단
- **추가 이유:** 같은 Risk-On이라도 어떤 유형의 자산이 유리한지 구분

- **Growth vs Value:** **VALUE TILT**
- **Duration Tilt:** **SHORT DURATION FAVORED**
- **Cyclical vs Defensive:** **NEUTRAL**

### 🧩 17) Factor Layer (v1)
- **정의:** 시장을 움직이는 핵심 위험 요인 판별
- **추가 이유:** 자금이 무엇에 민감하게 반응하는지 파악

- **Duration Factor:** SHORT DURATION FAVORED
- **Inflation Factor:** NEUTRAL
- **USD Factor:** NEUTRAL
- **Credit Factor:** CREDIT SUPPORTIVE

### 🏭 18) Sector Allocation Engine (v3.3)

**Context:** phase=TRANSITION / MIXED / T10Y2Y=0.00 (FLAT / FRAGILE) / VIX=17.68 (VOLATILITY NORMAL) / liquidity=UP-MID / credit=True

**Signal Priority:** VOL > LIQ > CURVE > CREDIT > PHASE > FLOW > MOM

**Macro Profile:** BALANCED
**Macro Inputs Debug:** phase=TRANSITION / MIXED / us10y_pct=+0.54% / dxy_pct=-0.11% / wti_pct=-3.23% / vix=17.68 / liq_easy=True / liq_tight=False / credit_calm=True / flow_score=6

**Flow Overlay:** flow_score=6 / flow_state=⚡ BUILDING / drift_label=NEUTRAL / gamma=🟡 POSITIVE-TRANSITION
**Flow Notes:** NEUTRAL + FLOW ACTIVE → XLK/XLI 소폭 가점 | Gamma POSITIVE → 리더 섹터 가점

**Overweight:** Technology, Consumer Staples, Health Care, Industrials, Financials

**Underweight:** Consumer Discretionary, Utilities, Energy, Real Estate

**Scoreboard:**
- Technology: +2.0  (+2 LIQ, +1.5 FLOW, +2 MOM, = +2.0)
- Consumer Staples: +0.9  (+1 CURVE, +0.5 PHASE, = +0.9)
- Health Care: +0.9  (+1 CURVE, +0.5 PHASE, = +0.9)
- Industrials: +0.8  (+1.5 LIQ, +1 FLOW, -1 MOM, = +0.8)
- Financials: +0.6  (+1 LIQ, +1 MOM, = +0.6)
- Real Estate: -0.2  (= -0.2)
- Energy: -0.4  (-2 MOM, = -0.4)
- Utilities: -0.6  (-1 LIQ, = -0.6)
- Consumer Discretionary: -1.2  (+1.5 LIQ, -1 MOM, = -1.2)

**Rationale (Why the score exists: 섹터 점수의 핵심 드라이버)**
- OW Technology: +2: 유동성 완화 → 성장주/베타 우호
- OW Technology: +1: Gamma Overlay → POSITIVE, 추세 지속 우호
- OW Consumer Staples: +1: 플랫 커브(0.00) → 경기 민감도 낮은 섹터 선호
- OW Consumer Staples: +0.5: Balanced Macro Profile → 방어 보완
- OW Health Care: +1: 플랫 커브(0.00) → 방어/퀄리티 선호
- OW Health Care: +0.5: Balanced Macro Profile → 퀄리티 보완
- OW Industrials: +1.5: 유동성 완화 → 경기민감 회복
- OW Industrials: +0.5: Flow Overlay → 경기민감 확인용 가점
- UW Consumer Discretionary: THEORY_TRAP → 거시/이론 우호 대비 실제 자금흐름 및 상대강도 약세
- UW Consumer Discretionary: +1.5: 유동성 완화 → 소비 민감주 우호
- UW Consumer Discretionary: -1: Relative Strength 약세 (vs SPY) → 소외 섹터
- UW Utilities: -1: 유동성 완화 → 방어주 상대매력 저하

**Regime Controller:**
- BALANCED (avg_divergence=-0.72, dispersion=0.97)
- Correlation Break: True / Type=UNCLASSIFIED_SECTOR_BREAK
- Break Reasons: US10Y ↑ but Technology ↑ | US10Y ↑ but SPY ↑ | US10Y ↑ but XLRE ↑ | US10Y ↑ but XLK ↑
- Interpretation: 균형 장세 / 강한 방향성보다 선별적 배분 필요

**Divergence / Classification Monitor (Theory vs Flow alignment: 이론과 실제 자금흐름 정렬 여부)**
- Technology: HIGH_CONVICTION_ALIGNED (theory=+2.0, flow=+1.9, final=+2.0)
- Communication Services: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Materials: NEUTRAL (theory=+0.0, flow=+0.0, final=+0.0)
- Real Estate: NEUTRAL (theory=+0.0, flow=+0.0, final=-0.2)
- Consumer Discretionary: THEORY_TRAP (theory=+1.5, flow=-0.7, final=-1.2)

### 💰 18.5) Tactical Asset Allocation (Execution Weight)
- **Strategic Exposure (15):** **61.0%** → **Regime Adjusted:** **61.0%**
- **Exposure Override:** BALANCED → Sector Weight Only (No Exposure Change)

| Sector | Score | Divergence | **Weight in Portfolio** | **Action** |
| :--- | :---: | :---: | :---: | :--- |
| Technology | +2.0 | ALIGNED | **15.0%** | DELEVERAGE |
| Consumer Staples | +0.9 | ALIGNED | **10.2%** | DELEVERAGE |
| Health Care | +0.9 | ALIGNED | **10.2%** | DELEVERAGE |
| Industrials | +0.8 | ALIGNED | **6.8%** | DELEVERAGE |
| Financials | +0.6 | ALIGNED | **7.0%** | DELEVERAGE |
| **Cash & Hedge** | - | - | **50.8%** | DEFENSIVE |

- **Allocation Check:** Sector Weights + Cash = **100.0%**
- **Regime Cap Profile:** BALANCED
- **Participation / Quality Cap Applied:**
  - Technology: 26.8% → 15.0% (-11.8%)
- **Strategic Cash (15):** 39.0%
- **Tactical Reserve (Cap / Unallocated):** 11.8%


**Deleveraging Priority Preview:**
- 기준: Divergence → Momentum → Score → Current Weight
1. Industrials (priority_score=1.12, score=0.76, weight=6.8%, div=ALIGNED, mom=-1)
2. Consumer Staples (priority_score=-0.39, score=0.88, weight=10.2%, div=ALIGNED, mom=0)
3. Health Care (priority_score=-0.39, score=0.88, weight=10.2%, div=ALIGNED, mom=0)
4. Financials (priority_score=-1.3, score=0.61, weight=7.0%, div=ALIGNED, mom=1)
5. Technology (priority_score=-7.23, score=2.02, weight=15.0%, div=ALIGNED, mom=2)

**Leveraging Priority Preview:**
- 기준: Score → Momentum → Positive Divergence
1. Technology (priority_score=7.02, score=2.02, weight=15.0%, div=ALIGNED, mom=2)
2. Financials (priority_score=2.11, score=0.61, weight=7.0%, div=ALIGNED, mom=1)
3. Consumer Staples (priority_score=0.88, score=0.88, weight=10.2%, div=ALIGNED, mom=0)
4. Health Care (priority_score=0.88, score=0.88, weight=10.2%, div=ALIGNED, mom=0)
5. Industrials (priority_score=0.76, score=0.76, weight=6.8%, div=ALIGNED, mom=-1)

### 🧬 19) Execution Layer (ETF Mapping)

| Sector | ETF | Weight | Action | Divergence | Classification |
| :--- | :---: | :---: | :--- | :--- | :--- |
| Technology | XLK | 15.0% | ACCUMULATE | ALIGNED | HIGH_CONVICTION_ALIGNED |
| Consumer Staples | XLP | 10.2% | ADD | ALIGNED | ALIGNED |
| Health Care | XLV | 10.2% | ADD | ALIGNED | ALIGNED |
| Industrials | XLI | 6.8% | SMALL | ALIGNED | ALIGNED |
| Financials | XLF | 7.0% | SMALL | ALIGNED | ALIGNED |


### 🧬 19.5) Execution / Style Translation Layer
- **Implementation Focus:** Environment-Aware Stock Types

**Execution Notes:**
- Flow building → selective expansion allowed if risk controls remain stable.
- Positioning heat elevated → prefer rebalancing over fresh chasing.

**Preferred Company Traits:**
- High-conviction leaders with cross-asset confirmation

**Risk Control / Avoid:**
- Crowded late-entry trades

---


---

## 🌐 Country ETF Risk Monitor

### BND
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** -0.4626802655988731
- **Z-Score (5d):** 1.077545579070084

### EEM
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.1307592798416644
- **Z-Score (5d):** 0.988442427174068

### EIS
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.6418453043681058
- **Z-Score (5d):** 0.9862969465461385

### EMB
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.06775343404689883
- **Z-Score (5d):** 0.777316417944756

### EWJ
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.2602736493576507
- **Z-Score (5d):** 0.6042863023566558

### FXI
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.8638007588611726
- **Z-Score (5d):** 0.8185452821729277

### GLD
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.15362842513613878
- **Z-Score (5d):** -0.2771044970778852

### SPY
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** 0.3613436043258432
- **Z-Score (5d):** -0.14606718707459268

### VXX
- **Crash?** False
- **Risk Level:** NORMAL
- **Z-Score (1d):** -1.1740368158955021
- **Z-Score (5d):** -0.2936905755239369
