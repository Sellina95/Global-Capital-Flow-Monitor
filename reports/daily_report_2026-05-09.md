...
- [2026-05-09 04:21:16] ALERT | SEW=DEADMAN | EVENT=NORMAL | flow=FLOW_FADE -> NO_FLOW_BASE | flow_delta=-1 | persistence=0 | flow_alert=NONE | Exp=0% | 🚨 DEAD MAN'S SWITCH: POS_Z Extreme (2.13) | spike=0 extreme=0 | corr_break=NO | z={'SPY': 0.3491320667064861, 'QQQ': 0.5694943441311158, 'VIX': 0.6358953133332057, 'DXY': -6.143831223753569e-05, 'WTI': 0.6645642969160414}
...
- **[14번 구조·수급 괴리]:** 🚨 **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.20). 반전 가능성 상존.
- **[실시간 보초병(SEW)]:** DEADMAN | 🚨 데드맨 스위치 발동 (익스포저 0% / 자산 보호 모드)
- **[SEW Event Type]:** NORMAL
  → 해석: 정상 상태 / 구조적 리스크 없음
- **[SEW Spike Monitor]:** Spike 0 / Extreme 0
- **[SEW Deadman Reason]:** 🚨 DEAD MAN'S SWITCH: POS_Z Extreme (2.13)
- **[15번 데드맨]:** 🚨 ACTIVATED
...
- **Why:** SEW DEADMAN 발동 → 즉시 EXIT / 익스포저 0% → 상위 레이어(SEW/Divergence)가 Narrative보다 우선 → Warning Score 1 → 경미한 이상신호, 모니터링 강화 → Tactical=REDUCE / Flow=NO CLEAR FLOW(1) / Drift=WEAK DRIFT (노이즈 가능)(1) / Gamma=🟢 POSITIVE GAMMA → Tactical REDUCE → 방어 기조 유지 / 익스포저 5% 추가 축소
...
- **원/달러 환율**: 1461.480  (+1.18% vs 1444.450)
...
- **현실(FCI):** level=EASY (완화) / dir(→) | as of: 2026-05-09 (FRED last available)
- **유인(Real Rates):** level=NEUTRAL (중립) / dir(→) | as of: 2026-05-09 (FRED last available)
...
- 원/달러(USDKRW) **(Strong, +1.18%)** → 원화 약세/수급 부담
...
- **WTI:** 🔴 DOWN | Short-term: SHORT DOWN | 1D=-0.14% / 5D=-7.12% | Strength: MEDIUM
- **DXY:** 🔴 DOWN | Short-term: SHORT DOWN | 1D=-0.41% / 5D=-0.37% | Strength: LOW
- **GOLD:** 🟢 UP | Short-term: SHORT DOWN | 1D=+0.51% / 5D=+2.03% | Strength: LOW
...
*(as of: RealRate: 2026-05-09 / FRED last available)*
...
- **Meaningful Move Check:** DXY=-0.4173065263199746 / GOLD=0.43831862020386086 / US10Y=-0.6375200297132972 / VIX=0.6440316794081948 / WTI=0.6433927075829471
- **판정:** **NEUTRAL**
...
- **Sentiment (Fear&Greed):** 60.072630293345895 (NEUTRAL)
- **Credit Calm:** True
...
- **Narrative:** 구조=EASING / 심리=NEUTRAL / 유동성=증가/중간 / 크레딧=안정 / 드리프트=WEAK DRIFT (노이즈 가능) (DISINFLATION_RISK_ON) / 수급=2.20 ⚠️ 수급 과열 감지 → Phase=TRANSITION / MIXED (전환·혼조)
...
- **Positioning Data:** Z-Score: `2.20` (>1.8 시 Run) | Gamma: `1.69` (<0.5 시 Run) | CTA: `1.0` (추세 변곡점 확인)
- **Status:** **⚡ TREND EXHAUSTION** -> **해석:** 추세와 정책은 일치하나 포지션 에너지 고갈(Z:2.20). 반전 가능성 상존.
...
**Macro Inputs Debug:** phase=TRANSITION / MIXED (전환·혼조) / us10y_pct=-0.64% / dxy_pct=-0.42% / wti_pct=+0.64% / vix=17.19 / liq_easy=True / liq_tight=False / credit_calm=True / flow_score=1

**Flow Overlay:** flow_score=1 / flow_state=NO CLEAR FLOW / drift_label=DISINFLATION_RISK_ON / gamma=🟢 POSITIVE GAMMA
