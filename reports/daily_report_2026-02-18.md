...
- **달러 인덱스**: 97.250  (-0.03% vs 97.281)
- **WTI 유가**: 62.380  (-0.18% vs 62.490)
- **변동성 지수 (VIX)**: 19.380  (-0.05% vs 19.390)
- **원/달러 환율**: 1444.420  (-0.02% vs 1444.680)
...
- **VIX 레벨:** 19.38 → **Mid (Neutral/Mixed)**
...
- **Policy Bias: EASING (완화) (STRONG, score=-2.0) | REAL_RATEΔ -0.030 / FCIΔ +0.000 / DXYΔ -0.031 / US10YΔ +0.000**
- **Expectations: dict received.**

- **판정:** **POLICY EASING (structure-led) (구조 주도)**
- **근거:** 구조(REAL/FCI/DXY/US10Y)가 완화 방향으로 강함 → 가격신호(POLICY MIXED (정책 신호 혼조))는 확인/노이즈로 처리
- **한줄요약 ~~** 구조는 **EASING (완화)**, 가격은 **POLICY MIXED (정책 신호 혼조)** → 최종 **POLICY EASING (structure-led) (구조 주도)**
...
- WTI **(Mild, -0.18%)** → 물가 부담 완화
- VIX **(Noise, -0.05%)** → 심리 개선/리스크온
...
- **Today snapshot:** US10Y(4.052), DXY(97.250), VIX(19.38)
