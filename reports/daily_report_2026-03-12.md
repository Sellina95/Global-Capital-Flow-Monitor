...
- **미국 10년물 금리**: 4.208  (+1.74% vs 4.136)
- **달러 인덱스**: 99.251  (+0.34% vs 98.914)
- **WTI 유가**: 89.810  (+2.59% vs 87.540)
- **변동성 지수 (VIX)**: 24.230  (-2.81% vs 24.930)
- **원/달러 환율**: 1476.560  (+0.19% vs 1473.730)
...
- **Context:** phase=TRANSITION / MIXED (전환·혼조) / liquidity=UP-HIGH / credit_calm=True / geo=NORMAL
...
- **VIX 레벨:** 24.23 → **High (Risk-off bias)**
...
- **현실(FCI):** level=EASY (완화) / dir(→) | as of: 2026-03-09 (FRED last available)
- **유인(Real Rates):** level=NEUTRAL (중립) / dir(↑) | as of: 2026-03-09 (FRED last available)
...
- **Policy Bias: MIXED (혼조) (WEAK, score=+0.5) | REAL_RATEΔ -0.020 / FCIΔ +0.000 / DXYΔ +0.337 / US10YΔ +0.072**
...
- **Spread as of:** 2026-03-09 (FRED latest)
- **HY_OAS level:** 3.19% → **WARM (경계)**
- **방향(전일 대비):** HY_OAS(↑) / +1.92%
- **판정:** **CREDIT WATCH**
...
- **방향(전일 대비):** HYG(↓) / LQD(↓)
- **HYG:** today 79.860 / prev 80.040 / pct -0.22%
- **LQD:** today 109.160 / prev 110.060 / pct -0.82%
- **판정:** **CREDIT NEUTRAL**
- **근거:** HYG/LQD 방향성이 뚜렷하지 않음

### 📌 5) Directional Signals (Legacy Filters)
**추가 이유:** 개별 자산의 단기 방향성과 노이즈 강도를 구분해 과도한 해석을 방지하기 위함
- 미국 금리(US10Y) **(Strong, +1.74%)** → 완화 기대 약화/금리 부담
- DXY **(Clear, +0.34%)** → 달러 강세/신흥국 부담
- WTI **(Strong, +2.59%)** → 인플레 재자극 가능성
- VIX **(Strong, -2.81%)** → 심리 개선/리스크온
- 원/달러(USDKRW) **(Mild, +0.19%)** → 원화 약세/수급 부담
- HYG (High Yield ETF) **(Mild, -0.22%)** → 크레딧 스트레스↑
- LQD (IG Bond ETF) **(Clear, -0.82%)** → 우량채 약세(리스크온 성향)
...
- DEBUG: pct XLK=0.4793919542638087, XLF=-0.8389971476642144, XLE=2.48201637767591, XLRE=-1.1882533627464398
Correlation Break Detected:
...
- **Top Drivers:**
  - EMB: z_used=+1.17 (z1d=-1.15, z5d=-1.21, raw_w=0.12, norm_w=0.09) → contrib=+0.10
  - KR10Y_SPREAD: z_used=+1.28 (mode=level, raw_w=0.08, norm_w=0.06) → contrib=+0.08
  - TR10Y_SPREAD: z_used=+1.64 (mode=level, raw_w=0.05, norm_w=0.04) → contrib=+0.06
  - VIX: z_used=-0.37 (z1d=-0.46, z5d=-0.24, raw_w=0.18, norm_w=0.13) → contrib=-0.05
...
- **판정:** **금리 상승(US10Y↑) + 달러 강세(DXY↑) + 유가 상승(WTI↑) + 변동성 완화(VIX↓)**
...
- **Today snapshot:** US10Y(4.208), DXY(99.251), VIX(24.23)
...
- **Structure Bias:** Policy Bias: MIXED (혼조) (WEAK, score=+0.5) | REAL_RATEΔ -0.020 / FCIΔ +0.000 / DXYΔ +0.337 / US10YΔ +0.072
- **Sentiment (Fear&Greed):** 30.924530102162755 (NEUTRAL)
...
- **Narrative:** 구조=MIXED / 심리=NEUTRAL / 유동성=증가/높음 / 크레딧=안정 → Phase=TRANSITION / MIXED (전환·혼조)
