...
def _strength_label(key: str, pct_change: Optional[float]) -> str:
    ...
    if key in ("US10Y",):
        if p < 0.02: return "Noise"
        if p < 0.07: return "Mild"
        if p < 0.15: return "Clear"
        return "Strong"

    if key in ("DXY",):
        if p < 0.05: return "Noise"
        if p < 0.15: return "Mild"
        if p < 0.35: return "Clear"
        return "Strong"

    if key in ("WTI",):
        if p < 0.15: return "Noise"
        if p < 0.60: return "Mild"
        if p < 1.30: return "Clear"
        return "Strong"

    if key in ("VIX",):
        if p < 0.40: return "Noise"
        if p < 1.20: return "Mild"
        if p < 2.50: return "Clear"
        return "Strong"

    if key in ("USDKRW",):
        if p < 0.05: return "Noise"
        if p < 0.20: return "Mild"
        if p < 0.50: return "Clear"
        return "Strong"

    if key in ("TGA", "RRP", "NET_LIQ"):
        if p < 0.05: return "Noise"
        if p < 0.20: return "Mild"
        if p < 0.50: return "Clear"
        return "Strong"

    if p < 0.10: return "Noise"
    if p < 0.30: return "Mild"
    if p < 0.80: return "Clear"
    return "Strong"
...
def get_regime_label(market_data: Dict[str, Any]) -> str:
    ...
    combo = (_sign_from(us10y), _sign_from(dxy), _sign_from(vix))

    if combo == (0, 0, 0):
        return "WAITING / RANGE (대기·박스권)"
    if combo == (-1, -1, -1):
        return "RISK-ON (완화 기대·리스크 선호)"
    if combo == (1, 1, 1):
        return "RISK-OFF (긴축/불안·리스크 회피)"

    # partials
    if combo[2] == 0 and (combo[0] != 0 or combo[1] != 0):
        return "EVENT-WATCHING (이벤트 관망)"
    if combo[0] == 1 and combo[1] == 1 and combo[2] != -1:
        return "TIGHTENING BIAS (긴축 편향)"
    if combo[2] == -1 and (combo[1] == -1 or combo[0] == -1):
        return "RISK-ON (부분 정렬)"
    if combo[2] == 1 and (combo[1] == 1 or combo[0] == 1):
        return "RISK-OFF (부분 정렬)"

    return "TRANSITION / MIXED (전환·혼조)"
...
def market_regime_filter(market_data: Dict[str, Any]) -> str:
    ...
    lines.append(f"- **핵심 조합(전일 대비 방향):** US10Y({_dir_str(us10y_dir)}) / DXY({_dir_str(dxy_dir)}) / VIX({_dir_str(vix_dir)})")
...
def liquidity_filter(market_data: Dict[str, Any]) -> str:
    ...
    lines = []
    lines.append("### 💧 2) Liquidity Filter (Macro Conditions)")
    lines.append("- **질문:** 시장에 새 돈이 들어오는가, 말라가는가?")
    lines.append(f"- **핵심 신호:** US10Y({_dir_str(us10y_dir)}, {us10y_str}) / DXY({_dir_str(dxy_dir)}, {dxy_str}) / VIX({_dir_str(vix_dir)}, {vix_str})")
...
def policy_filter(market_data: Dict[str, Any]) -> str:
    ...
    lines.append(f"- **핵심 신호:** US10Y({_dir_str(us10y_dir)}) / DXY({_dir_str(dxy_dir)}) / VIX({_dir_str(vix_dir)})")
    lines.append(f"- **판정:** **{regime}**")
...
def fed_plumbing_filter(market_data: Dict[str, Any]) -> str:
    ...
    if tga["today"] is None and rrp["today"] is None and net["today"] is None:
        ...
        lines.append("- **Status:** Not ready (TGA/RRP/NET_LIQ not found in market_data)")
        return "\n".join(lines)
    ...
    lines.append(f"- **방향(전일 대비):** TGA({_dir_str(tga_dir)}) / RRP({_dir_str(rrp_dir)}) / NET_LIQ({_dir_str(net_dir)})")
    lines.append(f"- **판정:** **{state}**")
    lines.append(f"- **근거:** {rationale}")
...
def legacy_directional_filters(market_data: Dict[str, Any]) -> str:
    ...
    if "TGA" in market_data:
        lines.append(line("TGA", "TGA", "시장 유동성 흡수 가능성", "시장 유동성 완화 가능성", "TGA 보합"))
    if "RRP" in market_data:
        lines.append(line("RRP", "RRP", "단기 유동성 흡수 가능성", "단기 유동성 완화 가능성", "RRP 보합"))
    if "NET_LIQ" in market_data:
        lines.append(line("NET_LIQ", "NET_LIQ", "달러 여력 증가(우호)", "달러 여력 감소(경계)", "NET_LIQ 보합"))
